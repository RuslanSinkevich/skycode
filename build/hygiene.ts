...
// --- SKYCODE_FORK_BEGIN: helper for unicode check with comment stripping ---
function stripCommentsForUnicodeCheck(line: string): string {
	// Ignore common one-line block comment bodies like " * text"
	if (line.match(/\s+(\*)/)) {
		return '';
	}

	const inlineCommentIndex = line.indexOf('//');
	return inlineCommentIndex === -1 ? line : line.substring(0, inlineCommentIndex);
}
// --- SKYCODE_FORK_END ---
...
export function hygiene(some: NodeJS.ReadWriteStream | string[] | undefined, runEslint = true): NodeJS.ReadWriteStream {
	...
	const unicode = es.through(function (file: VinylFileWithLines) {
		...
		lines.forEach((line, i) => {
			...
			// --- SKYCODE_FORK_BEGIN: allow any unicode in comments ---
			line = stripCommentsForUnicodeCheck(line);
			// --- SKYCODE_FORK_END ---
