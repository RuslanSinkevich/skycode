...
class ViewsExtensionHandler implements IWorkbenchContribution {
	...
	private getViewContainer(value: string): ViewContainer | undefined {
		switch (value) {
			...
			// [SKYCODE] Fallback: also try the raw value so built-in containers
			// (e.g. 'workbench.panel.chat') can be targeted by extensions directly
			default: return this.viewContainersRegistry.get(`workbench.view.extension.${value}`) || this.viewContainersRegistry.get(value);
		}
	}
