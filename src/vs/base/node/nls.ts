...
export async function resolveNLSConfiguration({ userLocale, osLocale, userDataPath, commit, nlsMetadataPath }: IResolveNLSConfigurationContext): Promise<INLSConfiguration> {
	...
	try {
		...
		// --- SKYCODE_FORK_BEGIN: bootstrap language pack from built-in extension on first launch ---
...
// --- SKYCODE_FORK_BEGIN: bootstrap built-in language pack on first launch ---
/**
 * When Skycode ships with a built-in language pack extension (e.g. vscode-language-pack-ru),
