...
function readArgvConfigSync(): IArgvConfig {
	...
	// --- SKYCODE_FORK_BEGIN: ensure Russian locale by default ---
	if (!argvConfig.locale) {
		argvConfig = { ...argvConfig, locale: 'ru' };
		try {
			const raw = fs.readFileSync(argvConfigPath).toString();
			const patched = raw.replace(/\}[\s]*$/, ',\n\t"locale": "ru"\n}');
			fs.writeFileSync(argvConfigPath, patched);
...
function getUserDefinedLocale(argvConfig: IArgvConfig): string | undefined {
	...
	// [SKYCODE] Default to Russian if no locale is configured anywhere
	return 'ru';
}
