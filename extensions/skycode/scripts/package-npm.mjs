...
/**
 * NPM Package Builder for Skycode CLI
 *
 * This script builds the Skycode CLI NPM package (dist-standalone/).
...
async function main() {
	console.log("🚀 Building Skycode NPM Package\n")
...
/**
 * Copy CLI binaries (skycode and skycode-host) for all platforms
...
async function copyCliBinaries() {
	...
	for (const { os, arch } of platforms) {
		...
		// Copy skycode binary
		const skycodeSource = path.join(CLI_BINARIES_DIR, `skycode-${platformSuffix}`)
		const skycodeDest = path.join(binDir, `skycode-${platformSuffix}`)

		if (!fs.existsSync(skycodeSource)) {
			console.error(`Error: CLI binary not found at ${skycodeSource}`)
			console.error(`Please run: npm run compile-cli-all-platforms`)
			process.exit(1)
		}

		await cpr(skycodeSource, skycodeDest)
		fs.chmodSync(skycodeDest, 0o755)
		console.log(`✓ skycode-${platformSuffix} copied`)

		// Copy skycode-host binary
		const hostSource = path.join(CLI_BINARIES_DIR, `skycode-host-${platformSuffix}`)
		const hostDest = path.join(binDir, `skycode-host-${platformSuffix}`)
		...
		console.log(`✓ skycode-host-${platformSuffix} copied`)
	}
 ...
 * Ripgrep is needed by skycode-core for file searching
...
async function createNpmPackageFiles() {
	...
	const manPageSource = path.join("cli", "man", "skycode.1")
	const manDir = path.join(BUILD_DIR, "man")
	const manPageDest = path.join(manDir, "skycode.1")
...
function setupBinaries() {
	...
	console.log(\`Setting up Skycode CLI for \${cliPlatformSuffix}...\`);

	// Setup CLI binaries
	const binDir = path.join(__dirname, 'bin');
	
	// Check if platform-specific binaries exist
	const skycodeSource = path.join(binDir, \`skycode-\${cliPlatformSuffix}\`);
	const skycodeHostSource = path.join(binDir, \`skycode-host-\${cliPlatformSuffix}\`);
	
	if (!fs.existsSync(skycodeSource)) {
		console.error(\`Error: Binary not found for platform \${cliPlatformSuffix}\`);
		console.error(\`Expected: \${skycodeSource}\`);
		console.error(\`Supported platforms: darwin-arm64, darwin-amd64, linux-amd64, linux-arm64\`);
		process.exit(1);
	}
	
	if (!fs.existsSync(skycodeHostSource)) {
		console.error(\`Error: Binary not found for platform \${cliPlatformSuffix}\`);
		console.error(\`Expected: \${skycodeHostSource}\`);
		process.exit(1);
	}

	// Create symlinks or copies to the generic names
	const skycodeTarget = path.join(binDir, 'skycode');
	const skycodeHostTarget = path.join(binDir, 'skycode-host');
	
	// Remove existing files if they exist
	[skycodeTarget, skycodeHostTarget].forEach(target => {
	...
	if (platform === 'win32') {
		// Windows: copy files
		fs.copyFileSync(skycodeSource, skycodeTarget);
		fs.copyFileSync(skycodeHostSource, skycodeHostTarget);
		console.log('✓ Copied platform-specific CLI binaries');
	} else {
		// Unix: create symlinks
		fs.symlinkSync(path.basename(skycodeSource), skycodeTarget);
		fs.symlinkSync(path.basename(skycodeHostSource), skycodeHostTarget);
		console.log('✓ Created symlinks to platform-specific CLI binaries');
		
		// Make binaries executable
		try {
			fs.chmodSync(skycodeSource, 0o755);
			fs.chmodSync(skycodeHostSource, 0o755);
			fs.chmodSync(skycodeTarget, 0o755);
			fs.chmodSync(skycodeHostTarget, 0o755);
		} catch (error) {
	...
	// Copy ripgrep binary to root (where skycode-core expects it)
	...
	console.log('✓ Skycode CLI installation complete');
	console.log('');
	console.log('Usage:');
	console.log('  skycode        - Start Skycode CLI');
	console.log('  skycode-host   - Start Skycode host service');
	console.log('');
