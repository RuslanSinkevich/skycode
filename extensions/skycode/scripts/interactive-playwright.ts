...
/**
 * Interactive Playwright launcher for the Skycode VS Code extension.
 *
 * Overview:
 *  - Starts the mock Skycode API server (from the e2e test fixtures).
 *  - Downloads a stable build of VS Code (via @vscode/test-electron).
 *  - Creates a temporary VS Code user profile directory.
 *  - Installs and links the Skycode extension (from dist/e2e.vsix and the dev path).
 *  - Opens a test workspace and automatically reveals the Skycode sidebar.
 ...
 *
 *   3. VS Code will launch with the Skycode extension loaded and gRPC recording enabled.
 *
...
import { SkycodeApiServerMock } from "../src/test/e2e/fixtures/server"
...
async function main() {
	...
	console.log("VSCode with Skycode extension is now running!")
