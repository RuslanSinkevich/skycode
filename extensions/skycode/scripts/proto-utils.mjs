...
// Get the fully qualified name for a proto type, e.g. getFqn('StringRequest') returns 'skycode.StringRequest'
...
export async function loadServicesFromProtoDescriptor() {
	...
	for (const [name, def] of Object.entries(proto.skycode)) {
		if (def && "service" in def) {
			protobusServices[name] = def
		} else {
			addTypeNameToFqn(name, `proto.skycode.${name}`)
		}
	}
