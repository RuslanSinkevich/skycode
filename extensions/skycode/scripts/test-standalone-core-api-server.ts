...
/**
 * Simple Skycode gRPC Server
 *
 * This script provides a minimal way to run the Skycode core gRPC service
 ...
 *   2. SkycodeApiServerMock (mock implementation of the Skycode API)
 ...
 *   SKYCODE_DIST_DIR - Override distribution directory (default: PROJECT_ROOT/dist-standalone)
 *   SKYCODE_CORE_FILE - Override core file name (default: skycode-core.js)
 ...
 *   SKYCODE_ENVIRONMENT - Environment setting (default: local)
 *
...
import { SkycodeApiServerMock } from "../src/test/e2e/fixtures/server/index"
...
const SKYCODE_ENVIRONMENT = process.env.SKYCODE_ENVIRONMENT || "local"
...
const distDir = process.env.SKYCODE_DIST_DIR || path.join(projectRoot, "dist-standalone")
const skycodeCoreFile = process.env.SKYCODE_CORE_FILE || "skycode-core.js"
const coreFile = path.join(distDir, skycodeCoreFile)

const childProcesses: ChildProcess[] = []

async function main(): Promise<void> {
	console.log("Starting Simple Skycode gRPC Server...")
	...
	if (!fs.existsSync(coreFile)) {
		...
		console.error("  SKYCODE_DIST_DIR - Override distribution directory")
		console.error("  SKYCODE_CORE_FILE - Override core file name")
		console.error("")
	...
	try {
		await SkycodeApiServerMock.startGlobalServer()
		console.log("Skycode API Server started in-process")
	} catch (error) {
		console.error("Failed to start Skycode API Server:", error)
		process.exit(1)
	}

	const extensionsDir = path.join(distDir, "vsce-extension")
	const userDataDir = mkdtempSync(path.join(os.tmpdir(), "vsce"))
	const skycodeTestWorkspace = mkdtempSync(path.join(os.tmpdir(), "skycode-test-workspace-"))
	...
	const hostbridge: ChildProcess = spawn("npx", ["tsx", path.join(__dirname, "test-hostbridge-server.ts")], {
		...
		env: {
			...process.env,
			TEST_HOSTBRIDGE_WORKSPACE_DIR: skycodeTestWorkspace,
	...
	const baseArgs = ["--enable-source-maps", path.join(distDir, "skycode-core.js")]

	const spawnArgs = USE_C8 ? ["c8", "--report-dir", covDir, "node", ...baseArgs] : ["node", ...baseArgs]

	console.log(`Starting Skycode Core Service... (useC8=${USE_C8})`)
	...
	const shutdown = async () => {
		...
		await SkycodeApiServerMock.stopGlobalServer()

		try {
			rmSync(userDataDir, { recursive: true, force: true })
			rmSync(skycodeTestWorkspace, { recursive: true, force: true })
	...
	console.log(`Skycode gRPC Server is running on 127.0.0.1:${PROTOBUS_PORT}`)
	console.log("Press Ctrl+C to stop")
}

if (require.main === module) {
	main().catch((err) => {
		console.error("Failed to start simple Skycode server:", err)
		process.exit(1)
	})
}
