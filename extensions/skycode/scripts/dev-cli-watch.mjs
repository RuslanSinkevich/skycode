...
console.log(`${colors.bright}${colors.cyan}🚀 Skycode CLI Dev Watch Mode (Fast Incremental)${colors.reset}`)
console.log(`${colors.dim}Starting initial build...${colors.reset}\n`)

// Function to kill all CLI instances
function killAllInstances() {
	try {
		execSync("./cli/bin/skycode instance kill --all", {
			cwd: projectRoot,
			stdio: "pipe",
		})
	} catch (error) {
...
function startNewInstance() {
	try {
		...
		const result = execSync("./cli/bin/skycode instance new", {
			cwd: projectRoot,
			stdio: "pipe",
			encoding: "utf-8",
		})
...
async function rebuildGo() {
	...
	try {
		...
		execSync("cd cli && GO111MODULE=on go build -o bin/skycode ./cmd/skycode", {
			cwd: projectRoot,
			stdio: "inherit",
			shell: true,
		})
		execSync("cd cli && GO111MODULE=on go build -o bin/skycode-host ./cmd/skycode-host", {
			cwd: projectRoot,
			stdio: "inherit",
			shell: true,
		})
