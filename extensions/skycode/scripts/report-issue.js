...
const getSkycodeVersion = () => {
	try {
		const extensions = execSync("code --list-extensions --show-versions").toString()
		const skycodeMatch = extensions.match(/skycode@(\d+\.\d+\.\d+)/)
		return skycodeMatch ? skycodeMatch[1] : "Not installed"
	} catch (_err) {
...
const collectSystemInfo = () => {
	...
	return {
		...
		skycodeVersion: getSkycodeVersion(),
	}
}
...
const createIssueUrl = (systemInfo, issueTitle) => {
	return (
		`https://github.com/skycode/skycode/issues/new?template=bug_report.yml` +
		`&title=${issueTitle}` +
		`&operating-system=${systemInfo.os}` +
		`&skycode-version=${systemInfo.skycodeVersion}` +
		`&system-info=${
