 ...
 * Returns array of:
 * { serviceName: string, serviceKey: string, protoPackage: "skycode"|"host", moduleBase: string }
 */
async function parseServicesWithFiles(protoDir, protoFiles) {
	...
	for (const relPath of protoFiles) {
		...
		const pkg = relPath.startsWith("host/") ? "host" : "skycode"
...
async function generateSkycodeClientPy(outDir, services) {
...
class SkycodeClient:
    """
    Unified Python client analogous to src/generated/grpc-go/client/SkycodeClient.

    Usage:
        client = SkycodeClient("localhost:17611")
        client.connect()
...
`
	...
	await fs.writeFile(path.join(clientDir, "skycode_client.py"), content)
}
...
async function generatePythonClient(protoDir, pyOutDir, clientDir, protoFiles) {
	...
	// skycode_client.py (unified that composes service wrappers)
	await generateSkycodeClientPy(clientDir, services)
}
...
[project]
name = "skycode-grpc-python"
version = "0.1.0"
description = "Generated Python gRPC stubs and client wrappers for Skycode protos"
