...
export async function goProtoc(outDir, protoFiles) {
	...
	const goProtocCommand = [
		...
		`--go_out="${outDir}"`,
		`--go_opt=module=github.com/skycode/grpc-go`,
		`--go-grpc_out="${outDir}"`,
		`--go-grpc_opt=module=github.com/skycode/grpc-go`,
		...protoFiles,
	].join(" ")
...
async function generateGoMod() {
	...
	const goModContent = `module github.com/skycode/grpc-go
...
import (
	...
	"google.golang.org/grpc"
	"github.com/skycode/grpc-go/client/services"
)

// SkycodeClient provides a unified interface to all Skycode services
type SkycodeClient struct {
...
// NewSkycodeClient creates a new unified Skycode client
func NewSkycodeClient(address string) (*SkycodeClient, error) {
	config := &ConnectionConfig{
		Address: address,
	}
	...
	return &SkycodeClient{
		connManager: connManager,
	}, nil
}

// NewSkycodeClientWithConfig creates a new Skycode client with custom configuration
func NewSkycodeClientWithConfig(config *ConnectionConfig) (*SkycodeClient, error) {
	connManager := NewConnectionManager(config)
...
// Connect establishes connection to Skycode Core and initializes service clients
func (c *SkycodeClient) Connect(ctx context.Context) error {
	c.mutex.Lock()
...
// Disconnect closes the connection to Skycode Core
func (c *SkycodeClient) Disconnect() error {
	c.mutex.Lock()
	defer c.mutex.Unlock()
...
// IsConnected returns true if the client is connected to Skycode Core
func (c *SkycodeClient) IsConnected() bool {
	c.mutex.RLock()
...
func (c *SkycodeClient) Reconnect(ctx context.Context) error {
	c.mutex.Lock()
	defer c.mutex.Unlock()
...
func (c *SkycodeClient) GetConnection() *grpc.ClientConn {
	return c.connManager.GetConnection()
}
`
	const clientPath = path.join(GO_CLIENT_DIR, "skycode_client.go")
...
func (sc *${capitalizedServiceName}Client) ${capitalizedMethodName}(ctx context.Context, req ${requestType}) (${responseType}, error) {
	...
	return resp, nil
		...
		const protoImportPath =
			serviceDef.protoPackage === "host" ? '"github.com/skycode/grpc-go/host"' : '"github.com/skycode/grpc-go/skycode"'

		// Check if we need to import skycode package for common types
		const needsSkycodeImport = serviceDef.methods.some((method) => {
			const requestTypeName = method.requestType.split(".").pop()
			const responseTypeName = method.responseType.split(".").pop()
		...
		// Always import skycode package if we need common types, regardless of service package
		const skycodeImport = needsSkycodeImport ? '	skycode "github.com/skycode/grpc-go/skycode"\n' : ""
...
${skycodeImport}	proto ${protoImportPath}
	"google.golang.org/grpc"
)
