 ...
 *    - Changes name to "skycode-nightly"
 *    - Changes displayName to "Skycode (Nightly)"
...
const config = {
	...
	nightlyName: "skycode-nightly",
	nightlyDisplayName: "Skycode (Nightly)",
	...
	get vsixPath() {
		return path.join(this.distDir, "skycode-nightly.vsix")
	},
}
...
class NightlyPublisher {
	 ...
	 */
	updatePackageJson() {
		// Replace any occurrences of skycode with nightly name
		const rawContent = fs.readFileSync(config.packageJsonPath, "utf-8")
		const content = rawContent.replaceAll('"skycode.', `"${config.nightlyName}.`)
