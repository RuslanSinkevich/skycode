import { BooleanRequest, EmptyRequest, StringArrayRequest } from "@shared/proto/skycode/common"
import { GetTaskHistoryRequest, TaskFavoriteRequest } from "@shared/proto/skycode/task"
...
const HISTORY_FILTERS = {
	...
	mostExpensive: "history.mostExpensive",
	mostTokens: "history.mostTokens",
	mostRelevant: "history.mostRelevant",
	workspaceOnly: "history.workspaceOnly",
...
const HistoryView = ({ onDone }: HistoryViewProps) => {
	...
	// Group tasks into localized date buckets (only for date-based sorts)
		...
		<div className="fixed overflow-hidden inset-0 flex flex-col w-full">
			...
			<div className="flex flex-col gap-3 px-3">
				...
				<div className="flex justify-between items-center">
					...
					<VSCodeTextField
						...
						placeholder={t("history.fuzzySearchPlaceholder")}
						value={searchQuery}>
			...
			<div className="p-2.5 border-t border-t-border-panel">
				...
				{selectedItems.length > 0 ? (
					<Button
						aria-label={t("history.deleteSelectedItems")}
						className="w-full"
						onClick={() => {
							handleDeleteSelectedHistoryItems(selectedItems)
						}}
						variant="danger">
						{t("history.delete")} {selectedItems.length > 1 ? selectedItems.length : ""} {t("history.selected")}
						{selectedItemsSize > 0 ? ` (${formatSize(selectedItemsSize)})` : ""}
					</Button>
				) : (
					<Button
						aria-label={t("history.deleteAllHistory")}
						className="w-full"
