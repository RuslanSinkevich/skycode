	...
	return (
		<span
			className={cn("text-link inline-flex items-center gap-1", {
				"hover:opacity-90 cursor-pointer": mode !== "act" && mode !== "debug",
				"cursor-not-allowed opacity-60": mode === "act" || mode === "debug",
			})}
			onClick={() => {
				// Toggle to Act mode if we're currently in a read-only mode
			...
			title={mode === "act" || mode === "debug" ? "Already in Act Mode" : "Click to toggle to Act Mode"}>
		...
		if (
			...
			if (tableLines.length >= 3) {
				...
				for (let c = 0; c < headerCells.length; c++) {
					...
					html += `<th style="text-align:${align}">${cellToHtml(headerCells[c].trim())}</th>`
				}
				html += "</tr></thead><tbody>"

				for (let r = 2; r < tableLines.length; r++) {
					const cells = parsePipeLine(tableLines[r])
					html += "<tr>"
					for (let c = 0; c < headerCells.length; c++) {
						const align = aligns[c] || "left"
						const content = cells[c]?.trim() || ""
						html += `<td style="text-align:${align}">${cellToHtml(content)}</td>`
					}
					html += "</tr>"
				}
				html += "</tbody></table>"
...
const MarkdownBlock = memo(({ markdown, compact, showCursor }: MarkdownBlockProps) => {
	...
	useEffect(() => {
		setMarkdown(convertMarkdownTablesToHtml(markdown || ""))
