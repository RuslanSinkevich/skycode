...
export const CheckmarkControl = ({ messageTs, isCheckpointCheckedOut }: CheckmarkControlProps) => {
	...
	const handleRestoreTask = async () => {
		...
		try {
			const restoreType: SkycodeCheckpointRestore = "task"
	...
	const handleRestoreWorkspace = async () => {
		...
		try {
			const restoreType: SkycodeCheckpointRestore = "workspace"
			await CheckpointsServiceClient.checkpointRestore(
				CheckpointRestoreRequest.create({
					number: messageTs,
					restoreType,
	...
	const handleRestoreBoth = async () => {
		...
		try {
			const restoreType: SkycodeCheckpointRestore = "taskAndWorkspace"
			await CheckpointsServiceClient.checkpointRestore(
				CheckpointRestoreRequest.create({
					number: messageTs,
					restoreType,
							...
							createPortal(
								<RestoreConfirmTooltip
									...
									<PrimaryRestoreOption>
										...
										<p>{t("checkpoint.restoreFilesAndTaskDesc")}</p>
									</PrimaryRestoreOption>
									...
									{showMoreOptions && (
										<AdditionalOptions>
											<RestoreOption>
												...
												<p>{t("checkpoint.restoreFilesOnlyDesc")}</p>
											</RestoreOption>
											<RestoreOption>
												<Button
												...
												<p>{t("checkpoint.restoreTaskOnlyDescShort")}</p>
											</RestoreOption>
										</AdditionalOptions>
									)}
								</RestoreConfirmTooltip>,
