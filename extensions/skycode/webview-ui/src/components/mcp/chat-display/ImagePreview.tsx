...
> {
	private t(key: string, params?: Record<string, string | number>) {
		const locale = localStorage.getItem("skycode.interfaceLanguage") === "en" ? "en" : "ru"
		const dict = locale === "en" ? (en as Record<string, string>) : (ru as Record<string, string>)
		let value = dict[key] ?? (ru as Record<string, string>)[key] ?? key
		if (params) {
	...
	handleImageError = () => {
		...
		this.setState({
			loading: false,
			error: this.t("mcp.failedToLoadImage"),
		})
		this.cleanup()
	}
			...
			return (
				<div
					...
					<div style={{ display: "flex", alignItems: "center", marginBottom: "8px" }}>
						...
						{this.t("mcp.loadingImageFrom", { host: getSafeHostname(url) })}
					</div>
					{elapsedSeconds > 3 && (
						<div style={{ fontSize: "11px", color: "var(--vscode-descriptionForeground)" }}>
							{elapsedSeconds > 60
								? this.t("mcp.waitingForMs", {
										minutes: Math.floor(elapsedSeconds / 60),
										seconds: elapsedSeconds % 60,
									})
								: this.t("mcp.waitingForS", { seconds: elapsedSeconds })}
						</div>
					)}
		...
		if (error) {
			return (
				<div
					...
					<div style={{ fontWeight: "bold" }}>{this.t("mcp.failedToLoadImage")}</div>
					...
					<div style={{ fontSize: "11px", marginTop: "8px", color: "var(--vscode-textLink-foreground)" }}>
						{this.t("mcp.clickToOpenInBrowser")}
					</div>
				</div>
			)
		}
