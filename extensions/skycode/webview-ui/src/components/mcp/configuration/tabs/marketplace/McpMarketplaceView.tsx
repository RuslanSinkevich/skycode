		...
		if (showMarketplace) {
			McpServiceClient.refreshMcpMarketplace(EmptyRequest.create({}))
				...
				.catch((error) => {
					...
					setError(t("mcp.failedToLoadMarketplaceData"))
					setIsLoading(false)
					setIsRefreshing(false)
				})
		}
		...
		<div
			...
			<div style={{ padding: "20px 20px 5px", display: "flex", flexDirection: "column", gap: "16px" }}>
				...
				<div
					...
					<div
						...
						<VSCodeDropdown
							...
							<VSCodeOption value="">{t("mcp.allCategories")}</VSCodeOption>
				...
				<div
					...
					<VSCodeRadioGroup
						...
						value={sortBy}>
						<VSCodeRadio value="downloadCount">{t("mcp.mostInstalls")}</VSCodeRadio>
						<VSCodeRadio value="newest">{t("history.newest")}</VSCodeRadio>
						<VSCodeRadio value="stars">{t("mcp.githubStars")}</VSCodeRadio>
						<VSCodeRadio value="name">{t("mcp.name")}</VSCodeRadio>
					</VSCodeRadioGroup>
				</div>
			</div>
			...
			{remoteConfigSettings?.allowedMCPServers && (
				<div className="flex items-center gap-2 px-5 py-3 mx-5 mb-4 bg-vscode-textBlockQuote-background border-l-[3px] border-vscode-textLink-foreground">
					...
					<span className="text-[13px]">{t("mcp.orgPreconfiguredAvailableServers")}</span>
				</div>
			)}
			...
			<div style={{ display: "flex", flexDirection: "column" }}>
				{filteredItems.length === 0 ? (
					<div
						...
						{searchQuery || selectedCategory ? t("mcp.noMatchingServers") : t("mcp.noServersInMarketplace")}
					</div>
				) : (
