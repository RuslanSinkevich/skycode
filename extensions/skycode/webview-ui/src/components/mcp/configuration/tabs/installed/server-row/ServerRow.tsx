...
import { McpServers, ToggleMcpServerRequest, ToggleToolAutoApproveRequest, UpdateMcpTimeoutRequest } from "@shared/proto/skycode/mcp"
			...
			<div
				...
				<Tooltip>
					...
					<TooltipContent className="max-w-xs" hidden={!isAlwaysEnabled} side="top">
						{t("mcp.serverCannotBeDisabledByOrg")}
					</TooltipContent>
				</Tooltip>
				<div
				...
				<div className="text-sm bg-text-block-background rounded-b-sm">
					...
					{server.oauthRequired && server.oauthAuthStatus === "unauthenticated" ? (
						...
						<Button
							...
							variant="secondary">
							{server.status === "connecting" || isRestarting ? t("mcp.retrying") : t("mcp.retryConnection")}
						</Button>
					)}
					...
					{!isRemoteManagedServer && (
						<Button
							...
							variant="danger">
							{isDeleting ? t("mcp.deleting") : t("mcp.deleteServer")}
						</Button>
					)}
				</div>
			) : (
				isExpanded && (
					<div className="bg-text-block-background p-2.5 pt-0 text-sm rounded-b-sm">
						<VSCodePanels>
							...
							<VSCodePanelView id="tools-view">
								{server.tools && server.tools.length > 0 ? (
									...
									<div className="text-description py-2.5">{t("mcp.noToolsFound")}</div>
								)}
							</VSCodePanelView>
							...
							<VSCodePanelView id="resources-view">
								...
								(server.resourceTemplates && server.resourceTemplates.length > 0) ? (
									...
									<div className="py-2.5 text-description">{t("mcp.noResourcesFound")}</div>
								)}
							</VSCodePanelView>
						</VSCodePanels>

						<div className="my-2.5 mx-1.5">
							<label className="block mb-1 text-[13px]">{t("mcp.requestTimeout")}</label>
						...
						<Button
							...
							onClick={handleRestart}
							variant="secondary">
							{server.status === "connecting" || isRestarting ? t("mcp.restarting") : t("mcp.restartServer")}
						</Button>
