import { CreateHookRequest, CreateSkillRequest, RuleFileRequest } from "@shared/proto/index.skycode"
		...
		if (filename.trim()) {
			...
			if (ruleType === "skill") {
				...
				if (!/^[a-zA-Z0-9_-]+$/.test(trimmedFilename)) {
					setError(t("skycodeRules.skillNameValidation"))
					return
				}
			...
			if (!isValidExtension(extension)) {
				setError(t("skycodeRules.allowedExtensions"))
				return
			}
				...
				{ruleType === "hook" ? (
					<>
						...
						<span className="sr-only" id="hook-select-description">
							Choose a hook type to create. Hooks execute at specific points in Skycode's lifecycle. Available:{" "}
							{availableHookTypes.map((h) => h.name).join(", ")}
						</span>
						<select
							aria-describedby="hook-select-description"
							aria-label={t("skycodeRules.selectHookTypeToCreate")}
							...
							<option disabled value="">
								{availableHookTypes.length === 0 ? t("skycodeRules.allHooksCreated") : t("skycodeRules.newHook")}
							</option>
					...
					<form className="flex flex-1 items-center" onSubmit={handleSubmit}>
						...
						<Button
							aria-label={
								isExpanded
									? ruleType === "skill"
										? t("skycodeRules.createSkill")
										: t("skycodeRules.createFile")
									: ruleType === "workflow"
										? t("skycodeRules.newWorkflowFile")
										: ruleType === "skill"
											? t("skycodeRules.newSkill")
