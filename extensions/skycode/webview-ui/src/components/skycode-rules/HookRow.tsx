...
import { DeleteHookRequest, HooksToggles } from "@shared/proto/skycode/file"
		...
		<div className="mb-2.5">
			<div className="flex items-center px-2 py-4 rounded bg-text-block-background max-h-4">
				...
				<div className="flex items-center space-x-2 gap-2">
					<div title={isWindows ? t("skycodeRules.hookTogglingNotSupportedWindows") : undefined}>
						<Switch
							checked={enabled}
							className="mx-1"
							disabled={isWindows}
					...
					<Button
						aria-label={t("skycodeRules.editHookFile")}
						onClick={handleEditClick}
						size="xs"
						title={t("skycodeRules.editHookFile")}
						variant="icon">
						<PenIcon />
					</Button>
					<Button
						aria-label={t("skycodeRules.deleteHookFile")}
						onClick={handleDeleteClick}
						size="xs"
						title={t("skycodeRules.deleteHookFile")}
						variant="icon">
						<Trash2Icon />
					</Button>
				</div>
