...
const SkycodeRulesToggleModal: React.FC = () => {
	const { t } = useI18n()
	const {
		globalSkycodeRulesToggles = {},
		localSkycodeRulesToggles = {},
		...
		hooksEnabled,
		skillsEnabled,
		setGlobalSkycodeRulesToggles,
		setLocalSkycodeRulesToggles,
		...
		if (isVisible) {
			FileServiceClient.refreshRules({} as EmptyRequest)
				.then((response: RefreshedRules) => {
					...
					if (response.globalSkycodeRulesToggles?.toggles) {
						setGlobalSkycodeRulesToggles(response.globalSkycodeRulesToggles.toggles)
					}
					if (response.localSkycodeRulesToggles?.toggles) {
						setLocalSkycodeRulesToggles(response.localSkycodeRulesToggles.toggles)
					}
	...
	const globalRules = Object.entries(globalSkycodeRulesToggles || {})
		.map(([path, enabled]): [string, boolean] => [path, enabled as boolean])
		.sort(([a], [b]) => a.localeCompare(b))

	// Format local rules for display with proper typing
	const localRules = Object.entries(localSkycodeRulesToggles || {})
		.map(([path, enabled]): [string, boolean] => [path, enabled as boolean])
		.sort(([a], [b]) => a.localeCompare(b))
	...
	const toggleRule = (isGlobal: boolean, rulePath: string, enabled: boolean) => {
		FileServiceClient.toggleSkycodeRule(
			ToggleSkycodeRuleRequest.create({
			...
			.catch((error) => {
				console.error("Error toggling Skycode rule:", error)
			})
	}
	...
	const toggleWindsurfRule = (rulePath: string, enabled: boolean) => {
		FileServiceClient.toggleWindsurfRule(
			...
			.then((response: SkycodeRulesToggles) => {
				if (response.toggles) {
	...
	return (
		<div className="inline-flex min-w-0 max-w-full items-center" ref={modalRef}>
			<div className="inline-flex w-full items-center" ref={buttonRef}>
				<Tooltip>
					{!isVisible && <TooltipContent>{t("skycodeRules.manageRulesWorkflows")}</TooltipContent>}
					<TooltipTrigger>
						<VSCodeButton
							appearance="icon"
							aria-label={isVisible ? t("skycodeRules.hideRulesWorkflows") : t("skycodeRules.showRulesWorkflows")}
				...
				<PopupModalContainer $arrowPosition={arrowPosition} $menuPosition={menuPosition}>
					...
					<div className="flex-shrink-0 px-2 pt-0">
						...
						{(currentView === "rules" && hasRemoteRules) || (currentView === "workflows" && hasRemoteWorkflows) ? (
							<div className="flex items-center gap-2 px-5 py-3 mb-4 bg-vscode-textBlockQuote-background border-l-[3px] border-vscode-textLink-foreground">
								...
								<span className="text-base">
									{currentView === "rules"
										? t("skycodeRules.orgManagesRules")
										: t("skycodeRules.orgManagesWorkflows")}
								</span>
							</div>
						) : null}
						...
						<div className="text-xs text-description mb-4">
							{currentView === "rules" ? (
								<p>
									{t("skycodeRules.rulesDescription")}{" "}
									<VSCodeLink className="text-xs" href="https://skycode-ai.ru/ru/docs/skycode-rules" style={{ display: "inline", fontSize: "inherit" }}>
										{t("skycodeRules.docs")}
									</VSCodeLink>
								</p>
							) : currentView === "workflows" ? (
								<p>
									{t("skycodeRules.workflowsDescription")}{" "}
								...
								<p>
									{t("skycodeRules.skillsDescriptionPrefix")} <span className="font-bold">use_skill</span>{" "}
									{t("skycodeRules.skillsDescriptionSuffix")}
								</p>
							) : (
								<p>{t("skycodeRules.hooksDescription")}</p>
							)}
						</div>
					</div>
					...
					<div className="flex-1 overflow-y-auto px-2 pb-3" style={{ minHeight: 0 }}>
						{currentView === "rules" ? (
							<>
								...
								{hasRemoteRules && (
									<div className="mb-3">
										<div className="text-sm font-normal mb-2">{t("skycodeRules.enterpriseRules")}</div>
								...
								<div className="mb-3">
									<div className="text-sm font-normal mb-2">{t("skycodeRules.globalRules")}</div>
								...
								<div className="-mb-2.5">
									<div className="text-sm font-normal mb-2">{t("skycodeRules.workspaceRules")}</div>
									<RulesToggleList
										isGlobal={false}
										listGap="small"
										rules={localRules}
							...
							<>
								...
								{hasRemoteWorkflows && (
									<div className="mb-3">
										<div className="text-sm font-normal mb-2">{t("skycodeRules.enterpriseWorkflows")}</div>
										<div className="flex flex-col gap-0">
							...
							<>
								<div className="text-xs text-description mb-4">
									<p>
										{t("skycodeRules.hooksToggleHint")}{" "}
										<VSCodeLink
											className="text-xs"
											href="https://skycode-ai.ru/ru/docs/skycode-rules"
								...
								{isWindows && (
									<div className="flex items-center gap-2 px-5 py-3 mb-4 bg-vscode-inputValidation-warningBackground border-l-[3px] border-vscode-inputValidation-warningBorder">
										...
										<span className="text-base">{t("skycodeRules.hookTogglingWindowsBanner")}</span>
									</div>
								)}

								{/* Global Hooks */}
								<div className="mb-3">
									<div className="text-sm font-normal mb-2">{t("skycodeRules.globalHooks")}</div>
									<div className="flex flex-col gap-0">
										{globalHooks
								...
								{workspaceHooks.map((workspace, index) => (
									<div
										...
										<div className="text-sm font-normal mb-2">
											{workspace.workspaceName}/.skycoderules/hooks/
										</div>
										<div className="flex flex-col gap-0">
											{workspace.hooks
												.sort((a, b) => a.name.localeCompare(b.name))
							...
							<>
								...
								<div className="mb-3">
									<div className="text-sm font-normal mb-2">{t("skycodeRules.globalSkills")}</div>
									<div className="flex flex-col gap-0">
										{globalSkills
											.sort((a, b) => a.name.localeCompare(b.name))
											.map((skill) => (
								...
								<div className="-mb-2.5">
									<div className="text-sm font-normal mb-2">{t("skycodeRules.workspaceSkills")}</div>
									<div className="flex flex-col gap-0">
										{localSkills
											.sort((a, b) => a.name.localeCompare(b.name))
											.map((skill) => (
...
export default SkycodeRulesToggleModal
