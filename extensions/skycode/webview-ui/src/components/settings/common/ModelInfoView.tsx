...
interface ModelInfoViewProps {
	...
	modelInfo: ModelInfo
	isPopup?: boolean
	// Provider routing props (optional - only shown for Skycode provider)
		...
		<div style={{ marginTop: 4 }}>
			...
			<CollapsibleContent $isExpanded={advancedExpanded}>
				<AdvancedSection>
					{/* Capabilities */}
					<AdvancedRow>
						<AdvancedLabel>{t("provider.images")}</AdvancedLabel>
						<AdvancedValue>{hasImages ? t("provider.yes") : t("provider.no")}</AdvancedValue>
					</AdvancedRow>
					<AdvancedRow>
						<AdvancedLabel>{t("provider.browser")}</AdvancedLabel>
						<AdvancedValue>{hasBrowser ? t("provider.yes") : t("provider.no")}</AdvancedValue>
					</AdvancedRow>
					{!isGemini && (
						<AdvancedRow>
							<AdvancedLabel>{t("provider.promptCaching")}</AdvancedLabel>
							<AdvancedValue>{hasCaching ? t("provider.yes") : t("provider.no")}</AdvancedValue>
						</AdvancedRow>
					)}
					...
					{hasTiers && (
						<div style={{ marginTop: 8 }}>
							<div style={{ fontWeight: 500, marginBottom: 4 }}>{t("provider.tieredPricing")}:</div>
							{modelInfo.tiers && (
								<>
									<div>
										<span style={{ fontWeight: 500 }}>{t("provider.input")}:</span>
										<br />
										{formatTiers(modelInfo.tiers, "inputPrice")}
									</div>
									<div style={{ marginTop: 4 }}>
										<span style={{ fontWeight: 500 }}>{t("provider.output")}:</span>
										<br />
					...
					{showProviderRouting && onProviderSortingChange && (
						<ProviderRoutingContainer>
							<ProviderRoutingLabel>{t("provider.providerRouting")}</ProviderRoutingLabel>
							<VSCodeDropdown
								onChange={(e: any) => onProviderSortingChange(e.target.value)}
								style={{ width: "100%" }}
								value={providerSorting || ""}>
								<VSCodeOption value="">{t("provider.default")}</VSCodeOption>
								<VSCodeOption value="price">{t("provider.price")}</VSCodeOption>
								<VSCodeOption value="throughput">{t("provider.throughput")}</VSCodeOption>
								<VSCodeOption value="latency">{t("provider.latency")}</VSCodeOption>
							</VSCodeDropdown>
							<p
								style={{
									fontSize: "11px",
								...
								{!providerSorting && t("provider.routingDefaultDescription")}
								{providerSorting === "price" && t("provider.routingPriceDescription")}
								{providerSorting === "throughput" && t("provider.routingThroughputDescription")}
								{providerSorting === "latency" && t("provider.routingLatencyDescription")}
							</p>
						</ProviderRoutingContainer>
					)}
				</AdvancedSection>
