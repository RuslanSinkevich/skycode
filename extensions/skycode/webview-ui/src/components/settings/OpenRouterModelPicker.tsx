...
// Featured models for Skycode provider organized by tabs
...
const FREE_SKYCODE_MODELS = freeModels.map((m) => m.id)
...
const OpenRouterModelPicker: React.FC<OpenRouterModelPickerProps> = ({ isPopup, currentMode, showProviderRouting }) => {
	...
	const { selectedModelId, selectedModelInfo } = useMemo(() => {
		...
		const isSkycode = selected.selectedProvider === "skycode"
		// Makes sure "Free" featured models have $0 pricing for Skycode provider
		if (isSkycode && FREE_SKYCODE_MODELS.includes(selected.selectedModelId)) {
			return {
				...selected,
				selectedModelInfo: {
		...
		<div style={{ width: "100%", paddingBottom: 2 }}>
			...
			<div style={{ display: "flex", flexDirection: "column" }}>
				<label htmlFor="model-search">
					<span style={{ fontWeight: 500 }}>{t("provider.model")}</span>
				</label>

				{modeFields.apiProvider === "skycode" && (
					<>
						{/* Tabs */}
				...
				<DropdownWrapper ref={dropdownRef}>
					<VSCodeTextField
						...
						placeholder={t("provider.searchSelectModel")}
						style={{
							width: "100%",
								...
								return (
									<DropdownItem
										...
										<div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
											{/* biome-ignore lint/security/noDangerouslySetInnerHtml: API returns pre-highlighted model names */}
			...
			{hasInfo ? (
					...
					{showThinkingLevel && (
						<DropdownContainer className="dropdown-container" zIndex={1}>
							<label htmlFor="thinking-level">
								<span className="font-medium">{t("settings.thinkingLevel")}</span>
							</label>
							<VSCodeDropdown
								className="w-full"
								id="thinking-level"
								...
								<VSCodeOption value="low">{t("settings.low")}</VSCodeOption>
								<VSCodeOption value="high">{t("settings.high")}</VSCodeOption>
							</VSCodeDropdown>
						</DropdownContainer>
					)}
				...
				<p
					...
					If you're unsure which model to choose, Skycode works best with{" "}
					<VSCodeLink
