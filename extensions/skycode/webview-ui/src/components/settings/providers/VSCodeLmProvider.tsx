...
export const VSCodeLmProvider = ({ currentMode }: VSCodeLmProviderProps) => {
	...
	const { handleFieldChange: _handleFieldChange, handleModeFieldChange } = useApiConfigurationHandlers()
		...
		<div>
			<DropdownContainer className="dropdown-container" zIndex={DROPDOWN_Z_INDEX - 2}>
				<label htmlFor="vscode-lm-model">
					<span style={{ fontWeight: 500 }}>{t("provider.languageModel")}</span>
				</label>
				{vsCodeLmModels.length > 0 ? (
					<VSCodeDropdown
						...
						<VSCodeOption value="">{t("provider.selectModel")}</VSCodeOption>
					...
					<p
						...
						{t("provider.useModelsFromCopilot")}{" "}
						<a href="https://marketplace.visualstudio.com/items?itemName=GitHub.copilot">
							{t("provider.copilotExtension")}
						</a>{" "}
						{t("account.and")} {t("provider.enableClaudeInCopilot")}
					</p>
				)}
			</DropdownContainer>
		</div>
