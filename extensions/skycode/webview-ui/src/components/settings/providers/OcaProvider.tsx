...
import type { OcaAuthState, OcaUserInfo } from "@shared/proto/index.skycode"
import { EmptyRequest, StringRequest } from "@shared/proto/index.skycode"
...
function InfoCard({ icon, children }: { icon: React.ReactNode; children: React.ReactNode }) {
	return (
		<div className="mt-2 mb-2 flex items-start gap-3 rounded-none px-5 py-4 pb-8 border shadow-sm min-w-[40%] max-w-[90%] w-full box-border bg-input-background border-input-border">
		...
		<div>
			{!ready ? (
				<div aria-live="polite" className="flex items-center gap-2 py-2" role="status">
					<VSCodeProgressRing />
					<span className={`text-[13px] [color:var(${VSC_DESCRIPTION_FOREGROUND})]`}>{t("mcp.connecting")}</span>
				</div>
			) : !isAuthenticated ? (
				<div>
					<div
						aria-label={t("provider.oracleEmployment")}
						style={{
							marginTop: 12,
							marginBottom: 4,
						}}>
					...
					<VSCodeButton
						...
						{t("provider.signInWithOracleCodeAssist")}
					</VSCodeButton>
					<p className="text-xs mt-0 text-(--vscode-descriptionForeground)">
						{t("provider.ocaSetupDescriptionPrefix")}{" "}
						<VSCodeLink
							href="https://confluence.oraclecorp.com/confluence/display/AICODE/Oracle+Code+Assist+via+Skycode"
				...
				<div>
					<div className={`flex items-center justify-between mt-0 mb-0 [color:var(${VSC_DESCRIPTION_FOREGROUND})]`}>
						<div className="flex flex-col gap-0 font-semibold text-[13px]">
							...
							{ocaUser?.email ? (
								...
								<span className="font-semibold opacity-95 mt-2">{t("provider.unknownUser")}</span>
							)}
						</div>
						<VSCodeButton
							onClick={async () => {
					...
					<div className="mt-0">
						<BaseUrlField
							...
							label={t("provider.customBaseUrlOptional")}
					...
					{isAuthenticated && ocaHasError && (
						<div
							...
							role="status">
							<div>{t("provider.failedRefreshModels")}</div>
					...
					<InfoCard
						...
						<div
							...
							<div
								...
								{t("provider.ideaForOracleCodeAssist")}
							</div>
						</div>
						<div
							style={{
