...
import { YandexGPTProvider } from "./providers/YandexGPTProvider"
...
import { QwenWebProvider } from "./providers/QwenWebProvider"
...
import { SkycodeProvider } from "./providers/SkycodeProvider"
	...
	*/
	...
	return (
		<div style={{ display: "flex", flexDirection: "column", gap: 5, marginBottom: isPopup ? -10 : 0 }}>
			...
			<DropdownContainer className="dropdown-container">
						...
						<TooltipTrigger>
							<div className="flex items-center gap-2 mb-1">
								<label htmlFor="api-provider">
									<span style={{ fontWeight: 500 }}>{t("apiOptions.provider")}</span>
								</label>
								<i className="codicon codicon-lock text-description text-sm" />
							</div>
						</TooltipTrigger>
						<TooltipContent>{t("apiOptions.providerLockedByOrg")}</TooltipContent>
					</Tooltip>
				) : (
					<label htmlFor="api-provider">
						<span style={{ fontWeight: 500 }}>{t("apiOptions.provider")}</span>
				...
				<ProviderDropdownWrapper ref={dropdownRef}>
					<VSCodeTextField
						...
						placeholder={t("apiOptions.searchProvider")}
						style={{
							width: "100%",
			...
			{apiConfiguration && selectedProvider === "skycode" && (
				<SkycodeProvider currentMode={currentMode} isPopup={isPopup} showModelOptions={showModelOptions} />
			)}
			...
			{apiConfiguration && selectedProvider === "qwen-web" && (
				<QwenWebProvider currentMode={currentMode} isPopup={isPopup} showModelOptions={showModelOptions} />
			)}
			...
			{apiConfiguration && selectedProvider === "gigachat" && (
				<GigaChatProvider currentMode={currentMode} isPopup={isPopup} showModelOptions={showModelOptions} />
			)}

			{apiConfiguration && selectedProvider === "yandexgpt" && (
				<YandexGPTProvider currentMode={currentMode} isPopup={isPopup} showModelOptions={showModelOptions} />
			)}
			...
			{/* Lightweight mode toggle for weak models */}
			<div className="mt-2.5">
				<VSCodeCheckbox
					checked={!!lightweightMode}
					onChange={(e: any) => {
						updateSetting("lightweightMode", e.target.checked === true)
					}}>
