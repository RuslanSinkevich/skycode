import {
	...
	deepSeekModels,
	gigaChatDefaultModelId,
	gigaChatModels,
	yandexGptDefaultModelId,
	yandexGptModels,
 ...
 * For providers with dynamic models (openrouter, skycode, ollama, etc.), returns undefined.
...
export function normalizeApiConfiguration(
	...
	const provider =
		(currentMode === "plan" ? apiConfiguration?.planModeApiProvider : apiConfiguration?.actModeApiProvider) || "openai-native"
	...
	switch (provider) {
		...
		case "gigachat":
			return getProviderData(gigaChatModels, gigaChatDefaultModelId)
		case "yandexgpt":
			return getProviderData(yandexGptModels, yandexGptDefaultModelId)
		case "qwen":
 ...
 * For Skycode provider: excludes :free models (except Minimax models)
 * For OpenRouter/Vercel: excludes skycode/ prefixed models
...
export function filterOpenRouterModelIds(modelIds: string[], provider: ApiProvider): string[] {
	if (provider === "skycode") {
		// For Skycode provider: exclude :free models, but keep Minimax models
	...
	// For OpenRouter and Vercel AI Gateway providers: exclude Skycode-specific models
	return modelIds.filter((id) => !id.startsWith("skycode/"))
}
