	...
	return (
		<StatusContainer>
			{isChecking ? (
				<>
					<Spinner />
					<StatusText>{t("browser.checkingConnection")}</StatusText>
				</>
			) : isConnected === true ? (
				<>
				...
				<StatusText style={{ color: "var(--vscode-errorForeground)" }}>{t("browser.notConnected")}</StatusText>
			) : null}
		</StatusContainer>
	)
}
...
export const BrowserSettingsSection: React.FC<BrowserSettingsSectionProps> = ({ renderSectionHeader }) => {
	const { t } = useI18n()
	const { browserSettings, skycodeWebToolsEnabled } = useExtensionState()
			...
			<Section>
				<div id="browser-settings-section" style={{ marginBottom: 20 }}>
					...
					<div style={{ marginBottom: 10 }}>
						<VSCodeCheckbox
							checked={skycodeWebToolsEnabled?.user}
							onChange={(e: any) => {
								const checked = e.target.checked === true
								updateSetting("skycodeWebToolsEnabled", checked)
							}}>
							{t("features.skycodeWebTools")}
						</VSCodeCheckbox>
						<p
							style={{
								fontSize: "12px",
							...
							{t("features.skycodeWebToolsDescription")}
						</p>
						<p
							style={{
								fontSize: "12px",
							...
							{t("features.skycodeWebToolsWarning")}
						</p>
					</div>
					...
					<div style={{ marginBottom: isSubSettingsOpen ? 0 : 10 }}>
						...
						<p
							...
							{t("browser.disableToolUseDescription")}
						</p>
					</div>
					...
					<CollapsibleContent isOpen={isSubSettingsOpen}>
						...
						<div style={{ marginBottom: 0 }}>
							...
							<p
								...
								{t("browser.remoteDescriptionPrefix")}
								{isBundled
									? t("browser.chromeNotDetected")
									: detectedChromePath
										? ` (${detectedChromePath})`
										: ""}
								{t("browser.remoteDescriptionSuffix")}
								{browserSettings.remoteBrowserEnabled ? (
									<>
										{" "}
										{t("browser.remoteManualOrButton")} <code>--remote-debugging-port=9222</code>{" "}
										{t("browser.remoteHostHint")}
									</>
								) : (
									"."
								)}
							...
							{browserSettings.remoteBrowserEnabled && (
								<div style={{ marginLeft: 0, marginTop: 8 }}>
									<DebouncedTextField
										...
										placeholder={t("browser.remoteHostPlaceholder")}
									...
									{shouldShowRelaunchButton && (
										<div style={{ display: "flex", gap: "10px", marginBottom: 8, justifyContent: "center" }}>
											<VSCodeButton
												...
												style={{ flex: 1 }}>
												{debugMode ? t("browser.launching") : t("browser.launchDebug")}
											</VSCodeButton>
										</div>
									)}
							...
							<div style={{ marginBottom: 8, marginTop: 8 }}>
								...
								<DebouncedTextField
									...
									placeholder={t("browser.chromePathPlaceholder")}
									style={{ width: "100%" }}
								/>
								<p
									style={{
									...
									{t("browser.leaveEmptyAutoDetect")}
								</p>
							</div>
							...
							<div style={{ marginBottom: 8, marginTop: 8 }}>
								...
								<DebouncedTextField
									...
									placeholder={t("browser.customArgsPlaceholder")}
									style={{ width: "100%" }}
								/>
								<p
									style={{
									...
									{t("browser.customArgsDescription")}
								</p>
							</div>
						</div>
					</CollapsibleContent>
