...
const getActivityText = (tool: SkycodeSayTool, t: (key: string) => string): string | null => {
...
const collectToolsInRange = (
	...
	endIdx: number,
	t: (key: string) => string,
	stopCondition?: (msg: SkycodeMessage) => boolean,
	...
	for (let i = startIdx; i < endIdx; i++) {
		...
		try {
			const tool = JSON.parse(msg.text || "{}") as SkycodeSayTool
...
const findCurrentApiReq = (messages: SkycodeMessage[]): { index: number; hasCost: boolean } | null => {
...
const findPrevCompletedApiReq = (messages: SkycodeMessage[], beforeIdx: number): number => {
...
export const RequestStartRow: React.FC<RequestStartRowProps> = ({
	...
	const currentActivities = useMemo(() => {
		const currentApiReq = findCurrentApiReq(skycodeMessages)
		if (!currentApiReq) {
			return []
		}
		...
		if (!currentApiReq.hasCost) {
			...
			const prevIdx = findPrevCompletedApiReq(skycodeMessages, currentApiReq.index)
			if (prevIdx === -1) {
				return []
			}
			return collectToolsInRange(skycodeMessages, prevIdx + 1, currentApiReq.index, t)
		}
