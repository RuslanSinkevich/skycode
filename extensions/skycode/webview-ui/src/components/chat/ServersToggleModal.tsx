...
import { McpServers } from "@shared/proto/skycode/mcp"
...
const ServersToggleModal: React.FC = () => {
	const { t } = useI18n()
	const { mcpServers, navigateToSettings, setMcpServers } = useExtensionState()
	...
	return (
		<div className="inline-flex min-w-0 max-w-full items-center" ref={modalRef}>
			<div className="inline-flex w-full items-center" ref={buttonRef}>
				<Tooltip>
					{!isVisible && <TooltipContent>{t("chat.manageMcpServers")}</TooltipContent>}
					<TooltipTrigger>
						<VSCodeButton
							appearance="icon"
							aria-label={isVisible ? t("chat.hideMcpServers") : t("chat.showMcpServers")}
				...
				<PopupModalContainer $arrowPosition={arrowPosition} $menuPosition={menuPosition}>
					<div className="flex-shrink-0 px-3 pt-2">
						<div className="flex justify-between items-center mb-2.5">
							<div className="m-0 text-sm font-medium">{t("chat.mcpServers")}</div>
							<VSCodeButton
								appearance="icon"
								aria-label={t("chat.goToMcpServerSettings")}
							onClick={() => {
								setIsVisible(false)
