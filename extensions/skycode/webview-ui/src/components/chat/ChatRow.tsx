...
import {
	...
	SkycodeApiReqInfo,
	SkycodeAskQuestion,
	SkycodeAskUseMcpServer,
	SkycodeMessage,
	SkycodePlanModeResponse,
	...
	({
		...
		const [cost, apiReqCancelReason, apiReqStreamingFailedMessage] = useMemo(() => {
			if (message.text != null && message.say === "api_req_started") {
				const info: SkycodeApiReqInfo = JSON.parse(message.text)
		...
		const [icon, title] = useMemo(() => {
			switch (type) {
				...
				case "use_mcp_server":
					const mcpServerUse = JSON.parse(message.text || "{}") as SkycodeAskUseMcpServer
					return [
		...
		if (message.ask === "use_mcp_server" || message.say === "use_mcp_server") {
			const useMcpServer = JSON.parse(message.text || "{}") as SkycodeAskUseMcpServer
			...
			case "say":
				switch (message.say) {
					case "api_req_started": {
						...
						return (
							<RequestStartRow
								...
								skycodeMessages={skycodeMessages}
							/>
						)
					}
					...
					case "skycodeignore_error":
						return <ErrorRow errorType="skycodeignore_error" message={message} />
					...
					case "generate_explanation": {
						let explanationInfo: SkycodeSayGenerateExplanation = {
							title: "code changes",
							fromRef: "",
							toRef: "",
							status: "generating",
					...
					case "completion_result": {
						// Render as plain text — no green "Task Completed" wrapper
						...
						return (
							<WithCopyButton
								...
								<div className="flex items-center">
									<div className={cn("flex-1 min-w-0 pl-1")} ref={markdownRenderedRef}>
										<MarkdownRow markdown={completionText || ""} showCursor={false} />
									</div>
								</div>
								{quoteButtonState.visible && (
									<QuoteButton
					...
					case "shell_integration_warning":
						return (
							<div className="flex flex-col bg-warning/20 p-2 rounded-xs border border-error">
								...
								<a className="px-1" href="https://skycode-ai.ru/ru/docs/terminal-troubleshooting">
			...
			case "ask":
				switch (message.ask) {
					...
					case "completion_result":
						if (message.text) {
							...
							return (
								<WithCopyButton
									markdownDualCopy={{
										markdownSource: askCompletionText || "",
										renderedRef: markdownRenderedRef,
									}}
									onMouseUp={handleMouseUp}
									position="bottom-right"
									...
									<div className="flex items-center">
										<div className={cn("flex-1 min-w-0 pl-1")} ref={markdownRenderedRef}>
											<MarkdownRow markdown={askCompletionText || ""} showCursor={false} />
										</div>
									</div>
									{quoteButtonState.visible && (
										<QuoteButton
					...
					case "followup":
						...
						try {
							const parsedMessage = JSON.parse(message.text || "{}") as SkycodeAskQuestion
					...
					case "plan_mode_respond": {
						...
						let selected: string | undefined
						try {
							const parsedMessage = JSON.parse(message.text || "{}") as SkycodePlanModeResponse
