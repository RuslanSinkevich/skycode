...
import type {
	...
	SessionStateProto,
} from "@shared/proto/skycode/session"
import { SendMessageRequest, ApprovalResponse } from "@shared/proto/skycode/session"
...
export interface UseSessionReturn {
	...
	sessionMessages: SkycodeMessage[]
	/** Прогресс конвейера */
...
export function useSession(sessionId: string | null): UseSessionReturn {
	...
	const [sessionMessages, setSessionMessages] = useState<SkycodeMessage[]>([])
			...
			StringRequest.create({ value: sessionId }),
			{
				onResponse: (event) => {
					...
					if (event.messageAdded?.message) {
						const msg = convertProtoToSkycodeMessage(event.messageAdded.message)
						if (msg) {
					...
					if (event.messageUpdated) {
						...
						if (ts && patch) {
							setSessionMessages((prev) =>
								prev.map((m) => {
									if (m.ts === ts) {
										return { ...m, ...convertProtoToSkycodeMessage(patch) }
									}
									return m
								}),
							)
