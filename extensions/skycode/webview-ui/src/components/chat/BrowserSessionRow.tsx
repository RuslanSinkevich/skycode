...
import { BrowserAction, BrowserActionResult, SkycodeMessage, SkycodeSayBrowserAction } from "@shared/ExtensionMessage"
	...
	const pages = useMemo(() => {
		const result: {
			currentState: {
				...
				consoleLogs?: string
				messages: SkycodeMessage[] // messages up to and including the result
			}
			nextAction?: {
				messages: SkycodeMessage[] // messages leading to next result
			}
		}[] = []

		let currentStateMessages: SkycodeMessage[] = []
		let nextActionMessages: SkycodeMessage[] = []
		...
		<BrowserSessionRowContainer style={{ marginBottom: -10 }}>
			<div style={browserSessionRowContainerInnerStyle}>
				...
				<span style={approveTextStyle}>
					{isAutoApproved ? t("chat.skycodeUsingBrowser") : t("chat.skycodeWantsToUseBrowser")}
				</span>
			</div>
			<div
				style={{
				...
				<div style={consoleLogsContainerStyle}>
					<div
						...
						<span style={consoleLogsTextStyle}>{t("chat.consoleLogs")}</span>
					</div>
					{consoleLogsExpanded && (
						<CodeBlock source={`${"```"}shell\n${displayState.consoleLogs || t("chat.noNewLogs")}\n${"```"}`} />
					)}
				</div>
			</div>
			...
			{pages.length > 1 && (
				<div style={paginationContainerStyle}>
					<div>
						{t("chat.step")} {currentPageIndex + 1} {t("chat.of")} {pages.length}
					</div>
			...
			return (
				<>
					<div style={headerStyle}>
						<span style={browserSessionStartedTextStyle}>{t("chat.browserSessionStarted")}</span>
					</div>
...
const BrowserActionBox = ({ action, coordinate, text }: { action: BrowserAction; coordinate?: string; text?: string }) => {
	...
	const getBrowserActionText = (action: BrowserAction, coordinate?: string, text?: string) => {
		switch (action) {
			case "launch":
				return `${t("chat.launchBrowserAt")} ${text}`
			case "click":
				return `${t("chat.click")} (${coordinate?.replace(",", ", ")})`
			case "type":
				return `${t("chat.type")} "${text}"`
			case "scroll_down":
				return t("chat.scrollDown")
			case "scroll_up":
				return t("chat.scrollUp")
			case "close":
				return t("chat.closeBrowser")
			default:
				return action
		}
	}
			...
			<div style={browserActionBoxContainerInnerStyle}>
				<div style={browseActionRowContainerStyle}>
					<span style={browseActionRowStyle}>
						<span style={browseActionTextStyle}>{t("chat.browseAction")}: </span>
