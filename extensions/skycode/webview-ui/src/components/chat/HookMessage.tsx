 ...
 */
function shouldExpandHookByDefault(message: SkycodeMessage, metadata: HookMetadata): boolean {
...
const HookMessage = memo(({ message, CommandOutput }: HookMessageProps) => {
	...
	const { metadata, output } = useMemo(() => {
		...
		try {
			...
			hookMetadata = { hookName: t("chat.unknown"), status: "unknown" }
		}
	...
	return (
		<>
			<div style={headerStyle}>
				...
				<span style={{ color: normalColor, fontWeight: "bold" }}>{t("chat.hook")}:</span>
