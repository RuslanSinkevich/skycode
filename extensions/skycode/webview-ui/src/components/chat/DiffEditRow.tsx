...
// Max lines to show before collapsing middle section
const MAX_VISIBLE_LINES = 20
const CONTEXT_LINES = 5 // Lines to show at start and end when collapsed
				...
				<button
					...
					<div className="flex items-center gap-3 flex-1 w-full overflow-hidden">
						<div className={cn("flex items-center gap-2 w-full min-w-0", actionStyle.borderClass)}>
							{isToolStreaming && (
								<span
									aria-label={t("chat.streamingFilePreview")}
									className="inline-flex shrink-0"
									role="status"
									title={t("chat.streamingFilePreview")}>
									<LoaderCircleIcon aria-hidden className="size-4 animate-spin text-description" />
								</span>
							)}
					...
					<div
						...
						<div className="font-mono text-xs w-max min-w-full">
							{(() => {
								...
								const shouldCollapse = !showAllLines && !isStreaming && totalLines > MAX_VISIBLE_LINES
								const hiddenCount = shouldCollapse ? totalLines - CONTEXT_LINES * 2 : 0

								if (shouldCollapse) {
									return (
										<>
											{/* First N lines */}
											{file.lines.slice(0, CONTEXT_LINES).map((line, index) => (
												<DiffLine
											...
											<div
												className="flex items-center justify-center py-1 bg-description/10 text-description text-xs cursor-pointer hover:bg-description/20"
												onClick={() => setShowAllLines(true)}
												title={t("chat.clickToShowAllLines")}>
												<span>... {hiddenCount} hidden lines ...</span>
											</div>
											{/* Last N lines */}
											{file.lines.slice(-CONTEXT_LINES).map((line, index) => {
												const actualIndex = totalLines - CONTEXT_LINES + index
												return (
													<DiffLine
														key={`${actualIndex}-${line.slice(0, 20)}`}
														line={line}
														lineNumber={lineNumbers?.[actualIndex]}
													/>
												)
											})}
										</>
