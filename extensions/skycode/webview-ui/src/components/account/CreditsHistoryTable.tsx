import type { PaymentTransaction, UsageTransaction } from "@shared/SkycodeAccount"
			...
			<div className="mt-[15px] mb-[0px] rounded-md overflow-auto grow">
				{isLoading ? (
					<div className="flex justify-center items-center p-4">
						<div className="text-(--vscode-descriptionForeground)">{t("provider.loading")}</div>
					</div>
				) : (
					<>
						{activeTab === "usage" &&
								...
								<VSCodeDataGrid>
									...
									{usageData.map((row, index) => (
										...
										<VSCodeDataGridRow key={index}>
											...
											<VSCodeDataGridCell grid-column="3">{`${t("common.currencyPrefix")}${Number(row.creditsUsed / 1000000).toFixed(4)}${t("common.currencySuffix")}`}</VSCodeDataGridCell>
										</VSCodeDataGridRow>
									))}
								</VSCodeDataGrid>
							) : (
								...
								<VSCodeDataGrid>
									...
									{paymentsData.map((row, index) => (
										...
										<VSCodeDataGridRow key={index}>
											...
											<VSCodeDataGridCell grid-column="2">{`${t("common.currencyPrefix")}${formatDollars(row.amountCents)}${t("common.currencySuffix")}`}</VSCodeDataGridCell>
