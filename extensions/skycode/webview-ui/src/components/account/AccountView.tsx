import { isSkycodeInternalTester } from "@shared/internal/account"
import type { UserOrganization } from "@shared/proto/skycode/account"
import { VSCodeButton, VSCodeDropdown, VSCodeOption } from "@vscode/webview-ui-toolkit/react"
import { memo, useMemo } from "react"
import { useExtensionState } from "@/context/ExtensionStateContext"
import { handleSignOut, type SkycodeUser } from "@/context/SkycodeAuthContext"
...
type SkycodeAccountViewProps = {
	skycodeUser: SkycodeUser
	skycodeEnv: "Production" | "Staging" | "Local"
}

const SkycodeEnvOptions = ["Production", "Staging", "Local"] as const

const AccountView = ({ onDone, skycodeUser, organizations, activeOrganization }: AccountViewProps) => {
	const { t } = useI18n()
			...
			<div className="grow overflow-hidden pr-[8px] flex flex-col">
				<div className="h-full mb-1.5">
					{skycodeUser?.uid ? (
						<SkycodeAccountView
							key={skycodeUser.uid}
							skycodeEnv={environment === "local" ? "Local" : environment === "staging" ? "Staging" : "Production"}
							skycodeUser={skycodeUser}
						/>
					) : (
						<AccountWelcomeView />
...
export const SkycodeAccountView = ({ skycodeUser, skycodeEnv }: SkycodeAccountViewProps) => {
	const { t } = useI18n()
	const { email, displayName } = skycodeUser

	const isSkycodeTester = useMemo(() => (email ? isSkycodeInternalTester(email) : false), [email])
			...
			<div className="flex flex-col pr-3 h-full">
				...
				{isSkycodeTester && (
					<div className="w-full gap-1 items-end mt-6">
						<div className="text-sm font-semibold">{t("account.skycodeEnvironment")}</div>
						<VSCodeDropdown
							className="w-full mt-1"
							currentValue={skycodeEnv}
							onChange={async (e) => {
								const target = e.target as HTMLSelectElement
								if (target?.value) {
									const value = target.value as "Local" | "Staging" | "Production"
									updateSetting("skycodeEnv", value.toLowerCase())
								}
							}}>
							{SkycodeEnvOptions.map((env) => (
