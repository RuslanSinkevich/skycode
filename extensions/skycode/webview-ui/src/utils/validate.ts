	...
	if (apiConfiguration) {
		...
		switch (apiProvider) {
			...
			case "hicap":
				...
				break
			case "gigachat":
				if (!apiConfiguration.gigaChatApiKey) {
					return "You must provide a valid GigaChat Authorization Key or choose a different provider."
				}
				break
			case "yandexgpt":
				if (!apiConfiguration.yandexGptApiKey) {
					return "You must provide a valid YandexGPT API key or choose a different provider."
				}
				if (!apiConfiguration.yandexGptFolderId) {
					return "You must provide a Yandex Cloud folder ID."
				}
				break
		}
	}
