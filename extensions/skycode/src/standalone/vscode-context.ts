...
log("Running standalone skycode", ExtensionRegistryInfo.version)
log(`SKYCODE_ENVIRONMENT: ${process.env.SKYCODE_ENVIRONMENT}`)

// WE WILL HAVE TO MIGRATE THIS FROM DATA TO v1 LATER
const SETTINGS_SUBFOLDER = "data"

export function initializeContext(skycodeDir?: string) {
	const SKYCODE_DIR = skycodeDir || process.env.SKYCODE_DIR || `${os.homedir()}/.skycode`
	const DATA_DIR = path.join(SKYCODE_DIR, SETTINGS_SUBFOLDER)
	...
	const extensionContext: ExtensionContext = {
		...
		storagePath: WORKSPACE_STORAGE_DIR, // Deprecated, not used in skycode.
		globalStorageUri: URI.file(DATA_DIR),
		globalStoragePath: DATA_DIR, // Deprecated, not used in skycode.

		// Logs are global per extension, not per workspace.
		logUri: URI.file(DATA_DIR),
		logPath: DATA_DIR, // Deprecated, not used in skycode.

		extensionUri: URI.file(EXTENSION_DIR),
		extensionPath: EXTENSION_DIR, // Deprecated, not used in skycode.
