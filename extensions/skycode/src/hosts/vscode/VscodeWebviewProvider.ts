...
export class VscodeWebviewProvider extends WebviewProvider implements vscode.WebviewViewProvider {
	...
	public async resolveWebviewView(webviewView: vscode.WebviewView): Promise<void> {
		...
		vscode.workspace.onDidChangeConfiguration(
			async (e) => {
				if (e && e.affectsConfiguration("skycode.mcpMarketplace.enabled")) {
			...
			case "executeVsCodeCommand": {
				...
				if (message.executeVsCodeCommand) {
					...
					try {
						...
						Logger.log(`[DEV] Executed command: ${command}`)
					} catch (error) {
						Logger.error(`[DEV] Failed to execute command ${command}:`, error)
					}
				}
				break
			}
			case "updateIndexingConfig": {
				...
				if (message.indexingConfigUpdate) {
					...
					try {
						...
						Logger.error(`[Indexing] Failed to update config ${key}:`, error)
					}
				}
				break
			}
			case "indexingCommand": {
				// [SKYCODE-SKYCODE] Execute indexing command from webview
				if (message.indexingCommandAction) {
					...
					const cmd = cmdMap[message.indexingCommandAction]
					if (cmd) {
	...
	/**
	 * [SKYCODE-SKYCODE] Send a lightweight message directly to the webview, bypassing gRPC state.
	 * Used for frequent updates like indexing progress that would be too slow via postStateToWebview.
	 */
	public postDirectMessage(msg: { type: string; data: unknown }): void {
