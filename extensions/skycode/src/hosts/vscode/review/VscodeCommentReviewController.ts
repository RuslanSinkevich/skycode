...
/**
 * Skycode's GitHub avatar URL
 */
const SKYCODE_AVATAR_URL = "https://avatars.githubusercontent.com/u/184127137"
...
export class VscodeCommentReviewController extends CommentReviewController implements vscode.Disposable {
	...
	constructor() {
		...
		this.commentController = vscode.comments.createCommentController("skycode-ai-review", "Skycode AI Review")

		// Configure options for the reply input
		this.commentController.options = {
			placeHolder: "Ask a question about this code...",
			prompt: "Reply to Skycode",
		}
		...
		this.disposables.push(
			vscode.commands.registerCommand("skycode.reviewComment.reply", async (reply: vscode.CommentReply) => {
				await this.handleReply(reply)
			}),
		)

		// Register add to chat command - sends the conversation to Skycode's main chat
		this.disposables.push(
			vscode.commands.registerCommand("skycode.reviewComment.addToChat", async (thread: vscode.CommentThread) => {
	...
	addReviewComment(comment: ReviewComment): void {
		...
		const commentObj: vscode.Comment = {
			...
			author: {
				name: "Skycode",
				iconPath: vscode.Uri.parse(SKYCODE_AVATAR_URL),
			},
		}
	...
	/**
	 * Handle adding the thread conversation to Skycode's main chat
	 */
...
Please continue helping the user with their question about this code.`
	...
	/**
	 * Close all tabs that use the skycode-diff URI scheme (both diff views and regular text documents)
	 */
	async closeDiffViews(): Promise<void> {
		const tabs = vscode.window.tabGroups.all
			...
			.filter((tab) => {
				...
				// Check for regular text document tabs with skycode-diff scheme (opened during comment reveal)
