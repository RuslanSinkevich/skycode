 ...
 *
 * Core implementation of Skycode's Checkpoints system that provides version control
...
class CheckpointTracker {
	 ...
	 *
	 * Configuration:
	 * - Respects 'skycode.enableCheckpoints' VS Code setting
	 */
	public static async create(
		...
		try {
			...
			try {
				...
				// Git не установлен — бросаем с маркером, который ловится в index.ts
	...
	public async commit(): Promise<string | undefined> {
		...
		try {
			...
			if (!lockResult.acquired && !lockResult.skipped) {
				throw new Error(
					"Failed to acquire checkpoint folder lock - another Skycode instance may be performing checkpoint operations",
				)
			}
