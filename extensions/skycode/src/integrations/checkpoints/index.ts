...
import { SkycodeApiReqInfo, SkycodeMessage, SkycodeSay } from "@shared/ExtensionMessage"
import { getApiMetrics } from "@shared/getApiMetrics"
import { HistoryItem } from "@shared/HistoryItem"
import { SkycodeCheckpointRestore } from "@shared/WebviewMessage"
...
export class TaskCheckpointManager implements ICheckpointManager {
	...
	async saveCheckpoint(isAttemptCompletionMessage: boolean = false, completionMessageTs?: number): Promise<void> {
		try {
			...
			const skycodeMessages = this.services.messageStateHandler.getSkycodeMessages()
			skycodeMessages.forEach((message) => {
			...
			if (!isAttemptCompletionMessage) {
				...
				if (messageTs) {
					const messages = this.services.messageStateHandler.getSkycodeMessages()
						...
						this.state.checkpointTracker
							...
							.then(async (commitHash) => {
								if (commitHash) {
									...
									await this.services.messageStateHandler.saveSkycodeMessagesAndUpdateHistory()
								}
							})
							.catch((error) => {
								Logger.error(
				...
				const lastFiveskycodeMessages = this.services.messageStateHandler.getSkycodeMessages().slice(-3)
				const lastCompletionResultMessage = findLast(lastFiveskycodeMessages, (m) => m.say === "completion_result")
	...
	async restoreCheckpoint(
		messageTs: number,
		restoreType: SkycodeCheckpointRestore,
		offset?: number,
	): Promise<CheckpointRestoreStateUpdate> {
		try {
			const skycodeMessages = this.services.messageStateHandler.getSkycodeMessages()
			const messageIndex = skycodeMessages.findIndex((m) => m.ts === messageTs) - (offset || 0)
			// Find the last message before messageIndex that has a lastCheckpointHash
			const lastHashIndex = findLastIndex(skycodeMessages.slice(0, messageIndex), (m) => m.lastCheckpointHash !== undefined)
			const message = skycodeMessages[messageIndex]
			const lastMessageWithHash = skycodeMessages[lastHashIndex]
			...
			switch (restoreType) {
				...
				case "workspace":
					if (!this.config.enableCheckpoints) {
						// Чекпоинты отключены в настройках
						Logger.error(`[TaskCheckpointManager] Checkpoints disabled for task ${this.task.taskId}`)
						HostProvider.window.showMessage({
							type: ShowMessageType.ERROR,
							message: t("checkpoint.error.disabled"),
						})
					...
					if (message.lastCheckpointHash && this.state.checkpointTracker) {
						try {
							...
							HostProvider.window.showMessage({
								type: ShowMessageType.ERROR,
								// Не удалось восстановить чекпоинт (с деталями ошибки)
								message: t("checkpoint.error.restoreFailed", { error: errorMessage }),
							})
							didWorkspaceRestoreFail = true
						}
						...
						try {
							...
							HostProvider.window.showMessage({
								type: ShowMessageType.ERROR,
								// Не удалось восстановить смещённый чекпоинт
								message: t("checkpoint.error.restoreOffsetFailed", { error: errorMessage }),
							})
							didWorkspaceRestoreFail = true
						}
					} else if (!offset && lastMessageWithHash.lastCheckpointHash && this.state.checkpointTracker) {
						...
						try {
							...
							HostProvider.window.showMessage({
								type: ShowMessageType.ERROR,
								// Не удалось восстановить чекпоинт (fallback, с деталями)
								message: t("checkpoint.error.restoreFailed", { error: errorMessage }),
							})
							didWorkspaceRestoreFail = true
						}
					} else {
						// Нет валидного хеша для восстановления
						Logger.error(`[TaskCheckpointManager] No valid checkpoint hash for task ${this.task.taskId}`)
						HostProvider.window.showMessage({
							type: ShowMessageType.ERROR,
							message: t("checkpoint.error.noValidHash"),
						})
						didWorkspaceRestoreFail = true
					}
					break
	...
	async presentMultifileDiff(messageTs: number, seeNewChangesSinceLastTaskCompletion: boolean): Promise<void> {
		...
		try {
			if (!this.config.enableCheckpoints) {
				// Чекпоинты отключены — невозможно показать diff
				Logger.error(`[TaskCheckpointManager] Checkpoints disabled, cannot show diff for task ${this.task.taskId}`)
				HostProvider.window.showMessage({
					type: ShowMessageType.INFORMATION,
					message: t("checkpoint.error.disabledNoDiff"),
				})
				relinquishButton()
				return
			}
			...
			if (seeNewChangesSinceLastTaskCompletion) {
				...
				const lastTaskCompletedMessageCheckpointHash = findLast(
					this.services.messageStateHandler.getSkycodeMessages().slice(0, messageIndex),
				...
				if (!previousCheckpointHash) {
					// Непредвиденная ошибка — хеш чекпоинта не найден
					Logger.error(`[TaskCheckpointManager] Unexpected: no checkpoint hash for task ${this.task.taskId}`)
					HostProvider.window.showMessage({
						type: ShowMessageType.ERROR,
						message: t("checkpoint.error.unexpectedNoHash"),
					})
					relinquishButton()
					return
				}
				...
				if (!changedFiles?.length) {
					...
					HostProvider.window.showMessage({
						type: ShowMessageType.INFORMATION,
						message: t("checkpoint.info.noChanges"),
					})
					relinquishButton()
					return
				}
			...
			// Не удалось получить набор различий
			HostProvider.window.showMessage({
				type: ShowMessageType.ERROR,
				message: t("checkpoint.error.diffFailed", { error: errorMessage }),
			})
			relinquishButton()
		}
	}
	...
	async doesLatestTaskCompletionHaveNewChanges(): Promise<boolean> {
		try {
			...
			const skycodeMessages = this.services.messageStateHandler.getSkycodeMessages()
			const messageIndex = findLastIndex(skycodeMessages, (m) => m.say === "completion_result")
			const message = skycodeMessages[messageIndex]
			if (!message) {
	...
	private async handleSuccessfulRestore(
		...
		switch (restoreType) {
			...
			case "taskAndWorkspace":
				...
				const skycodeMessages = this.services.messageStateHandler.getSkycodeMessages()
				const deletedMessages = skycodeMessages.slice(messageIndex + 1)
				...
				const newSkycodeMessages = skycodeMessages.slice(0, messageIndex + 1)
				await this.services.messageStateHandler.overwriteSkycodeMessages(newSkycodeMessages) // calls saveSkycodeMessages which saves historyItem
		...
		// Уведомления об успешном восстановлении
		switch (restoreType) {
			case "task":
				HostProvider.window.showMessage({
					type: ShowMessageType.INFORMATION,
					message: t("checkpoint.info.taskRestored"),
				})
				break
			case "workspace":
				HostProvider.window.showMessage({
					type: ShowMessageType.INFORMATION,
					message: t("checkpoint.info.workspaceRestored"),
				})
				break
			case "taskAndWorkspace":
				HostProvider.window.showMessage({
					type: ShowMessageType.INFORMATION,
					message: t("checkpoint.info.taskAndWorkspaceRestored"),
				})
				break
		}
	...
	private async initializeCheckpointTracker(): Promise<CheckpointTracker | undefined> {
		...
		try {
			// Таймер предупреждения — если инициализация > 7 сек, показываем баннер
			checkpointsWarningTimer = setTimeout(async () => {
				if (!checkpointsWarningShown) {
					checkpointsWarningShown = true
					// Отправляем i18n-ключ — фронтенд переведёт через t()
					await this.setcheckpointManagerErrorMessage("checkpoint.error.slowInit")
				}
			}, 7_000)

			// Таймаут — если инициализация > 15 сек, отключаем чекпоинты для задачи
			const workspacePath = await this.getWorkspacePath()
			const tracker = await pTimeout(
				CheckpointTracker.create(this.task.taskId, this.config.enableCheckpoints, workspacePath),
				{
					milliseconds: 15_000,
					// Это сообщение pTimeout — используется для проверки ниже в catch
					message: "Checkpoints taking too long to initialize",
				},
			)
			...
			// Таймаут — отправляем i18n-ключ для баннера
			if (errorMessage.includes("Checkpoints taking too long to initialize")) {
				await this.setcheckpointManagerErrorMessage("checkpoint.error.timeout")
			} else if (errorMessage.includes("Git must be installed")) {
				// Git не установлен — отправляем ключ для баннера с инструкциями
				await this.setcheckpointManagerErrorMessage("checkpoint.error.gitRequired")
			} else {
				// Динамическая ошибка — отправляем как есть (фронтенд покажет as-is)
				await this.setcheckpointManagerErrorMessage(errorMessage)
			}
			return undefined
		} finally {
