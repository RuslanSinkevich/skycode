...
// Export SyntaxValidator for code integrity checking
export { SyntaxValidator, syntaxValidator } from "./SyntaxValidator"
export type { SyntaxError, ValidationResult, ChangeValidationResult } from "./SyntaxValidator"
...
export async function parseSourceCodeForDefinitionsTopLevel(
	...
	const allowedFilesToParse = skycodeIgnoreController ? skycodeIgnoreController.filterPaths(filesToParse) : filesToParse

	for (const filePath of allowedFilesToParse) {
		const definitions = await parseFile(filePath, languageParsers, skycodeIgnoreController)
		if (definitions) {
...
async function parseFile(
	...
	if (skycodeIgnoreController && !skycodeIgnoreController.validateAccess(filePath)) {
		return null
	}
