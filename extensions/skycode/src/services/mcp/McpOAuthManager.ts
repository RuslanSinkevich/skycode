...
/**
 * Implementation of OAuthClientProvider for Skycode
 * Manages OAuth state and token storage for a single MCP server
 */
class SkycodeOAuthClientProvider implements OAuthClientProvider {
	...
	get clientMetadata(): OAuthClientMetadata {
		return {
			...
			response_types: ["code"],
			client_name: "Skycode",
			client_uri: "https://skycode-ai.local", // TODO: заменить на свой домен
			software_id: "skycode",
		}
	}
...
export class McpOAuthManager {
	...
	async getOrCreateProvider(serverName: string, serverUrl: string): Promise<OAuthClientProvider> {
		...
		// Create provider
		const provider = new SkycodeOAuthClientProvider(serverName, serverUrl)
