...
export class McpHub {
	 ...
	 */
	private isUpdatingSkycodeSettings: boolean = false
	...
	private async watchMcpSettingsFile(): Promise<void> {
		...
		this.settingsWatcher.on("change", async () => {
			...
			// Skip processing if we're updating Skycode-specific settings (autoApprove, timeout)
			if (this.isUpdatingSkycodeSettings) {
				return
			}
	...
	async updateServerConnectionsRPC(newServers: Record<string, McpServerConfig>): Promise<void> {
		...
		for (const [name, config] of Object.entries(newServers)) {
			...
			if (!currentConnection) {
				...
				// Existing server with changed connection config (excludes Skycode-specific settings)
				try {
					if (config.type === "stdio") {
						this.setupFileWatcher(name, config)
					}
				...
				// Only Skycode-specific settings changed - update in-memory state without restart
				...
				// Also update Skycode-specific settings in the stored config.
	...
	async updateServerConnections(newServers: Record<string, McpServerConfig>): Promise<void> {
		...
		// Track if any connection-level changes occurred (excludes Skycode-specific settings)
		...
		// For Skycode-specific settings changes, the RPC response already updated the webview,
	 ...
	 * Excludes Skycode-specific settings since they don't affect the MCP server transport connection.
	 *
	 * ## Skycode-specific settings (don't require restart):
	 ...
	 *
	 * ## Adding new Skycode-specific settings:
	 ...
	 * 2. Add it to `isUpdatingSkycodeSettings` flag usage in the update function
	...
	private configsRequireRestart(oldConfig: McpServerConfig, newConfig: McpServerConfig): boolean {
		// Exclude Skycode-specific settings from comparison (add new ones here)
	...
	private setupFileWatcher(name: string, config: Extract<McpServerConfig, { type: "stdio" }>) {
		...
		if (filePath) {
			// we use chokidar instead of onDidSaveTextDocument because it doesn't require the file to be open in the editor. The settings config is better suited for onDidSave since that will be manually updated by the user or Skycode (and we want to detect save events, not every file change)
	...
	async toggleToolAutoApproveRPC(serverName: string, toolNames: string[], shouldAllow: boolean): Promise<McpServer[]> {
		...
		this.isUpdatingSkycodeSettings = true
		try {
			const settingsPath = await getMcpSettingsFilePathHelper(await this.getSettingsDirectoryPath())
			const content = await fs.readFile(settingsPath, "utf-8")
			const config = JSON.parse(content)
