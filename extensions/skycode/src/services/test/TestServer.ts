import { getSavedApiConversationHistory, getSavedSkycodeMessages } from "@core/storage/disk"
...
export async function createTestServer(controller: Controller): Promise<http.Server> {
	// Try to show the Skycode sidebar
	Logger.log("[createTestServer] Opening Skycode in sidebar...")
		...
		req.on("end", async () => {
			try {
				...
				if (!visibleWebview || !visibleWebview.controller) {
					res.writeHead(500)
					res.end(JSON.stringify({ error: "No active Skycode instance found" }))
					return
				}
				...
				try {
					...
					if (apiKey) {
						...
						const updatedConfig = {
							...apiConfiguration,
							apiProvider: "skycode" as ApiProvider,
							skycodeAccountId: apiKey,
						}

						// Store the API key securely
						visibleWebview.controller.stateManager.setSecret("skycodeAccountId", apiKey)

						visibleWebview.controller.stateManager.setApiConfiguration(updatedConfig)

						// Update cache service to use skycode provider
				...
				if (mode !== "act") {
					// Switch to Act mode if not already there
					...
					try {
						...
						try {
							if (typeof taskId === "string") {
								messages = await getSavedSkycodeMessages(taskId)
							}
						} catch (error) {
							Logger.log(`Error getting saved Skycode messages: ${error}`)
						}
