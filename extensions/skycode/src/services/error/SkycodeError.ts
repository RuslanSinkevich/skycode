...
export class SkycodeError extends Error {
	readonly title = "SkycodeError"
	...
	/**
	 * Parses a stringified error into a SkycodeError instance.
	 */
	static parse(errorStr?: string, modelId?: string): SkycodeError | undefined {
		if (!errorStr || typeof errorStr !== "string") {
			return undefined
		}
		return SkycodeError.transform(errorStr, modelId)
	}

	/**
	 * Transforms any object into a SkycodeError instance.
	 * Always returns a SkycodeError, even if the input is not a valid error object.
	 */
	static transform(error: any, modelId?: string, providerId?: string): SkycodeError {
		try {
			// If already a SkycodeError, return it directly to prevent infinite recursion
			if (error instanceof SkycodeError) {
				return error
			}
			return new SkycodeError(JSON.parse(error), modelId, providerId)
		} catch {
			return new SkycodeError(error, modelId, providerId)
		}
	}

	public isErrorType(type: SkycodeErrorType): boolean {
		return SkycodeError.getErrorType(this) === type
	}
	 ...
	 */
	static getErrorType(err: SkycodeError): SkycodeErrorType | undefined {
		...
		if (code === "insufficient_credits" && typeof details?.current_balance === "number") {
			return SkycodeErrorType.Balance
		}
		...
		if (message) {
			...
			if (message?.includes(SKYCODE_ACCOUNT_AUTH_ERROR_MESSAGE) || authErrorRegex.some((regex) => regex.test(message))) {
				return SkycodeErrorType.Auth
			}
			...
			if (RATE_LIMIT_PATTERNS.some((pattern) => pattern.test(lowerMessage))) {
				return SkycodeErrorType.RateLimit
			}
		}
...
export class AuthNetworkError extends Error {
	constructor(
		...
		super(message)
		this.name = SkycodeErrorType.Network
	}
}
...
export class AuthInvalidTokenError extends Error {
	constructor(message: string) {
		super(message)
		this.name = SkycodeErrorType.Auth
	}
}
