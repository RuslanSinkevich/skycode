...
export class PostHogClientProvider {
	 ...
	 * For exceptions, we only capture those from the Skycode extension.
	 * this is specifically to avoid capturing errors from anything other than Skycode
	 */
	static eventFilter(event: EventMessage | null) {
		...
		// Check if any exception is from Skycode
		for (let i = 0; i < exceptionList.length; i++) {
			const stacktrace = exceptionList[i].stacktrace
			// Fast check: error message contains "skycode"
			if (stacktrace?.value?.toLowerCase().includes("skycode")) {
				return event
			}
			// Check stack frames for Skycode extension path
			const frames = stacktrace?.frames
			if (frames?.length) {
				for (let j = 0; j < frames.length; j++) {
					if (frames[j]?.filename?.includes("skycode")) {
						return event
					}
				}
			}
