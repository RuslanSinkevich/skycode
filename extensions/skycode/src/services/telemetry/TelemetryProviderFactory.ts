...
export type TelemetryProviderConfig =
	 ...
	 * @param bypassUserSettings - When true, telemetry is sent regardless of the user's Skycode telemetry opt-in/opt-out settings.
	 * This is used for:
	 * 	- User-controlled collectors configured via environment variables (e.g., SKYCODE_OTEL_TELEMETRY_ENABLED).
	...
	public static getDefaultConfigs(): TelemetryProviderConfig[] {
		...
		if (runtimeOtelConfig) {
			configs.push({
				...
				// If the user has `SKYCODE_OTEL_TELEMETRY_ENABLED` in his environment, enable
				// OTEL regardless of his Skycode telemetry settings
