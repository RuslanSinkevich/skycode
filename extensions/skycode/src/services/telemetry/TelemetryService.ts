...
export type TelemetryMetadata = {
	/**
	 * The extension or skycode-core version. JetBrains and CLI have different
	 * versioning than the VSCode Extension, but on those platforms this will be the _skycode-core version_
	 * which uses the same as the versioning as the VSCode extension.
	 */
	extension_version: string
	/**
	 * The type of skycode distribution, e.g VSCode Extension, JetBrains Plugin or CLI. This
...
/**
 * TelemetryService handles telemetry event tracking for the Skycode extension
...
export class TelemetryService {
	...
	public static readonly METRICS = {
		TASK: {
			TURNS_TOTAL: "skycode.turns.total",
			TURNS_PER_TASK: "skycode.turns.per_task",
			TOKENS_INPUT_TOTAL: "skycode.tokens.input.total",
			TOKENS_INPUT_PER_RESPONSE: "skycode.tokens.input.per_response",
			TOKENS_OUTPUT_TOTAL: "skycode.tokens.output.total",
			TOKENS_OUTPUT_PER_RESPONSE: "skycode.tokens.output.per_response",
			COST_TOTAL: "skycode.cost.total",
			COST_PER_EVENT: "skycode.cost.per_event",
		},
		CACHE: {
			WRITE_TOTAL: "skycode.cache.write.tokens.total",
			WRITE_PER_EVENT: "skycode.cache.write.tokens.per_event",
			READ_TOTAL: "skycode.cache.read.tokens.total",
			READ_PER_EVENT: "skycode.cache.read.tokens.per_event",
			HITS_TOTAL: "skycode.cache.hits.total",
		},
		TOOLS: {
			CALLS_TOTAL: "skycode.tool.calls.total",
			CALLS_PER_TASK: "skycode.tool.calls.per_task",
		},
		ERRORS: {
			TOTAL: "skycode.errors.total",
			PER_TASK: "skycode.errors.per_task",
		},
		API: {
			TTFT_SECONDS: "skycode.api.ttft.seconds",
			DURATION_SECONDS: "skycode.api.duration.seconds",
			THROUGHPUT_TOKENS_PER_SECOND: "skycode.api.throughput.tokens_per_second",
		},
		HOOKS: {
			EXECUTIONS_TOTAL: "skycode.hooks.executions.total",
			DURATION_SECONDS: "skycode.hooks.duration.seconds",
			FAILURES_TOTAL: "skycode.hooks.failures.total",
			CANCELLATIONS_TOTAL: "skycode.hooks.cancellations.total",
			CONTEXT_MODIFICATIONS_TOTAL: "skycode.hooks.context_modifications.total",
			CACHE_ACCESSES_TOTAL: "skycode.hooks.cache.accesses.total",
		},
	}
	...
	private static readonly EVENTS = {
		...
		TASK: {
			...
			// Tracks when individual Skycode rules are toggled on/off
			...
			// Tracks when Skycode web tools setting is toggled on/off
			SKYCODE_WEB_TOOLS_TOGGLED: "task.skycode_web_tools_toggled",
			...
			// Search guardrails and strategy telemetry
			SEARCH_GUARDRAIL_TRIGGERED: "task.search_guardrail_triggered",
			SEARCH_STRATEGY_CHOSEN: "task.search_strategy_chosen",
			SEARCH_FILES_PROFILE_APPLIED: "task.search_files_profile_applied",
			SEARCH_FILES_TIMEOUT: "task.search_files_timeout",
			SEARCH_FILES_SLOW_QUERY: "task.search_files_slow_query",
	...
	public static async create(): Promise<TelemetryService> {
		...
		const metadata: TelemetryMetadata = {
			...
			skycode_type: hostVersion.skycodeType || "unknown",
			os_type: os.platform(),
	 ...
	 */
	public identifyAccount(userInfo: SkycodeAccountUserInfo) {
		const propertiesWithMetadata: TelemetryProperties = {
			...this.telemetryMetadata,
		}
	...
	/**
	 * Records when skycode calls the task completion_result tool signifying that skycode is done with the task
	...
	/**
	 * Records when individual Skycode rules are toggled on/off
	 ...
	 */
	public captureSkycodeRuleToggled(ulid: string, ruleFileName: string, enabled: boolean, isGlobal: boolean) {
	...
	/**
	 * Records when Skycode web tools are enabled/disabled by the user
	 * @param ulid Unique identifier for the task
	 * @param enabled Whether Skycode web tools are enabled (true) or disabled (false)
	 */
	public captureSkycodeWebToolsToggle(ulid: string, enabled: boolean) {
		this.capture({
			event: TelemetryService.EVENTS.TASK.SKYCODE_WEB_TOOLS_TOGGLED,
			properties: {
				ulid,
				enabled,
			},
	...
	public captureWorkspaceInitialized(
		...
		this.recordGauge("skycode.workspace.active_roots", rootCount, {
			is_multi_root: isMultiRoot,
		})
		// Retire the previous series to avoid leaking gauge entries when the flag flips.
		this.recordGauge("skycode.workspace.active_roots", null, {
			is_multi_root: !isMultiRoot,
		})
	}
	...
	public captureSearchGuardrailTriggered(
		ulid: string,
		reason: string,
		pathScope: "workspace_root" | "subdir" | "multi_root",
		appliedSafeProfile: boolean,
	) {
		this.capture({
			event: TelemetryService.EVENTS.TASK.SEARCH_GUARDRAIL_TRIGGERED,
			properties: {
				ulid,
				reason,
				path_scope: pathScope,
				applied_safe_profile: appliedSafeProfile,
			},
		})
	}

	public captureSearchStrategyChosen(
		ulid: string,
		strategy: "index_first" | "regex_only" | "regex_first",
		shortlistCount: number,
		hadWorkspaceHint: boolean,
	) {
		this.capture({
			event: TelemetryService.EVENTS.TASK.SEARCH_STRATEGY_CHOSEN,
			properties: {
				ulid,
				strategy,
				shortlist_count: shortlistCount,
				had_workspace_hint: hadWorkspaceHint,
			},
		})
	}

	public captureSearchFilesProfileApplied(
		ulid: string,
		profile: "safe_code_files",
		filePattern: string,
		reason: string,
	) {
		this.capture({
			event: TelemetryService.EVENTS.TASK.SEARCH_FILES_PROFILE_APPLIED,
			properties: {
				ulid,
				profile,
	...
	public captureSearchFilesTimeout(
		ulid: string,
		searchDurationMs: number,
		pathScope: "workspace_root" | "subdir" | "multi_root",
	) {
		this.capture({
			event: TelemetryService.EVENTS.TASK.SEARCH_FILES_TIMEOUT,
			properties: {
				ulid,
				search_duration_ms: searchDurationMs,
				path_scope: pathScope,
	...
	public captureSearchSlowQuery(
		...
		this.capture({
			event: TelemetryService.EVENTS.TASK.SEARCH_FILES_SLOW_QUERY,
			properties: {
				ulid,
				search_duration_ms: searchDurationMs,
				path_scope: pathScope,
				regex_length: regex.length,
				has_file_pattern: !!filePattern,
			},
		})
	}
