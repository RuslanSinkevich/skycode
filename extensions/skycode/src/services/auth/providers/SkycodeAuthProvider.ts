...
import { SkycodeEnv, EnvironmentConfig } from "@/config"
import { Controller } from "@/core/controller"
import { HostProvider } from "@/hosts/host-provider"
import { buildBasicSkycodeHeaders } from "@/services/EnvUtils"
import { AuthInvalidTokenError, AuthNetworkError } from "@/services/error/SkycodeError"
import { telemetryService } from "@/services/telemetry"
import { SKYCODE_API_ENDPOINT } from "@/shared/skycode/api"
import { fetch, getAxiosSettings } from "@/shared/net"
import { Logger } from "@/shared/services/Logger"
import { type SkycodeAccountUserInfo, type SkycodeAuthInfo } from "../AuthService"
import { parseJwtPayload } from "../oca/utils/utils"

interface SkycodeAuthApiUser {
	subject: string | null
	email: string
	name: string
	skycodeUserId: string | null
	accounts: string[] | null
}

// Unified API response data shape for token exchange/refresh
interface SkycodeAuthResponseData {
	/**
...
export interface SkycodeAuthApiTokenExchangeResponse {
	success: boolean
	data: SkycodeAuthResponseData
}

export interface SkycodeAuthApiTokenRefreshResponse {
	success: boolean
	data: SkycodeAuthResponseData
}

export class SkycodeAuthProvider {
	readonly name = "skycode"
	...
	private clearSession(controller: Controller, reason: string, storedAuthData?: SkycodeAuthInfo) {
		Logger.error(reason)
		...
		controller.stateManager.setSecret("skycode:skycodeAccountId", undefined)
		this.refreshRetryCount = 0
		this.lastRefreshAttempt = 0
		return null
	}

	private logFailedRefreshAttempt(response: Response, storedAuthData?: SkycodeAuthInfo) {
		const startedAt = storedAuthData?.startedAt
		const timeSinceStarted = Date.now() - (startedAt || 0)
	...
	/**
	 * Retrieves Skycode auth info using the stored access token.
	 * @param controller - The controller instance to access stored secrets.
	 * @returns {Promise<SkycodeAuthInfo | null>} A promise that resolves with the auth info or null.
	 */
	async retrieveSkycodeAuthInfo(controller: Controller): Promise<SkycodeAuthInfo | null> {
		try {
			// Get the stored auth data from secure storage
			const storedAuthDataString = controller.stateManager.getSecretKey("skycode:skycodeAccountId")
			...
			let storedAuthData: SkycodeAuthInfo
			try {
			...
			if (await this.shouldRefreshIdToken(storedAuthData.refreshToken, storedAuthData.expiresAt)) {
				...
				try {
					...
					if (newAuthInfoString !== storedAuthDataString) {
						controller.stateManager.setSecret("skycodeAccountId", undefined) // cleanup old key
						controller.stateManager.setSecret("skycode:skycodeAccountId", newAuthInfoString)
					}
					...
					// when the user actually tries to use Skycode, not at startup
					return storedAuthData
				}
			}
	 ...
	 * @returns {Promise<SkycodeAuthInfo>} The new access token and user info.
	 */
	async refreshToken(refreshToken: string, storedData: SkycodeAuthInfo): Promise<SkycodeAuthInfo> {
		try {
			const endpoint = new URL(SKYCODE_API_ENDPOINT.REFRESH_TOKEN, this.config.apiBaseUrl)
			...
			const data: SkycodeAuthApiTokenExchangeResponse = await response.json()
	...
	async getAuthRequest(callbackUrl: string): Promise<string> {
		const authUrl = new URL(SKYCODE_API_ENDPOINT.AUTH, this.config.apiBaseUrl)
		...
		try {
			...
			Logger.error("Auth fetch failed. URL:", authUrl.toString())
			Logger.error("Error name:", error?.name)
			Logger.error("Error message:", error?.message)
			Logger.error("Error code:", error?.code)
			Logger.error("Error cause:", error?.cause)
			if (error?.cause) {
				Logger.error("Cause name:", error.cause?.name)
				Logger.error("Cause message:", error.cause?.message)
				Logger.error("Cause code:", error.cause?.code)
				Logger.error("Cause errno:", error.cause?.errno)
				Logger.error("Cause syscall:", error.cause?.syscall)
			}
			...
			if (cause) {
				const parts = [cause.code, cause.message, cause.errno, cause.syscall].filter(Boolean)
				if (parts.length > 0) {
	...
	async signIn(controller: Controller, authorizationCode: string, provider: string): Promise<SkycodeAuthInfo | null> {
		try {
			...
			const tokenUrl = new URL(SKYCODE_API_ENDPOINT.TOKEN_EXCHANGE, this.config.apiBaseUrl)
			...
			const responseType: SkycodeAuthApiTokenExchangeResponse = responseJSON
			...
			controller.stateManager.setSecret("skycode:skycodeAccountId", JSON.stringify(skycodeAuthInfo))

			return skycodeAuthInfo
		} catch (error) {
	...
	private async fetchRemoteUserInfo(tokenData: SkycodeAuthApiTokenExchangeResponse["data"]): Promise<SkycodeAccountUserInfo> {
		try {
			const userResponse = await axios.get(`${SkycodeEnv.config().apiBaseUrl}/api/v1/users/me`, {
				headers: {
			...
			return {
				id: tokenData.userInfo.skycodeUserId || "",
	...
	private async headers() {
		return {
			...
			...(await buildBasicSkycodeHeaders()),
		}
	}
}
