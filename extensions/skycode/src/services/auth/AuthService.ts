import { AuthState, UserInfo } from "@shared/proto/skycode/account"
import { type EmptyRequest, String } from "@shared/proto/skycode/common"
...
export interface SkycodeAuthInfo {
	/**
	 * accessToken
	 */
	idToken: string
	 ...
	 */
	expiresAt?: number
	userInfo: SkycodeAccountUserInfo
	provider: string
	startedAt?: number
}

export interface SkycodeAccountUserInfo {
	createdAt: string
	displayName: string
	email: string
	id: string
	organizations: SkycodeAccountOrganization[]
	/**
	 * Skycode app base URL, used for webview UI and other client-side operations
	 */
	appBaseUrl?: string
	/**
...
export interface SkycodeAccountOrganization {
	active: boolean
	memberId: string
	name: string
...
export class AuthService {
	...
	protected _skycodeAuthInfo: SkycodeAuthInfo | null = null
	protected _provider: SkycodeAuthProvider
	...
	protected constructor(controller: Controller) {
		this._provider = new SkycodeAuthProvider()
	...
	getActiveOrganizationId(): string | null {
		if (!this._skycodeAuthInfo?.userInfo?.organizations) {
			return null
		}
		const activeOrg = this._skycodeAuthInfo.userInfo.organizations.find((org) => org.active)
	 ...
	 */
	getUserOrganizations(): SkycodeAccountOrganization[] | undefined {
		return this._skycodeAuthInfo?.userInfo?.organizations
	}

	private async internalGetAuthToken(provider: SkycodeAuthProvider): Promise<string | null> {
		try {
			let skycodeAccountAuthToken = this._skycodeAuthInfo?.idToken
			if (!this._skycodeAuthInfo || !skycodeAccountAuthToken || this._skycodeAuthInfo.provider !== provider.name) {
				// Not authenticated
				return null
			}

			// Check if token has expired
			if (await provider.shouldRefreshIdToken(skycodeAccountAuthToken, this._skycodeAuthInfo.expiresAt)) {
				...
				this._refreshPromise = (async () => {
					...
					try {
						const updatedAuthInfo = await provider.retrieveSkycodeAuthInfo(this._controller)
						if (updatedAuthInfo) {
							this._skycodeAuthInfo = updatedAuthInfo
							this._authenticated = true
							skycodeAccountAuthToken = updatedAuthInfo.idToken
					...
					return skycodeAccountAuthToken
				})()

				skycodeAccountAuthToken = await this._refreshPromise
			}

			return skycodeAccountAuthToken || null
		} catch (error) {
	 ...
	 * @returns The provider name (e.g., "skycode", "firebase"), or null if not authenticated
	 */
	getProviderName(): string | null {
		return this._skycodeAuthInfo?.provider ?? null
	}
	...
	getInfo(): AuthState {
		...
		let user: any = null
		if (this._skycodeAuthInfo && this._authenticated) {
			const userInfo = this._skycodeAuthInfo.userInfo
			this._skycodeAuthInfo.userInfo.appBaseUrl = SkycodeEnv.config()?.appBaseUrl
	...
	private async retrieveAuthInfo(): Promise<SkycodeAuthInfo | null> {
		// If a refresh is already in progress, wait for it to complete
		if (this._refreshPromise) {
			Logger.info("Token refresh already in progress, waiting for completion")
		...
		return this._provider.retrieveSkycodeAuthInfo(this._controller)
	}
	...
	private destroyTokens() {
		this._controller.stateManager.setSecret("skycodeAccountId", undefined)
		this._controller.stateManager.setSecret("skycode:skycodeAccountId", undefined)
	}
}
