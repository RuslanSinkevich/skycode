import { String } from "@shared/proto/skycode/common"
...
export class AuthServiceMock extends AuthService {
	protected constructor(controller: Controller) {
		...
		if (process?.env?.SKYCODE_ENVIRONMENT !== "local") {
	...
	override async getAuthToken(): Promise<string | null> {
		...
		return this._skycodeAuthInfo.idToken
	}

	override async createAuthRequest(): Promise<String> {
		// Use URL object for more graceful query construction
		const authUrl = new URL(SkycodeEnv.config().apiBaseUrl)
		const authUrlString = authUrl.toString()
		// Call the parent implementation
		if (this._authenticated && this._skycodeAuthInfo) {
			Logger.log("Already authenticated with mock server")
			return String.create({ value: authUrlString })
		}

		try {
			// Use token exchange endpoint like SkycodeAuthProvider
			const tokenExchangeUri = new URL(SKYCODE_API_ENDPOINT.TOKEN_EXCHANGE, SkycodeEnv.config().apiBaseUrl)
			...
			// Convert to SkycodeAuthInfo format matching SkycodeAuthProvider
			this._skycodeAuthInfo = {
				idToken: authData.accessToken,
				refreshToken: authData.refreshToken,
				expiresAt: new Date(authData.expiresAt).getTime() / 1000,
				userInfo: {
					id: authData.userInfo.skycodeUserId || authData.userInfo.subject,
					email: authData.userInfo.email,
					displayName: authData.userInfo.name,
					createdAt: new Date().toISOString(),
					organizations: authData.organizations,
					appBaseUrl: SkycodeEnv.config().appBaseUrl,
