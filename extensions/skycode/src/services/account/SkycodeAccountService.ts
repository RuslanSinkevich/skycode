...
export class SkycodeAccountService {
	private static instance: SkycodeAccountService
	...
	/**
	 * Returns the singleton instance of SkycodeAccountService
	 * @returns Singleton instance of SkycodeAccountService
	 */
	public static getInstance(): SkycodeAccountService {
		if (!SkycodeAccountService.instance) {
			SkycodeAccountService.instance = new SkycodeAccountService()
		}
		return SkycodeAccountService.instance
	}

	/**
	 * Returns the base URL for the Skycode API
	 * @returns The base URL as a string
	 */
	get baseUrl(): string {
		return SkycodeEnv.config().apiBaseUrl
	}

	/**
	 * Helper function to make authenticated requests to the Skycode API
	...
	private async authenticatedRequest<T>(endpoint: string, config: AxiosRequestConfig = {}): Promise<T> {
		...
		if (!skycodeAccountAuthToken) {
			throw new Error("No Skycode account auth token found")
		}
	...
	async fetchMe(): Promise<UserResponse | undefined> {
		try {
			const data = await this.authenticatedRequest<UserResponse>(SKYCODE_API_ENDPOINT.USER_INFO)
			return data
		} catch (error) {
	...
	async switchAccount(organizationId?: string): Promise<void> {
		...
		try {
			...
			const _response = await this.authenticatedRequest<string>(SKYCODE_API_ENDPOINT.ACTIVE_ACCOUNT, {
				method: "PUT",
				headers: {
					"Content-Type": "application/json",
				},
	...
	/**
	 * Transcribes audio using the Skycode transcription service
