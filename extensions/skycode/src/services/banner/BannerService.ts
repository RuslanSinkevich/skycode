import type { Banner, BannerRules, BannersResponse } from "@shared/SkycodeBanner"
import { BannerActionType, type BannerCardData } from "@shared/skycode/banner"
...
import { buildBasicSkycodeHeaders } from "../EnvUtils"
...
export class BannerService {
	...
	private async fetchActiveBanners(): Promise<Banner[]> {
		try {
			...
			const urlObj = new URL("/api/banners/v1/messages", this._baseUrl)
			...
			const backendFilteredBanners = response.data.data.items.map((banner) => ({
				...banner,
				actions: typeof banner.actions === "string"
					? JSON.parse(banner.actions || "[]")
					: Array.isArray(banner.actions) ? banner.actions : [],
			}))
	...
	private async getExtensionVersion(): Promise<string> {
		try {
			...
			return hostVersion.skycodeVersion || ""
		} catch (error) {
	...
	private async getIdeType(): Promise<string> {
		try {
			...
			// Use skycodeType field which contains values like "VSCode Extension", "Skycode for JetBrains", "CLI", etc.
			const skycodeType = hostVersion.skycodeType?.toLowerCase() || ""

			if (skycodeType.includes("vscode")) {
				return "vscode"
			}
			if (skycodeType.includes("jetbrains")) {
				return "jetbrains"
			}
			if (skycodeType.includes("cli")) {
				return "cli"
			}
	...
	public async sendBannerEvent(bannerId: string, eventType: "dismiss"): Promise<void> {
		try {
			const url = new URL("/api/banners/v1/events", this._baseUrl).toString()
	...
	private convertToBannerCardData(banner: Banner): BannerCardData | null {
		...
		const actionsArray = Array.isArray(banner.actions) ? banner.actions : []
		...
		const placement = (banner.placement === "modal" || banner.placement === "banner")
			? banner.placement
			: "banner" as const
	...
	public async getActiveBanners(forceRefresh = false): Promise<BannerCardData[]> {
		try {
			const banners = await this.internalGetActiveBanners(forceRefresh)
			...
			Logger.error("BannerService: Error getting active banners", error)
			return []
		}
	}
