...
const WHISPER_MODELS: Record<string, { file: string; sizeMB: number; archive: string; bundled: boolean }> = {
	tiny: { file: "ggml-tiny.bin", sizeMB: 75, archive: "skycode-voice-tiny.zip", bundled: true },
	base: { file: "ggml-base.bin", sizeMB: 142, archive: "skycode-voice-base.zip", bundled: false },
	small: { file: "ggml-small.bin", sizeMB: 466, archive: "skycode-voice-small.zip", bundled: false },
}
	...
	private async downloadFile(
		...
		return new Promise((resolve, reject) => {
			...
			const doRequest = (requestUrl: string, redirectCount = 0) => {
				...
				const headers: Record<string, string> = { "User-Agent": "Skycode" }
				if (this.authToken) {
	...
	async ensureReady(onProgress?: (message: string, pct?: number) => void): Promise<void> {
		...
		try {
			if (bundled) {
				...
				const archiveUrl = `${SKYCODE_VOICE_CDN}/${model.archive}`
