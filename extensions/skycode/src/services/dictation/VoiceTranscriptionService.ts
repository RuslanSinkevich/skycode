...
const STATUS_ERROR_MAP = new Map<number, string>([
	[401, "Authentication failed. Please reauthenticate your Skycode account"],
...
export class VoiceTranscriptionService {
	private readonly skycodeAccountService: SkycodeAccountService

	constructor() {
		this.skycodeAccountService = SkycodeAccountService.getInstance()
	}
	...
	private parseTranscriptionError(error: unknown): { error: string } {
		...
		if (axios.isAxiosError(error)) {
			...
			return {
				error: "Transcription failed. Please try again later or raise an issue on #",
			}
		}
	...
	async transcribeAudio(audioBase64: string, language?: string): Promise<{ text?: string; error?: string }> {
		try {
			Logger.info("Transcribing audio with Skycode transcription service...")
			...
			const result = await this.skycodeAccountService.transcribeAudio(audioBase64, language)
