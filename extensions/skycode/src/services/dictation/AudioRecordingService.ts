...
export class AudioRecordingService {
	 ...
	 * On Windows: sends "q" to stdin (ffmpeg's graceful stop).
	...
	private async terminateProcess(): Promise<void> {
		...
		if (process.platform === "win32") {
			// Windows: SIGINT doesn't work — ffmpeg won't finalize the file.
			// Send "q\n" to stdin instead (ffmpeg says "Press [q] to stop").
			try {
		...
		await new Promise<void>((resolve) => {
			const timeoutId = setTimeout(() => {
				...
				try { this.recordingProcess?.kill("SIGKILL") } catch { /* ignore */ }
				resolve()
			}, 5000)
	...
	async startRecording(deviceId?: string): Promise<{ success: boolean; error?: string }> {
		try {
			...
			this.outputFile = path.join(tempDir, `skycode_recording_${Date.now()}.webm`)
			...
			// Set up recording arguments (pass deviceId for user-selected microphone)
			const args = recordProgram.getArgs(this.outputFile, deviceId)
			// Spawn the recording process (stdin: pipe needed for Windows graceful stop via "q")
			this.recordingProcess = spawn(recordProgram.path, args, { stdio: ["pipe", "pipe", "pipe"] })
			...
			this.recordingProcess.stderr?.on("data", (data) => {
				...
				if (message && !message.includes("In:") && !message.includes("Out:") && !message.startsWith("size=")) {
					Logger.info(`Recording stderr: ${message.substring(0, 200)}`)
				}
			})
	...
	private getRecordProgram(): { path: string; getArgs: (outputFile: string, deviceId?: string) => string[] } | undefined {
		const platform = os.platform() as keyof typeof AUDIO_PROGRAM_CONFIG
		const config = AUDIO_PROGRAM_CONFIG[platform]
		...
		// 0. Check our bundled ffmpeg first (from WhisperLocalService)
		try {
			const { getWhisperLocalService } = require("./WhisperLocalService")
			const whisper = getWhisperLocalService()
			if (whisper) {
				const bundledPath = whisper.ffmpegPath
				if (fs.existsSync(bundledPath)) {
					Logger.info(`[AudioRecording] Using bundled ffmpeg: ${bundledPath}`)
					return { path: bundledPath, getArgs: config.getArgs }
				}
			}
		} catch {
			// WhisperLocalService not initialized yet — fall through
		}
