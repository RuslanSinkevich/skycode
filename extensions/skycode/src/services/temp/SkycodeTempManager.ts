/**
 * SkycodeTempManager - Manages temporary files for Skycode with automatic cleanup.
 *
 * Simple approach:
 * - Uses a "skycode" subdirectory inside the system temp dir (falls back to system temp if creation fails)
...
/**
 * Singleton manager for Skycode's temporary files.
 */
class SkycodeTempManagerImpl {
	private readonly tempDir: string
	private cleanupIntervalId: NodeJS.Timeout | null = null

	constructor() {
		// Uses system temp directory with a dedicated "skycode" subdirectory when possible:
		// macOS: /var/folders/xx/.../T/skycode
		// Windows: C:\Users\{user}\AppData\Local\Temp\skycode
		// Linux: /tmp/skycode
		const baseTempDir = os.tmpdir()
		const skycodeTempDir = path.join(baseTempDir, "skycode")

		try {
			fs.mkdirSync(skycodeTempDir, { recursive: true })
			this.tempDir = skycodeTempDir
		} catch {
	...
	/**
	 * Clean up old Skycode temp files based on age and total size constraints.
	 * Called on extension activation.
	 *
	 * Strategy:
	 * 1. Scan the Skycode temp directory
	...
	async cleanup(): Promise<{ deletedCount: number; freedBytes: number }> {
		...
		try {
			...
			if (deletedCount > 0) {
				Logger.info(`Skycode temp cleanup: deleted ${deletedCount} files, freed ${Math.round(freedBytes / 1024 / 1024)}MB`)
			}
		} catch (error) {
			Logger.error("Error during Skycode temp cleanup", error)
		}
...
export const SkycodeTempManager = new SkycodeTempManagerImpl()
