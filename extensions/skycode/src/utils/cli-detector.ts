...
/**
 * Check if the Skycode CLI tool is installed on the system
 * @returns true if CLI is installed, false otherwise
 */
export async function isSkycodeCliInstalled(): Promise<boolean> {
	try {
		// Try to get the version of the skycode CLI tool
		// This will fail if the tool is not installed
		const { stdout } = await execAsync("skycode version", {
...
/**
 * Detect if the current Skycode instance is running as a CLI subagent.
 * CLI subagents are identified by specific parameter patterns set by the transformSkycodeCommand function.
 *     TODO - For now we are relying on the maxConsecutiveMistakes value, which will only ever be "3"
 *     unless users pass in "-s max_consecutive_mistakes=6" via Skycode CLI. Would like better detection.
