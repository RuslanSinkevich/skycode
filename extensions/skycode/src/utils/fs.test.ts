...
describe("Filesystem Utilities", () => {
	const tmpDir = path.join(os.tmpdir(), "skycode-test-" + Math.random().toString(36).slice(2))
	...
	it("should exclude .skycoderules/workflows directory specifically", async () => {
		// Create a test directory structure
		const skycoderulesDirTest = path.join(tmpDir, "skycoderules-test")
		const skycoderulesDirPath = path.join(skycoderulesDirTest, ".skycoderules")

		// Create .skycoderules directory and root files
		await fs.mkdir(skycoderulesDirPath, { recursive: true })
		await fs.writeFile(path.join(skycoderulesDirPath, "config.json"), "{}")
		await fs.writeFile(path.join(skycoderulesDirPath, "settings.js"), "// settings")

		// Create .skycoderules/other directory and files
		const otherDirPath = path.join(skycoderulesDirPath, "other")
		...
		// Create .skycoderules/workflows directory and files
		const workflowsDirPath = path.join(skycoderulesDirPath, "workflows")
		...
		const allFiles = await readDirectory(skycoderulesDirPath)
		...
		const filteredFiles = await readDirectory(skycoderulesDirPath, [[".skycoderules", "workflows"]])

		// Verify workflows files are excluded but others remain
		filteredFiles.length.should.equal(4) // 2 in root + 2 in other

		const expectedFiles = [
			path.resolve(skycoderulesDirPath, "config.json"),
			path.resolve(skycoderulesDirPath, "settings.js"),
		...
		const multiExcludeFiles = await readDirectory(skycoderulesDirPath, [
		...
		const rootOnlyFiles = [path.resolve(skycoderulesDirPath, "config.json"), path.resolve(skycoderulesDirPath, "settings.js")]

		multiExcludeFiles.sort().should.deepEqual(rootOnlyFiles.sort())
	})

	it("should exclude .skycoderules/hooks directory specifically", async () => {
		// Create a test directory structure
		const skycoderulesDirTest = path.join(tmpDir, "skycoderules-hooks-test")
		const skycoderulesDirPath = path.join(skycoderulesDirTest, ".skycoderules")
		...
		// Create .skycoderules/hooks directory and files
		const hooksDirPath = path.join(skycoderulesDirPath, "hooks")
		...
		const filteredFiles = await readDirectory(skycoderulesDirPath, [[".skycoderules", "hooks"]])
