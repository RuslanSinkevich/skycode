 ...
 * - SKYCODE_OTEL_TELEMETRY_ENABLED: "1" to enable OpenTelemetry (default: off)
 * - SKYCODE_OTEL_METRICS_EXPORTER: Comma-separated list: "console", "otlp", "prometheus"
 * - SKYCODE_OTEL_LOGS_EXPORTER: Comma-separated list: "console", "otlp"
 * - SKYCODE_OTEL_EXPORTER_OTLP_PROTOCOL: "grpc", "http/json", or "http/protobuf"
 * - SKYCODE_OTEL_EXPORTER_OTLP_ENDPOINT: OTLP collector endpoint (if not using specific endpoints)
 * - SKYCODE_OTEL_EXPORTER_OTLP_METRICS_PROTOCOL: Metrics-specific protocol override
 * - SKYCODE_OTEL_EXPORTER_OTLP_METRICS_ENDPOINT: Metrics-specific endpoint override
 * - SKYCODE_OTEL_EXPORTER_OTLP_LOGS_PROTOCOL: Logs-specific protocol override
 * - SKYCODE_OTEL_EXPORTER_OTLP_LOGS_ENDPOINT: Logs-specific endpoint override
 * - SKYCODE_OTEL_METRIC_EXPORT_INTERVAL: Milliseconds between metric exports (default: 60000)
 * - SKYCODE_OTEL_EXPORTER_OTLP_INSECURE: "true" to disable TLS for gRPC (for local development)
 * - SKYCODE_OTEL_LOG_BATCH_SIZE: Maximum batch size for log records (default: 512)
 * - SKYCODE_OTEL_LOG_BATCH_TIMEOUT: Maximum time to wait before exporting logs in ms (default: 5000)
 * - SKYCODE_OTEL_LOG_MAX_QUEUE_SIZE: Maximum queue size for log records (default: 2048)
 *
 * @private
...
function getRuntimeOtelConfig(): OpenTelemetryClientConfig {
	return {
		enabled: process.env.SKYCODE_OTEL_TELEMETRY_ENABLED === "true",
		metricsExporter: process.env.SKYCODE_OTEL_METRICS_EXPORTER,
		logsExporter: process.env.SKYCODE_OTEL_LOGS_EXPORTER,
		otlpProtocol: process.env.SKYCODE_OTEL_EXPORTER_OTLP_PROTOCOL,
		otlpEndpoint: process.env.SKYCODE_OTEL_EXPORTER_OTLP_ENDPOINT,
		otlpMetricsProtocol: process.env.SKYCODE_OTEL_EXPORTER_OTLP_METRICS_PROTOCOL,
		otlpMetricsEndpoint: process.env.SKYCODE_OTEL_EXPORTER_OTLP_METRICS_ENDPOINT,
		otlpLogsProtocol: process.env.SKYCODE_OTEL_EXPORTER_OTLP_LOGS_PROTOCOL,
		otlpLogsEndpoint: process.env.SKYCODE_OTEL_EXPORTER_OTLP_LOGS_ENDPOINT,
		metricExportInterval: process.env.SKYCODE_OTEL_METRIC_EXPORT_INTERVAL
			? parseInt(process.env.SKYCODE_OTEL_METRIC_EXPORT_INTERVAL, 10)
			: undefined,
		otlpInsecure: process.env.SKYCODE_OTEL_EXPORTER_OTLP_INSECURE === "true",
		logBatchSize: process.env.SKYCODE_OTEL_LOG_BATCH_SIZE
			? Math.max(1, parseInt(process.env.SKYCODE_OTEL_LOG_BATCH_SIZE, 10))
			: undefined,
		logBatchTimeout: process.env.SKYCODE_OTEL_LOG_BATCH_TIMEOUT
			? Math.max(1, parseInt(process.env.SKYCODE_OTEL_LOG_BATCH_TIMEOUT, 10))
			: undefined,
		logMaxQueueSize: process.env.SKYCODE_OTEL_LOG_MAX_QUEUE_SIZE
			? Math.max(1, parseInt(process.env.SKYCODE_OTEL_LOG_MAX_QUEUE_SIZE, 10))
			: undefined,
	}
}
