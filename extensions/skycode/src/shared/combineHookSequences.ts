import { SkycodeMessage } from "./ExtensionMessage"

/**
 * Hook metadata extracted from hook message text.
 * Mirrors the SkycodeSayHook interface but represents parsed data.
 */
...
function getSay(msg: SkycodeMessage): string | undefined {
 ...
 */
function isToolOrCommandMessage(msg: SkycodeMessage): boolean {
 ...
 */
function parseHookMetadata(hookMessage: SkycodeMessage): HookMetadata | null {
 ...
 */
function filterPartialToolMessages(messages: SkycodeMessage[]): SkycodeMessage[] {
...
function combineHookWithOutputs(
	hookMessage: SkycodeMessage,
	startIndex: number,
	messages: SkycodeMessage[],
): { combined: SkycodeMessage; nextIndex: number } {
 ...
 */
function combineAllHooks(messages: SkycodeMessage[]): SkycodeMessage[] {
	// Pass 1: Build map of combined hooks by timestamp
	const combinedHooksByTs = new Map<number, SkycodeMessage>()
 ...
 */
function findImmediateNextToolTimestamp(hookIndex: number, messages: SkycodeMessage[]): number | null {
 ...
 */
function buildPreToolUseMap(processedMessages: SkycodeMessage[], originalMessages: SkycodeMessage[]): Map<number, SkycodeMessage[]> {
	const map = new Map<number, SkycodeMessage[]>()
 ...
 */
function reorderWithPreToolUseHooks(messages: SkycodeMessage[], preToolUseMap: Map<number, SkycodeMessage[]>): SkycodeMessage[] {
	const result: SkycodeMessage[] = []
 ...
 *
 * @param messages Array of SkycodeMessage objects to process
 * @returns New array with hooks combined and PreToolUse hooks reordered
 */
export function combineHookSequences(messages: SkycodeMessage[]): SkycodeMessage[] {
