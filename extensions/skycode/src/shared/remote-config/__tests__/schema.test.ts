...
describe("Remote Config Schema", () => {
	...
	describe("SkycodeSettingsSchema", () => {
		it("should accept valid Skycode provider settings", () => {
			const validSettings = {
				models: [{ id: "claude-3-5-sonnet-20241022" }, { id: "claude-3-5-haiku-20241022" }],
			}
			const result = SkycodeSettingsSchema.parse(validSettings)
			expect(result).to.deep.equal(validSettings)
		})

		it("should accept empty settings object", () => {
			const result = SkycodeSettingsSchema.parse({})
			expect(result.models).to.be.undefined
		})
		...
		it("should accept models with only id field", () => {
			...
			expect(() => SkycodeSettingsSchema.parse(settings)).to.not.throw()
		})

		it("should reject models with missing id field", () => {
			expect(() =>
				SkycodeSettingsSchema.parse({
					models: [{}],
				}),
			).to.throw()
		})
	...
	describe("RemoteConfigSchema", () => {
		...
		it("should handle complete config with all fields", () => {
			...
			// Verify Skycode settings
			expect(result.providerSettings?.Skycode?.models).to.have.lengthOf(2)
			expect(result.providerSettings?.Skycode?.models?.[0].id).to.equal("claude-3-5-sonnet-20241022")
			expect(result.providerSettings?.Skycode?.models?.[1].id).to.equal("claude-3-5-haiku-20241022")
