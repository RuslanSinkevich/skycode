...
/**
 * S3/R2 blob storage implementation of SkycodeStorage.
 * Uses AWS S3 or Cloudflare R2 as the backend storage.
 */
export class SkycodeBlobStorage extends SkycodeStorage {
	override name = "SkycodeBlobStorage"

	private static store: SkycodeBlobStorage | null = null
	static get instance(): SkycodeBlobStorage {
		if (!SkycodeBlobStorage.store) {
			SkycodeBlobStorage.store = new SkycodeBlobStorage()
		}
		return SkycodeBlobStorage.store
	}
	...
	public init(settings?: BlobStoreSettings) {
		...
		try {
			if (!SkycodeBlobStorage.isConfigured(settings)) {
			...
			if (adapter) {
				...
				Logger.log("[SkycodeBlobStorage] Adapter created")
			}
		} catch (error) {
			// Log but don't throw - allow startup to continue
			Logger.error("[SkycodeBlobStorage] initialization failed:", error)
		}
	}
	...
	protected async _get(key: string): Promise<string | undefined> {
		...
		try {
			...
			Logger.error(`[SkycodeBlobStorage] failed to get '${key}':`, error)
			return undefined
		}
	}
	...
	protected async _store(key: string, value: string): Promise<void> {
		...
		try {
			...
			Logger.error(`[SkycodeBlobStorage] failed to store '${key}':`, error)
			throw error
		}
	}
	...
	protected async _delete(key: string): Promise<void> {
		...
		try {
			...
			Logger.error(`[SkycodeBlobStorage] failed to delete '${key}':`, error)
			throw error
		}
	}
}
 ...
 */
export const blobStorage = SkycodeBlobStorage.instance
