...
describe("Hook Executor", () => {
	...
	let mockMessages: SkycodeMessage[]
...
`
	...
	beforeEach(async () => {
		...
		// Create .skycoderules/hooks subdirectory structure
		tempDir = path.join(baseTempDir, ".skycoderules", "hooks")
		await fs.mkdir(tempDir, { recursive: true })
		testHandler = createTestHandler()
		mockMessages = []

		// Mock StateManager to return baseTempDir as workspace root
		// This allows HookFactory to find hooks in baseTempDir/.skycoderules/hooks/
	...
	describe("Error Handling", () => {
		...
		it("should update message state on hook failure", async function () {
			...
			const mockHandler = {
				...testHandler,
				getSkycodeMessages: () => messages,
				addToSkycodeMessages: async (msg: SkycodeMessage) => {
					messages.push(msg)
				},
				updateSkycodeMessage: async (index: number, updates: Partial<SkycodeMessage>) => {
					if (messages[index]) {
