...
e2e("Views - can set up API keys and navigate to Settings from Chat", async ({ sidebar }) => {
	...
	await expect(sidebar.getByRole("button", { name: "Login to Skycode" })).toBeVisible()
	...
	// Test Skycode provider option
	await providerSelectorInput.click({ delay: 100 })
	// Wait for dropdown to appear and find Skycode option
	await expect(sidebar.getByTestId("provider-option-skycode")).toBeVisible()
	await sidebar.getByTestId("provider-option-skycode").click({ delay: 100 })
	await expect(sidebar.getByRole("button", { name: "Sign Up with Skycode" })).toBeVisible()
	...
	await expect(sidebar.getByRole("button", { name: "Login to Skycode" })).not.toBeVisible()
	...
	// skycode logo container
	const skycodeLogo = sidebar.locator(".size-20")
	await expect(skycodeLogo).toBeVisible()
