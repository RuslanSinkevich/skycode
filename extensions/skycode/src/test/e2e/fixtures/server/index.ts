...
import type { BalanceResponse, OrganizationBalanceResponse, UserResponse } from "../../../../shared/SkycodeAccount"
import { E2E_MOCK_API_RESPONSES, E2E_REGISTERED_MOCK_ENDPOINTS } from "./api"
import { SkycodeDataMock } from "./data"

const E2E_API_SERVER_PORT = 7777

export const MOCK_SKYCODE_API_SERVER_URL = `http://localhost:${E2E_API_SERVER_PORT}`

const useVerboseLogging = process.env.SKYCODE_E2E_TESTS_VERBOSE === "true"
function log(...args: unknown[]) {
	if (useVerboseLogging) {
		console.log("[SkycodeApiServerMock]", ...args)
	}
}

export class SkycodeApiServerMock {
	static globalSharedServer: SkycodeApiServerMock | null = null
	...
	public readonly API_USER = new SkycodeDataMock("personal")
	...
	public static async startGlobalServer(): Promise<SkycodeApiServerMock> {
		log("=== SERVER FIXTURE CALLED ===")
		if (SkycodeApiServerMock.globalSharedServer) {
			log("Using existing global server")
			return SkycodeApiServerMock.globalSharedServer
		}
		...
		const server = createServer((req: IncomingMessage, res: ServerResponse) => {
			...
			if (isAuthRequired && authToken) {
				...
				const user = SkycodeApiServerMock.globalSharedServer!.API_USER.getUserByToken(authToken)
				if (!user) {
					return sendApiError("Invalid token", 401)
				}
				SkycodeApiServerMock.globalSharedServer!.setCurrentUser(user)
			}
			...
			const handleRequest = async () => {
				...
				const routeMatch = SkycodeApiServerMock.matchRoute(path, method)
				...
				const controller = SkycodeApiServerMock.globalSharedServer!
				...
				if (baseRoute === "/api/v1") {
					...
					if (endpoint === "/auth/token" && method === "POST") {
						...
						// Return format matching SkycodeAuthProvider expectations
						return sendApiResponse({
					...
					if (endpoint === "/chat/completions" && method === "POST") {
						...
						if (body.includes("[diff.test.ts] Hello, Skycode!")) {
								...
								choices: [
									{
										...
										message: {
											role: "assistant",
											content: "Hello! I'm a mock Skycode API response.",
										},
										finish_reason: "stop",
									},
								],
		...
		const controller = new SkycodeApiServerMock(server)
		SkycodeApiServerMock.globalSharedServer = controller

		// Track connections for proper cleanup
		server.on("connection", (socket) => {
			SkycodeApiServerMock.globalSockets.add(socket)
			socket.on("close", () => {
				SkycodeApiServerMock.globalSockets.delete(socket)
			})
		})
		...
		await new Promise<void>((resolve, reject) => {
			server.listen(E2E_API_SERVER_PORT, (error?: Error) => {
				if (error) {
					...
					log(`SkycodeApiServerMock listening on port ${E2E_API_SERVER_PORT}`)
					resolve()
				}
			})
		})
	...
	public static async stopGlobalServer(): Promise<void> {
		...
		SkycodeApiServerMock.globalSharedServer = null
	}
}
