...
import { SkycodeApiServerMock } from "../fixtures/server"
...
export class E2ETestHelper {
	...
	public async signin(webview: Frame): Promise<void> {
		await webview.getByRole("button", { name: "Login to Skycode" }).click({ delay: 100 })

		// Verify start up page is no longer visible
		await expect(webview.getByRole("button", { name: "Login to Skycode" })).not.toBeVisible()

		await webview.getByRole("button", { name: "Close" }).click({ delay: 50 })
	}

	public static async openSkycodeSidebar(page: Page): Promise<void> {
		await page.getByRole("tab", { name: /Skycode/ }).locator("a").click()
	}
...
/**
 * NOTE: Use the `e2e` test fixture for all E2E tests to test the Skycode extension.
 *
 * Extended Playwright test configuration for Skycode E2E testing.
 *
 * This test configuration provides a comprehensive setup for end-to-end testing of the Skycode VS Code extension,
 ...
 *
 * Fixtures provided:
 * - `server`: Shared SkycodeApiServerMock instance for API mocking (reused across all tests)
 ...
 * - `page`: Playwright Page object representing the main VS Code window with Skycode sidebar opened
 * - `sidebar`: Playwright Frame object representing the Skycode extension's sidebar iframe
 *
 * @returns Extended test object with all fixtures available for E2E test scenarios:
 * - **server**: Automatically starts and manages a SkycodeApiServerMock instance
 ...
 * - **page**: Configures the main VS Code window with notifications disabled and Skycode sidebar open
 * - **sidebar**: Provides access to the Skycode extension's sidebar frame
 *
 * @example
 * ```typescript
 ...
 * - Installs the Skycode extension in development mode
...
export const e2e = test
	.extend<{ server: SkycodeApiServerMock | null }>({
		server: async ({}, use) => {
			// Start server if it doesn't exist
			if (!SkycodeApiServerMock.globalSharedServer) {
				await SkycodeApiServerMock.startGlobalServer()
			}
			await use(SkycodeApiServerMock.globalSharedServer)
		},
	})
	...
	.extend<{ sidebar: Frame }>({
		sidebar: async ({ page, helper, server }, use) => {
			await E2ETestHelper.openSkycodeSidebar(page)
