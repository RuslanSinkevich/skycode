 ...
 * to prevent race conditions, particularly the TOCTOU bug in addToSkycodeMessages
 */
describe("MessageStateHandler Mutex Protection", () => {
	...
	/**
	 * Helper to create a test SkycodeMessage
	 */
	function createTestMessage(text: string): SkycodeMessage {
		return {
			ts: Date.now(),
			type: "say",
			say: "text",
	...
	it("should initialize with empty message arrays", () => {
		...
		handler.getSkycodeMessages().length.should.equal(0)
	...
	it("should set and get skycode messages", () => {
		const handler = createTestHandler()
		const testMessages = [createTestMessage("test1"), createTestMessage("test2")]

		handler.setSkycodeMessages(testMessages)
		handler.getSkycodeMessages().should.deepEqual(testMessages)
	})

	/**
	 * CRITICAL TEST: Verify that addToSkycodeMessages is atomic
	 * This test simulates the race condition that can occur when multiple
	 * addToSkycodeMessages calls happen concurrently without proper mutex protection
	 */
	it("should handle concurrent addToSkycodeMessages atomically", async function () {
		...
		await handler.addToSkycodeMessages(initialMsg)

		// Verify initial state
		const messages = handler.getSkycodeMessages()
		...
		const results = await Promise.all([
			handler.addToSkycodeMessages(msg1),
			handler.addToSkycodeMessages(msg2),
			handler.addToSkycodeMessages(msg3),
		])
		...
		// Get final state
		const finalMessages = handler.getSkycodeMessages()
	...
	/**
	 * Test that updateSkycodeMessage operations are atomic
	 */
	it("should handle concurrent updateSkycodeMessage atomically", async function () {
		this.timeout(5000)
		...
		handler.setSkycodeMessages(msgs)

		// Perform concurrent updates to different messages
		await Promise.all([
			handler.updateSkycodeMessage(0, { text: "updated1" }),
			handler.updateSkycodeMessage(1, { text: "updated2" }),
			handler.updateSkycodeMessage(2, { text: "updated3" }),
		])
	...
	/**
	 * Test that deleteSkycodeMessage operations are atomic
	 */
	it("should handle deleteSkycodeMessage with proper validation", async () => {
		const handler = createTestHandler()
		...
		await handler.deleteSkycodeMessage(1)
	 ...
	 */
	it("should throw error for invalid message index in updateSkycodeMessage", async () => {
		const handler = createTestHandler()
		handler.setSkycodeMessages([createTestMessage("msg1")])

		try {
			await handler.updateSkycodeMessage(5, { text: "invalid" })
	...
	/**
	 * Test that invalid indices are rejected in deleteSkycodeMessage
	 */
	it("should throw error for invalid message index in deleteSkycodeMessage", async () => {
		const handler = createTestHandler()
		handler.setSkycodeMessages([createTestMessage("msg1")])
	 ...
	 */
	it("should handle overwriteSkycodeMessages atomically", async () => {
		const handler = createTestHandler()

		// Set initial messages
		handler.setSkycodeMessages([createTestMessage("old1"), createTestMessage("old2")])

		// Overwrite with new messages
		const newMessages = [createTestMessage("new1"), createTestMessage("new2"), createTestMessage("new3")]
		await handler.overwriteSkycodeMessages(newMessages)
