...
describe("Skycode Extension", () => {
	...
	it("should verify extension ID matches package.json", async () => {
		...
		const skycodeExtensionApi = vscode.extensions.getExtension(id)

		skycodeExtensionApi?.id.should.equal(id)
	})

	it("should successfully execute the plus button command", async () => {
		await new Promise((resolve) => setTimeout(resolve, 400))
		await vscode.commands.executeCommand("skycode.plusButtonClicked")
	})
