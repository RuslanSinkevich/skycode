...
import { SkycodeAsk } from "@/shared/ExtensionMessage"
...
export class ToolResultUtils {
	...
	static pushToolResult(
		...
		if (typeof content === "string") {
			...
			// Get tool_use_id from map using call_id, or use "skycode" as fallback for backward compatibility
			const toolUseId = toolUseIdMap?.get(block.call_id || "") || "skycode"
			...
			// If using backward-compatible "skycode" ID and content is an array, spread it directly
			// instead of wrapping it (which would cause JSON.stringify in createToolResultBlock)
			if ((toolUseId === "skycode" || !toolUseId) && Array.isArray(content)) {
	...
	private static createToolResultBlock(content: ToolResponse, id?: string, call_id?: string) {
		// If id is "skycode", we treat it as a plain text result for backward compatibility
	 ...
	 */
	static async askApprovalAndPushFeedback(type: SkycodeAsk, completeMessage: string, config: TaskConfig) {
