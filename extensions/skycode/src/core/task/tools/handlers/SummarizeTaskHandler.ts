...
export class SummarizeTaskHandler implements IToolHandler, IPartialBlockHandler {
	readonly name = SkycodeDefaultTool.SUMMARIZE_TASK
		...
		try {
			...
			if (hooksEnabled) {
				try {
					...
					const result = await executePreCompactHookWithCleanup({
						...
						skycodeMessages: config.messageState.getSkycodeMessages(),
			...
			if (match) {
				...
				for (const relPath of filePaths) {
					...
					// Check .skycodeignore first and skip ignored files
					...
					if (await config.callbacks.shouldAutoApproveToolWithPath(SkycodeDefaultTool.FILE_READ, relPath)) {
						try {
			...
			const telemetryData = config.services.contextManager.getContextTelemetryData(
				config.messageState.getSkycodeMessages(),
				config.api,
