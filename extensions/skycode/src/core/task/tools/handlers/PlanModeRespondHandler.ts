...
import { SkycodePlanModeResponse } from "@/shared/ExtensionMessage"
...
export class PlanModeRespondHandler implements IToolHandler, IPartialBlockHandler {
	readonly name = SkycodeDefaultTool.PLAN_MODE
	...
	async handlePartialBlock(block: ToolUse, uiHelpers: StronglyTypedUIHelpers): Promise<void> {
		...
		} satisfies SkycodePlanModeResponse
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		if (config.yoloModeToggled && (config.mode === "act" || config.mode === "debug")) {
		...
		if ((config.mode === "plan" || config.mode === "ask" || config.mode === "chat") && config.yoloModeToggled && !needsMoreExploration) {
			...
			if (switchSuccessful) {
				...
				const lastPlanMessage = findLast(config.messageState.getSkycodeMessages(), (m: any) => m.ask === this.name)
				if (lastPlanMessage) {
