...
import type { SkycodeSayGenerateExplanation } from "@/shared/ExtensionMessage"
...
/**
 * Helper to create a stringified SkycodeSayGenerateExplanation message
 */
function createExplanationMessage(
	title: string,
	fromRef: string,
	toRef: string,
	status: SkycodeSayGenerateExplanation["status"],
	error?: string,
): string {
	const message: SkycodeSayGenerateExplanation = { title, fromRef, toRef, status }
	if (error) {
		message.error = error
	}
...
export class GenerateExplanationToolHandler implements IToolHandler, IPartialBlockHandler {
	readonly name = SkycodeDefaultTool.GENERATE_EXPLANATION
