...
export const PatchSkycodeSayMap = {
...
export class ApplyPatchHandler implements IFullyManagedTool {
	readonly name = SkycodeDefaultTool.APPLY_PATCH
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		try {
			...
			for (const message of messages) {
				...
				// [SKYCODE] For DELETE operations, check deleteFiles auto-approval setting
				if (change.type === PatchActionType.DELETE) {
					const autoApproveResult = config.autoApprover
						? config.autoApprover.shouldAutoApproveTool(SkycodeDefaultTool.FILE_DELETE)
						: false
					...
					if (!didAutoApprove) {
						...
						showNotificationForApproval(
							`Skycode wants to delete: ${getReadablePath(config.cwd, operationPath)}`,
			...
			for (const changedFilePath of changedFiles) {
				...
				config.services.fileContextTracker.markFileAsEditedBySkycode(pathToTrack)
				await config.services.fileContextTracker.trackFileContext(pathToTrack, "skycode_edited")
			}
	...
	private async generateChangeSummary(changes: Record<string, FileChange>): Promise<SkycodeSayTool[]> {
	...
	private async handleApproval(config: TaskConfig, block: ToolUse, message: SkycodeSayTool, rawInput: string): Promise<boolean> {
		...
		// [SKYCODE-SKYCODE] Cursor-style: always auto-approve patches (no ask)
