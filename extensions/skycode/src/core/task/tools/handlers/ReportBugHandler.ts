...
export class ReportBugHandler implements IToolHandler, IPartialBlockHandler {
	readonly name = SkycodeDefaultTool.REPORT_BUG
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		if (config.autoApprovalSettings.enableNotifications) {
			showSystemNotification({
				subtitle: "Skycode wants to create a github issue...",
				message: `Skycode is suggesting to create a github issue with the title: ${title}`,
			})
		}
		...
		const skycodeVersion = ExtensionRegistryInfo.version
		...
		const bugReportData = JSON.stringify({
			...
			skycode_version: skycodeVersion,
		})
		...
		if (text || (images && images.length > 0) || (reportBugFiles && reportBugFiles.length > 0)) {
			...
			try {
				...
				params.set("skycode-version", skycodeVersion)
				...
				await createAndOpenGitHubIssue("skycode", "skycode", "bug_report.yml", params)
			} catch (error) {
