...
export class EditNotebookToolHandler implements IFullyManagedTool {
	readonly name = SkycodeDefaultTool.EDIT_NOTEBOOK
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		// Check skycodeignore
		const accessValidation = this.validator.checkSkycodeIgnorePath(rawPath)
		if (!accessValidation.ok) {
			await config.callbacks.say("skycodeignore_error", rawPath)
			return formatResponse.toolError(formatResponse.skycodeIgnoreError(rawPath))
		}
		...
		try {
			...
			// [SKYCODE] Check editNotebooks auto-approval setting
			const autoApproveResult = config.autoApprover
				? config.autoApprover.shouldAutoApproveTool(SkycodeDefaultTool.EDIT_NOTEBOOK)
				: false
			const didAutoApprove = autoApproveResult === true || (Array.isArray(autoApproveResult) && autoApproveResult[0])

			const toolMessage: SkycodeSayTool = {
			...
			if (didAutoApprove) {
				...
				showNotificationForApproval(
					`Skycode wants to edit notebook: ${readablePath}`,
