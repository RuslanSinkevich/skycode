...
export class ToolValidator {
	constructor(private readonly skycodeIgnoreController: SkycodeIgnoreController) {}
	...
	/**
	 * Verifies access is allowed to a given path via .skycodeignore rules.
	 * Callers should pass a repo-relative (workspace-relative) path.
	 */
	checkSkycodeIgnorePath(relPath: string): ValidationResult {
		const accessAllowed = this.skycodeIgnoreController.validateAccess(relPath)
		if (!accessAllowed) {
			return {
				ok: false,
				error: `Access to path '${relPath}' is blocked by .skycodeignore settings.`,
			}
		}
		return { ok: true }
	}
