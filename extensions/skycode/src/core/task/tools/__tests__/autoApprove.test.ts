...
describe("AutoApprove.shouldAutoApproveTool", () => {
	...
	describe("YOLO mode", () => {
		it("should return [true, true] for FILE_DELETE in YOLO mode", () => {
			...
			const result = aa.shouldAutoApproveTool(SkycodeDefaultTool.FILE_DELETE)
			result.should.deepEqual([true, true])
		})

		it("should return [true, true] for EDIT_NOTEBOOK in YOLO mode", () => {
			const aa = createAutoApprove({ yoloMode: true })
			const result = aa.shouldAutoApproveTool(SkycodeDefaultTool.EDIT_NOTEBOOK)
			result.should.deepEqual([true, true])
		})

		it("should return [true, true] for BASH in YOLO mode", () => {
			const aa = createAutoApprove({ yoloMode: true })
			const result = aa.shouldAutoApproveTool(SkycodeDefaultTool.BASH)
			result.should.deepEqual([true, true])
		})

		it("should return true for BROWSER in YOLO mode", () => {
			const aa = createAutoApprove({ yoloMode: true })
			const result = aa.shouldAutoApproveTool(SkycodeDefaultTool.BROWSER)
			result.should.equal(true)
		})

		it("should return true for MCP_USE in YOLO mode", () => {
			const aa = createAutoApprove({ yoloMode: true })
			const result = aa.shouldAutoApproveTool(SkycodeDefaultTool.MCP_USE)
			result.should.equal(true)
		})

		it("should return [true, true] for FILE_READ in YOLO mode", () => {
			const aa = createAutoApprove({ yoloMode: true })
			const result = aa.shouldAutoApproveTool(SkycodeDefaultTool.FILE_READ)
			result.should.deepEqual([true, true])
		})
	})
	...
	describe("FILE_DELETE", () => {
		...
		it("should return false when deleteFiles is undefined", () => {
			...
			const result = aa2.shouldAutoApproveTool(SkycodeDefaultTool.FILE_DELETE)
			result.should.equal(false)
		})
	...
	describe("EDIT_NOTEBOOK", () => {
		...
		it("should NOT be grouped with FILE_EDIT anymore", () => {
			...
			const notebookResult = aa.shouldAutoApproveTool(SkycodeDefaultTool.EDIT_NOTEBOOK)
			notebookResult.should.equal(false)

			// FILE_EDIT should be true (separate setting)
			const editResult = aa.shouldAutoApproveTool(SkycodeDefaultTool.FILE_EDIT)
	...
	describe("Other tools", () => {
		...
		it("should return tuple for APPLY_PATCH", () => {
			const aa = createAutoApprove({})
			const result = aa.shouldAutoApproveTool(SkycodeDefaultTool.APPLY_PATCH)
			Array.isArray(result).should.be.true()
		})
	...
	describe("Default settings behavior", () => {
		...
		it("useBrowser defaults to true", () => {
			const aa = createAutoApprove({})
			aa.shouldAutoApproveTool(SkycodeDefaultTool.BROWSER).should.equal(true)
		})

		it("useMcp defaults to true", () => {
			const aa = createAutoApprove({})
			aa.shouldAutoApproveTool(SkycodeDefaultTool.MCP_USE).should.equal(true)
		})
	})
})
