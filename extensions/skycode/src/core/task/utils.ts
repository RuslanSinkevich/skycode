...
import { SkycodeApiReqCancelReason, SkycodeApiReqInfo } from "@/shared/ExtensionMessage"
...
export const updateApiReqMsg = async (params: UpdateApiReqMsgParams) => {
	const skycodeMessages = params.messageStateHandler.getSkycodeMessages()
	const currentApiReqInfo: SkycodeApiReqInfo = JSON.parse(skycodeMessages[params.lastApiReqIndex].text || "{}")
	delete currentApiReqInfo.retryStatus // Clear retry status when request is finalized

	await params.messageStateHandler.updateSkycodeMessage(params.lastApiReqIndex, {
		text: JSON.stringify({
