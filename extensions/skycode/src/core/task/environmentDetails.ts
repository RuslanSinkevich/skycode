...
export async function buildEnvironmentDetails(
	...
	const allowedVisibleFiles = ctx.skycodeIgnoreController
	...
	const allowedOpenTabs = ctx.skycodeIgnoreController
	...
	if (includeFileDetails) {
		...
		if (isDesktop) {
			...
			const result = formatResponse.formatFilesList(ctx.cwd, files, didHitLimit, ctx.skycodeIgnoreController)
			details += result
		}
