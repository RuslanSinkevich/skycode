...
import { SKYCODE_MCP_TOOL_IDENTIFIER } from "@/shared/mcp"
import {
...
export interface PendingReasoning {
	...
	content: string
	signature: string
	redactedThinking: SkycodeAssistantRedactedThinkingBlock[]
...
/**
 * Handles streaming native tool use blocks and converts them to SkycodeAssistantToolUseBlock format
 */
class ToolUseHandler {
	...
	getFinalizedToolUse(id: string): SkycodeAssistantToolUseBlock | undefined {
	...
	getAllFinalizedToolUses(summary?: SkycodeAssistantToolUseBlock["reasoning_details"]): SkycodeAssistantToolUseBlock[] {
		const results: SkycodeAssistantToolUseBlock[] = []
	...
	getPartialToolUsesAsContent(): ToolUse[] {
		...
		for (const pending of pendingToolUses) {
			...
			if (pending.name.includes(SKYCODE_MCP_TOOL_IDENTIFIER)) {
				const [key, toolName] = pending.name.split(SKYCODE_MCP_TOOL_IDENTIFIER)
				results.push({
					type: "tool_use",
					name: SkycodeDefaultTool.MCP_USE,
					params: {
				...
				results.push({
					type: "tool_use",
					name: pending.name as SkycodeDefaultTool,
...
class ReasoningHandler {
	...
	getCurrentReasoning(): SkycodeAssistantThinkingBlock | null {
		if (!this.pendingReasoning) {
			return null
		}
	...
	getRedactedThinking(): SkycodeAssistantRedactedThinkingBlock[] {
