...
import {
	...
	getSavedSkycodeMessages,
} from "@core/storage/disk"
import { releaseTaskLock } from "@core/task/TaskLockUtils"
import { isMultiRootEnabled } from "@core/workspace/multi-root-utils"
import { WorkspaceRootManager } from "@core/workspace/WorkspaceRootManager"
// [SKYCODE] TEMPORARILY DISABLED — legacy Cline shadow-git checkpoint imports
...
export class Task {
	...
	private skycodeIgnoreController: SkycodeIgnoreController
	...
	constructor(params: TaskParams) {
		...
		this.skycodeIgnoreController = new SkycodeIgnoreController(cwd)
		...
		const commandExecutorCallbacks: CommandExecutorCallbacks = {
			...
			ask: async (type: string, text?: string, partial?: boolean) => {
				const result = await this.ask(type as SkycodeAsk, text, partial)
				return {
			...
			updateSkycodeMessage: async (index: number, updates: { commandCompleted?: boolean; text?: string }) => {
				await this.messageStateHandler.updateSkycodeMessage(index, updates)
			},
			getSkycodeMessages: () => this.messageStateHandler.getSkycodeMessages() as Array<{ ask?: string; say?: string }>,
			addToUserMessageContent: (content: { type: string; text: string }) => {
				// Cast to SkycodeTextContentBlock which is compatible with SkycodeContent
				this.taskState.userMessageContent.push({ type: "text", text: content.text } as SkycodeTextContentBlock)
			},
		}
	...
	async ask(
		...
		if (partial !== undefined) {
			const skycodeMessages = this.messageStateHandler.getSkycodeMessages()
			const lastMessage = skycodeMessages.at(-1)
			const lastMessageIndex = skycodeMessages.length - 1
	...
	async handleWebviewAskResponse(
		askResponse: SkycodeAskResponse,
		text?: string,
		images?: string[],
		files?: string[],
		approvalId?: string,
	...
	async say(
		...
		if (partial !== undefined) {
			const lastMessage = this.messageStateHandler.getSkycodeMessages().at(-1)
			const isUpdatingPreviousPartial =
	...
	async sayAndCreateMissingParamError(toolName: SkycodeDefaultTool, paramName: string, relPath?: string) {
		await this.say(
			"error",
			`Skycode tried to use ${toolName}${
	...
	async removeLastPartialMessageIfExistsWithType(type: "ask" | "say", askOrSay: SkycodeAsk | SkycodeSay) {
		...
		if (lastMessage?.partial && lastMessage.type === type && (lastMessage.ask === askOrSay || lastMessage.say === askOrSay)) {
			this.messageStateHandler.setSkycodeMessages(skycodeMessages.slice(0, -1))
			await this.messageStateHandler.saveSkycodeMessagesAndUpdateHistory()
		}
	}
	 ...
	 */
	private calculatePreCompactDeletedRange(apiConversationHistory: SkycodeStorageMessage[]): [number, number] {
	...
	public async startTask(task?: string, images?: string[], files?: string[]): Promise<void> {
		try {
			await this.skycodeIgnoreController.initialize()
		} catch (error) {
			Logger.error("Failed to initialize SkycodeIgnoreController:", error)
			// Optionally, inform the user or handle the error appropriately
		}
		// conversationHistory (for API) and skycodeMessages (for webview) need to be in sync
		// if the extension process were killed, then on restart the skycodeMessages might not be empty, so we need to set it to [] when we create a new Skycode client (otherwise webview would show stale messages from previous session)
		this.messageStateHandler.setSkycodeMessages([])
		...
		const imageBlocks: SkycodeImageContentBlock[] = formatResponse.imageBlocks(images)

		const userContent: SkycodeUserContent[] = [
			{
				type: "text",
				text: `<task>\n${task}\n</task>`,
			},
	...
	public async resumeTaskFromHistory() {
		...
		const savedSkycodeMessages = await getSavedSkycodeMessages(this.taskId)

		// Remove any resume messages that may have been added before

		const lastRelevantMessageIndex = findLastIndex(
			savedSkycodeMessages,
			(m) => !(m.ask === "resume_task" || m.ask === "resume_completed_task"),
		)
		if (lastRelevantMessageIndex !== -1) {
			savedSkycodeMessages.splice(lastRelevantMessageIndex + 1)
		}

		// since we don't use api_req_finished anymore, we need to check if the last api_req_started has a cost value, if it doesn't and no cancellation reason to present, then we remove it since it indicates an api request without any partial content streamed
		const lastApiReqStartedIndex = findLastIndex(savedSkycodeMessages, (m) => m.type === "say" && m.say === "api_req_started")
		if (lastApiReqStartedIndex !== -1) {
			const lastApiReqStarted = savedSkycodeMessages[lastApiReqStartedIndex]
			const { cost, cancelReason }: SkycodeApiReqInfo = JSON.parse(lastApiReqStarted.text || "{}")
			if (cost === undefined && cancelReason === undefined) {
				savedSkycodeMessages.splice(lastApiReqStartedIndex, 1)
			}
		}

		await this.messageStateHandler.overwriteSkycodeMessages(savedSkycodeMessages)
		this.messageStateHandler.setSkycodeMessages(await getSavedSkycodeMessages(this.taskId))

		// Now present the skycode messages to the user and ask if they want to resume (NOTE: we ran into a bug before where the apiconversationhistory wouldn't be initialized when opening a old task, and it was because we were waiting for resume)
		...
		const lastSkycodeMessage = this.messageStateHandler
			.getSkycodeMessages()
			.slice()
			.reverse()
			.find((m) => !(m.ask === "resume_task" || m.ask === "resume_completed_task")) // could be multiple resume tasks

		let askType: SkycodeAsk
		if (lastSkycodeMessage?.ask === "completion_result") {
		...
		const newUserContent: SkycodeContent[] = []
			...
			const taskResumeResult = await executeHook({
				...
				hookInput: {
					taskResume: {
						...
						previousState: {
							lastMessageTs: lastSkycodeMessage?.ts?.toString() || "",
							messageCount: skycodeMessages.length.toString(),
		...
		if (response === "messageResponse" || text || (images && images.length > 0) || (files && files.length > 0)) {
			...
			// [SKYCODE] TEMPORARILY DISABLED — legacy Cline shadow-git checkpoint
		...
		// need to make sure that the api conversation history can be resumed by the api, even if it goes out of sync with skycode messages
		...
		let modifiedOldUserContent: SkycodeContent[] // either the last message if its user message, or the user message before the last (assistant) message
		let modifiedApiConversationHistory: SkycodeStorageMessage[] // need to remove the last user message to replace with new modified user message
		if (existingApiConversationHistory.length > 0) {
			...
			if (lastMessage.role === "assistant") {
				...
				const existingUserContent: SkycodeContent[] = Array.isArray(lastMessage.content)
					? lastMessage.content
		...
		const agoText = (() => {
			const timestamp = lastSkycodeMessage?.ts ?? Date.now()
			const now = Date.now()
		...
		const wasRecent = lastSkycodeMessage?.ts && Date.now() - lastSkycodeMessage.ts < 30_000
	...
	private async initiateTaskLoop(userContent: SkycodeContent[]): Promise<void> {
		...
		while (!this.taskState.abort) {
			...
			const didEndLoop = await this.recursivelyMakeSkycodeRequests(nextUserContent, includeFileDetails)
			includeFileDetails = false // we only need file details the first time

			//  The way this agentic loop works is that skycode will be given a task that he then calls tools to complete. unless there's an attempt_completion call, we keep responding back to him with his tool's responses until he either attempt_completion or does not use anymore tools. If he does not use anymore tools, we ask him to consider if he's completed the task and then call attempt_completion, otherwise proceed with completing the task.
			...
			if (didEndLoop) {
				...
				// this.say(
				// 	"tool",
				// 	"Skycode responded with only text blocks but has not called attempt_completion yet. Forcing him to continue with task..."
				// )
				nextUserContent = [
					{
						type: "text",
	...
	async abortTask() {
		try {
			...
			this.skycodeIgnoreController.dispose()
	...
	public async cancelHookExecution(): Promise<boolean> {
		...
		try {
			...
			const skycodeMessages = this.messageStateHandler.getSkycodeMessages()
			const hookMessageIndex = skycodeMessages.findIndex((m) => m.ts === messageTs)
			if (hookMessageIndex !== -1) {
				...
				await this.messageStateHandler.updateSkycodeMessage(hookMessageIndex, {
		...
		if (hooksEnabled) {
			try {
				...
				await executePreCompactHookWithCleanup({
					...
					skycodeMessages: this.messageStateHandler.getSkycodeMessages(),
					messageStateHandler: this.messageStateHandler,
	...
	async *attemptApiRequest(previousApiReqIndex: number): ApiStream {
		...
		// skycode browser tool uses image recognition for navigation (requires model image support).
		...
		const preferredLanguageInstructions = preferredLanguage
			? `# Preferred Language\n\nSpeak in ${preferredLanguage}.${
					alwaysThinkInPreferredLanguage
						? `\n\nCRITICAL: You MUST also THINK in ${preferredLanguage} inside the <thinking> tags. This is a STRICT requirement.`
						: ""
				}`
			: ""
		...
		if (subagentsEnabled) {
			const skycodeCliInstalled = await isSkycodeCliInstalled()
			isSubagentsEnabledAndCliInstalled = subagentsEnabled && skycodeCliInstalled
		}

		const { globalToggles, localToggles } = await refreshSkycodeRulesToggles(this.controller, this.cwd)
		...
		const skycodeIgnoreContent = this.skycodeIgnoreController.skycodeIgnoreContent
		let skycodeIgnoreInstructions: string | undefined
		if (skycodeIgnoreContent) {
			skycodeIgnoreInstructions = formatResponse.skycodeIgnoreInstructions(skycodeIgnoreContent)
		}
		...
		const promptContext: SystemPromptContext = {
			...
			globalSkycodeRulesFileInstructions,
			localSkycodeRulesFileInstructions,
			localCursorRulesFileInstructions,
			localCursorRulesDirInstructions,
			localWindsurfRulesFileInstructions,
			localAgentsRulesFileInstructions,
			skycodeIgnoreInstructions,
			preferredLanguageInstructions,
			preferredLanguage,
			alwaysThinkInPreferredLanguage,
			browserSettings: this.stateManager.getGlobalSettingsKey("browserSettings"),
			yoloModeToggled: this.stateManager.getGlobalSettingsKey("yoloModeToggled"),
			skycodeWebToolsEnabled: this.stateManager.getGlobalSettingsKey("skycodeWebToolsEnabled"),
			...
			enableNativeToolCalls:
				...
				providerInfo.providerId === "gigachat" ||
		...
		try {
			...
			const skycodeError = ErrorService.get().toSkycodeError(error, model.id, providerId)

			// Capture provider failure telemetry using skycodeError
			ErrorService.get().logMessage(skycodeError.message)
			...
			if (isContextWindowExceededError && !this.taskState.didAutomaticallyRetryFailedApiRequest) {
				...
				if (isContextWindowExceededError) {
					...
					if (truncatedConversationHistory.length > 3) {
						skycodeError.message = "Context window exceeded. Click retry to truncate the conversation and try again."
						this.taskState.didAutomaticallyRetryFailedApiRequest = false
					}
				}

				const streamingFailedMessage = skycodeError.serialize()
				...
				const isAuthError = skycodeError.isErrorType(SkycodeErrorType.Auth)

				// Check if this is a Skycode provider insufficient credits error - don't auto-retry these
				const isSkycodeProviderInsufficientCredits = (() => {
					if (providerId !== "skycode") {
						return false
					}
					try {
						const parsedError = SkycodeError.transform(error, model.id, providerId)
						return parsedError.isErrorType(SkycodeErrorType.Balance)
					} catch {
						return false
					}
				})()

				let response: SkycodeAskResponse
				// Skip auto-retry for Skycode provider insufficient credits or auth errors
				if (!isSkycodeProviderInsufficientCredits && !isAuthError && this.taskState.autoRetryAttempts < 3) {
					...
					if (!isSkycodeProviderInsufficientCredits && !isAuthError) {
						await this.say(
							"error_retry",
							JSON.stringify({
								attempt: 3,
				...
				if (manualRetryApiReqIndex !== -1) {
					const skycodeMessages = this.messageStateHandler.getSkycodeMessages()
					const currentApiReqInfo: SkycodeApiReqInfo = JSON.parse(skycodeMessages[manualRetryApiReqIndex].text || "{}")
					delete currentApiReqInfo.streamingFailedMessage
					await this.messageStateHandler.updateSkycodeMessage(manualRetryApiReqIndex, {
						text: JSON.stringify(currentApiReqInfo),
					})
				}
	...
	async recursivelyMakeSkycodeRequests(userContent: SkycodeContent[], includeFileDetails: boolean = false): Promise<boolean> {
		...
		if (this.taskState.consecutiveMistakeCount >= this.stateManager.getGlobalSettingsKey("maxConsecutiveMistakes")) {
			...
			if (autoApprovalSettings.enableNotifications) {
				showSystemNotification({
					subtitle: "Error",
					message: "Skycode is having trouble. Would you like to continue the task?",
				})
			}
			...
			if (response === "messageResponse") {
				...
				const feedbackUserContent: SkycodeUserContent[] = []
		...
		if (shouldCompact) {
			...
			;[parsedUserContent, environmentDetails, skycoderulesError] = await this.loadContext(
				userContent,
				includeFileDetails,
				useCompactPrompt,
			)
		...
		// error handling if the user uses the /newrule command & their .skycoderules is a file, for file read operations didnt work properly
		if (skycoderulesError === true) {
			await this.say(
				"error",
				"Issue with processing the /newrule command. Double check that, if '.skycoderules' already exists, it's a directory and not a file. Otherwise there was an issue referencing this file/directory.",
			)
		}
		...
		const lastApiReqIndex = findLastIndex(this.messageStateHandler.getSkycodeMessages(), (m) => m.say === "api_req_started")
		await this.messageStateHandler.updateSkycodeMessage(lastApiReqIndex, {
			text: JSON.stringify({
				request: userContent.map((block) => formatContentBlockToMarkdown(block)).join("\n\n"),
			} satisfies SkycodeApiReqInfo),
		})
		...
		try {
			...
			const abortStream = async (cancelReason: SkycodeApiReqCancelReason, streamingFailedMessage?: string) => {
			...
			try {
				for await (const chunk of stream) {
					switch (chunk.type) {
						...
						case "text": {
							...
							void this.presentAssistantMessage().catch((error) => {
								// Abort during stream teardown is expected; avoid unhandled rejection noise.
								if (error instanceof Error && error.message === "Skycode instance aborted") {
									return
								}
								Logger.debug("[Task] Failed to present message (async text stream): " + error)
							})
							break
						}
					}
					...
					if (this.taskState.abort) {
						...
						if (!this.taskState.abandoned) {
							// only need to gracefully abort if this instance isn't abandoned (sometimes openrouter stream hangs, in which case this would affect future instances of skycode)
				...
				// abandoned happens when extension is no longer waiting for the skycode instance to finish aborting (error is thrown here when any function in the for loop throws due to this.abort)
				if (!this.taskState.abandoned) {
					const skycodeError = ErrorService.get().toSkycodeError(error, this.api.getModel().id)
					const errorMessage = skycodeError.serialize()
			...
			// OpenRouter/Skycode may not return token usage as part of the stream (since it may abort early), so we fetch after the stream is finished
			...
			if (assistantHasContent) {
				...
				const assistantContent: Array<SkycodeAssistantContent> = [
				...
				if (hasAssistantText) {
					assistantContent.push({
						...
						// reasoning_details only exists for skycode/openrouter providers
			...
			if (assistantHasContent) {
				...
				if (this.taskState.pendingUserMessage) {
					...
					nextUserContent = [{ type: "text", text: `<feedback>\n${pending.text}\n</feedback>` }]
					this.taskState.consecutiveMistakeCount = 0
				}
				...
				const baseErrorMessage =
					"Invalid API Response: The provider returned an empty or unparsable response. This is a provider-side issue where the model failed to generate valid output or returned tool calls that Skycode cannot process. Retrying the request may help resolve this issue."
	...
	async loadContext(
	...
	): Promise<[SkycodeContent[], string, boolean]> {
		let needsSkycoderulesFileCheck = false
		...
		const parseTextBlock = async (text: string): Promise<string> => {
			...
			const { processedText, needsSkycoderulesFileCheck: needsCheck } = await parseSlashCommands(
				parsedText,
			...
			if (needsCheck) {
				needsSkycoderulesFileCheck = true
			}

			return processedText
		}

		const processTextContent = async (block: SkycodeTextContentBlock): Promise<SkycodeTextContentBlock> => {
		...
		const processContentBlock = async (block: SkycodeContent): Promise<SkycodeContent> => {
			if (block.type === "text") {
		...
		// Check skycoderulesData if needed
		const skycoderulesError = needsSkycoderulesFileCheck
			? await ensureLocalSkycodeDirExists(this.cwd, GlobalFileNames.skycodeRules)
			: false
		...
		return [processedUserContent, environmentDetails, skycoderulesError]
	}
