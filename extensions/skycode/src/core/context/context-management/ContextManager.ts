...
import { SkycodeApiReqInfo, SkycodeMessage } from "@shared/ExtensionMessage"
...
export class ContextManager {
	...
	shouldCompactContextWindow(
		...
		if (previousApiReqIndex >= 0) {
			const previousRequest = skycodeMessages[previousApiReqIndex]
			if (previousRequest && previousRequest.text) {
				const { tokensIn, tokensOut, cacheWrites, cacheReads }: SkycodeApiReqInfo = JSON.parse(previousRequest.text)
	...
	getContextTelemetryData(
		...
		if (triggerIndex !== undefined) {
			...
			const apiReqIndices = skycodeMessages
		...
		if (targetIndex >= 0) {
			const targetRequest = skycodeMessages[targetIndex]
			if (targetRequest && targetRequest.text) {
				try {
					const { tokensIn, tokensOut, cacheWrites, cacheReads }: SkycodeApiReqInfo = JSON.parse(targetRequest.text)
	...
	public getNextTruncationRange(
		...
		// NOTE: anthropic format messages are always user-assistant-user-assistant, while openai format messages can have multiple user messages in a row (we use anthropic format throughout skycode)
