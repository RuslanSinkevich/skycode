...
describe("Skills Utility Functions", () => {
	...
	const GLOBAL_SKILLS_DIR = path.join("/home", "user", ".skycode", "skills")
...
Instructions here`)
		...
		it("should discover skills from project .skycoderules/skills directory", async () => {
			const projectSkillsDir = path.join(TEST_CWD, ".skycoderules", "skills")
...
Use analogies and ASCII diagrams when explaining code.`)
		...
		it("should discover skills from project .skycode/skills directory", async () => {
			const skycodeSkillsDir = path.join(TEST_CWD, ".skycode", "skills")
			const skillDir = path.join(skycodeSkillsDir, "debugging")
			const skillMdPath = path.join(skillDir, "SKILL.md")

			fileExistsStub.withArgs(skycodeSkillsDir).resolves(true)
			fileExistsStub.withArgs(skillMdPath).resolves(true)
			isDirectoryStub.withArgs(skycodeSkillsDir).resolves(true)
			readdirStub.withArgs(skycodeSkillsDir).resolves(["debugging"])
			statStub.withArgs(skillDir).resolves({ isDirectory: () => true })
			readFileStub.withArgs(skillMdPath, "utf-8").resolves(`---
name: debugging
