...
type MessageStateHandlerLike = {
	getSkycodeMessages(): SkycodeMessageLike[]
}
...
/**
 * Builds the evaluation context used for conditional Skycode Rules (e.g. YAML frontmatter `paths:`).
 *
 * Kept in the user-instructions domain so Task remains orchestration-focused.
 *
 * Path context is gathered from multiple sources in skycodeMessages:
...
export class RuleContextBuilder {
	...
	private static async getRulePathContext(deps: RuleContextBuilderDeps): Promise<string[]> {
		...
		const skycodeMessages = deps.messageStateHandler.getSkycodeMessages()
		...
		const lastUserMsg = [...skycodeMessages]
			.reverse()
