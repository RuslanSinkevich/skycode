...
export async function synchronizeRuleToggles(
	...
	currentToggles: SkycodeRulesToggles,
	allowedFileExtension: string = "",
	excludedPaths: string[][] = [],
): Promise<SkycodeRulesToggles> {
...
export function synchronizeRemoteRuleToggles(
	...
	const updatedToggles: SkycodeRulesToggles = {}
 ...
 */
export function combineRuleToggles(toggles1: SkycodeRulesToggles, toggles2: SkycodeRulesToggles): SkycodeRulesToggles {
 ...
 */
export const getRuleFilesTotalContent = async (rulesFilePaths: string[], basePath: string, toggles: SkycodeRulesToggles) => {
 ...
 * Used by getGlobalSkycodeRules and getLocalSkycodeRules.
 */
...
export function getRemoteRulesTotalContentWithMetadata(
	remoteRules: GlobalInstructionsFile[],
	remoteToggles: SkycodeRulesToggles,
...
/**
 * Handles converting any directory into a file (specifically used for .skycoderules and .skycoderules/workflows)
 * The old .skycoderules file or .skycoderules/workflows file will be renamed to a default filename
 * Doesn't do anything if the dir already exists or doesn't exist
 * Returns whether there are any uncaught errors
 */
export async function ensureLocalSkycodeDirExists(skycoderulePath: string, defaultRuleFilename: string): Promise<boolean> {
	try {
		const exists = await fileExistsAtPath(skycoderulePath)

		if (exists && !(await isDirectory(skycoderulePath))) {
			// logic to convert .skycoderules file into directory, and rename the rules file to {defaultRuleFilename}
			const content = await fs.readFile(skycoderulePath, "utf8")
			const tempPath = skycoderulePath + ".bak"
			await fs.rename(skycoderulePath, tempPath) // create backup
			try {
				await fs.mkdir(skycoderulePath, { recursive: true })
				await fs.writeFile(path.join(skycoderulePath, defaultRuleFilename), content, "utf8")
				...
				try {
					await fs.rm(skycoderulePath, { recursive: true, force: true }).catch(() => {})
					await fs.rename(tempPath, skycoderulePath) // restore backup
				} catch (_restoreError) {}
...
export const createRuleFile = async (isGlobal: boolean, filename: string, cwd: string, type: string) => {
	try {
		...
		if (isGlobal) {
			if (type === "workflow") {
				const globalSkycodeWorkflowFilePath = await ensureWorkflowsDirectoryExists()
				filePath = path.join(globalSkycodeWorkflowFilePath, filename)
			} else {
				const globalSkycodeRulesFilePath = await ensureRulesDirectoryExists()
				filePath = path.join(globalSkycodeRulesFilePath, filename)
			}
		} else {
			const localSkycodeRulesFilePath = path.resolve(cwd, GlobalFileNames.skycodeRules)

			const hasError = await ensureLocalSkycodeDirExists(localSkycodeRulesFilePath, "default-rules.md")
			if (hasError === true) {
				return { filePath: null, fileExists: false }
			}

			await fs.mkdir(localSkycodeRulesFilePath, { recursive: true })

			if (type === "workflow") {
				const localWorkflowsFilePath = path.resolve(cwd, GlobalFileNames.workflows)

				const hasError = await ensureLocalSkycodeDirExists(localWorkflowsFilePath, "default-workflows.md")
				if (hasError === true) {
					return { filePath: null, fileExists: false }
				}
				...
				// skycoderules file creation
				filePath = path.join(localSkycodeRulesFilePath, filename)
			}
		}
