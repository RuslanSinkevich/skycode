...
/**
 * Discover all skills from global (~/.skycode/skills) and project directories.
...
export async function discoverSkills(cwd: string): Promise<SkillMetadata[]> {
	...
	const projectDirs = [
		path.join(cwd, GlobalFileNames.skycoderuleSkillsDir),
		path.join(cwd, GlobalFileNames.skycodeSkillsDir),
	...
	// Load global skills last (~/.skycode/skills) - higher priority
