...
// If a user modifies a file outside of Skycode, the context may become stale and need to be updated.
// We do not want Skycode to reload the context every time a file is modified, so we use this class merely
// to inform Skycode that the change has occurred, and tell Skycode to reload the file before making
// any changes to it. This fixes an issue with diff editing, where Skycode was unable to complete a diff edit.
// a diff edit because the file was modified since Skycode last read it.

// FileContextTracker
/**
This class is responsible for tracking file operations.
If the full contents of a file are passed to Skycode via a tool, mention, or edit, the file is marked as active.
If a file is modified outside of Skycode, we detect and track this change to prevent stale context.
...
export class FileContextTracker {
	...
	private recentlyEditedBySkycode = new Set<string>()
	...
	async setupFileWatcher(filePath: string) {
		...
		// Track file changes
		watcher.on("change", () => {
			if (this.recentlyEditedBySkycode.has(filePath)) {
				this.recentlyEditedBySkycode.delete(filePath) // This was an edit by Skycode, no need to inform Skycode
			} else {
				this.recentlyModifiedFiles.add(filePath) // This was a user edit, we will inform Skycode
	 ...
	 * This is the main entry point for FileContextTracker and is called when a file is passed to Skycode via a tool, mention, or edit.
	 */
	async trackFileContext(filePath: string, operation: "read_tool" | "user_edited" | "skycode_edited" | "file_mentioned") {
		try {
			const cwd = await getCwd()
			if (!cwd) {
				Logger.info("No workspace folder available - cannot determine current working directory")
	...
	async addFileToFileContextTracker(taskId: string, filePath: string, source: FileMetadataEntry["record_source"]) {
		try {
			...
			const newEntry: FileMetadataEntry = {
				...
				skycode_read_date: getLatestDateForField(filePath, "skycode_read_date"),
				skycode_edit_date: getLatestDateForField(filePath, "skycode_edit_date"),
			...
			switch (source) {
				...
				// skycode_edited: Skycode has edited the file
				case "skycode_edited":
					newEntry.skycode_read_date = now
					newEntry.skycode_edit_date = now
					break

				// read_tool/file_mentioned: Skycode has read the file via a tool or file mention
				case "read_tool":
				case "file_mentioned":
					newEntry.skycode_read_date = now
					break
	...
	/**
	 * Marks a file as edited by Skycode to prevent false positives in file watchers
	 */
	markFileAsEditedBySkycode(filePath: string): void {
		this.recentlyEditedBySkycode.add(filePath)
	}
	...
	/**
	 * Detects files that were edited by Skycode or users after a specific message timestamp
	 * This is used when restoring checkpoints to warn about potential file content mismatches
	 */
	async detectFilesEditedAfterMessage(messageTs: number, deletedMessages: SkycodeMessage[]): Promise<string[]> {
		const editedFiles: string[] = []

		try {
			// Check task metadata for files that were edited by Skycode or users after the message timestamp
			...
			if (taskMetadata?.files_in_context) {
				for (const fileEntry of taskMetadata.files_in_context) {
					const skycodeEditedAfter = fileEntry.skycode_edit_date && fileEntry.skycode_edit_date > messageTs
					const userEditedAfter = fileEntry.user_edit_date && fileEntry.user_edit_date > messageTs

					if (skycodeEditedAfter || userEditedAfter) {
