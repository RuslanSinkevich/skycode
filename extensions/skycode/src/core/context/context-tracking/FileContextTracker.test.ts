...
describe("FileContextTracker", () => {
	...
	it("should add a record when a file is read by a tool", async () => {
		...
		expect(fileEntry.skycode_read_date).to.be.a("number")
		expect(fileEntry.skycode_edit_date).to.be.null
	})

	it("should add a record when a file is edited by Skycode", async () => {
		await tracker.trackFileContext(filePath, "skycode_edited")
		...
		expect(activeEntry.record_source).to.equal("skycode_edited")
		expect(activeEntry.skycode_read_date).to.be.a("number")
		expect(activeEntry.skycode_edit_date).to.be.a("number")
	})
	...
	it("should mark existing entries as stale when adding a new entry for the same file", async () => {
		...
		mockTaskMetadata.files_in_context = [
			{
				...
				skycode_read_date: Date.now() - 1000, // 1 second ago
				skycode_edit_date: null,
				user_edit_date: null,
			},
		]
		...
		expect(newEntry.record_source).to.equal("skycode_edited")
	})
	...
	it("should not track Skycode edits as user edits", async () => {
		// First track the file to set up the watcher
		await tracker.trackFileContext(filePath, "read_tool")

		// Mark the file as edited by Skycode
		tracker.markFileAsEditedBySkycode(filePath)
