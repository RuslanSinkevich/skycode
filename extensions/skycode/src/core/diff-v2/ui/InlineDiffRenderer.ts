...
export class InlineDiffRenderer implements vscode.Disposable {
  ...
  private async onHunkPositionChanged(hunkId: string, fsPath: string): Promise<void> {
  ...
  private removeGreenDecorations(hunkId: string): void {
    ...
    // Refresh: gather remaining ranges for same file and re-apply
    const editor = vscode.window.visibleTextEditors.find(
...
document.addEventListener('wheel',e=>{vscode.postMessage({command:'scroll',deltaY:e.deltaY,deltaX:e.deltaX});e.preventDefault()},{passive:false});
</script></body></html>`;
  }
