...
export async function parseSlashCommands(
...
): Promise<{ processedText: string; needsSkycoderulesFileCheck: boolean }> {
	...
	for (const { regex } of tagPatterns) {
		...
		if (tagMatch) {
			...
			if (SUPPORTED_DEFAULT_COMMANDS.includes(commandName)) {
				...
				return { processedText: processedText, needsSkycoderulesFileCheck: commandName === "newrule" }
			}
			...
			if (matchingWorkflow) {
				try {
					...
					return { processedText, needsSkycoderulesFileCheck: false }
				} catch (error) {
	...
	return { processedText: text, needsSkycoderulesFileCheck: false }
}
