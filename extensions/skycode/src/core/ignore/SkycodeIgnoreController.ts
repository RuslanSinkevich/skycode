 ...
 * Designed to be instantiated once in Skycode.ts and passed to file manipulation services.
 * Uses the 'ignore' library to support standard .gitignore syntax in .skycodeignore files.
 */
export class SkycodeIgnoreController {
	private cwd: string
	private ignoreInstance: Ignore
	private fileWatcher?: FSWatcher
	skycodeIgnoreContent: string | undefined

	constructor(cwd: string) {
		this.cwd = cwd
		this.ignoreInstance = ignore()
		this.skycodeIgnoreContent = undefined
	}
	...
	async initialize(): Promise<void> {
		// Set up file watcher for .skycodeignore
		this.setupFileWatcher()
		await this.loadSkycodeIgnore()
	}

	/**
	 * Set up the file watcher for .skycodeignore changes
	 */
	private setupFileWatcher(): void {
		const ignorePath = path.join(this.cwd, ".skycodeignore")
		...
		this.fileWatcher.on("error", (error) => {
			Logger.error("Error watching .skycodeignore file:", error)
		})
	}

	/**
	 * Load custom patterns from .skycodeignore if it exists.
	 * Supports "!include <filename>" to load additional ignore patterns from other files.
	 */
	private async loadSkycodeIgnore(): Promise<void> {
		try {
			...
			if (await fileExistsAtPath(ignorePath)) {
				...
				this.skycodeIgnoreContent = content
				await this.processIgnoreContent(content)
				this.ignoreInstance.add(".skycodeignore")
			} else {
				this.skycodeIgnoreContent = undefined
			}
		} catch (error) {
			// Should never happen: reading file failed even though it exists
			Logger.error("Unexpected error loading .skycodeignore:", error)
		}
	}
	...
	private async processIgnoreContent(content: string): Promise<void> {
		...
		const combinedContent = await this.processSkycodeIgnoreIncludes(content)
	 ...
	 */
	private async processSkycodeIgnoreIncludes(content: string): Promise<string> {
		let combinedContent = ""
	...
	private async readIncludedFile(includeLine: string): Promise<string | null> {
		...
		if (!(await fileExistsAtPath(resolvedIncludePath))) {
			Logger.debug(`[SkycodeIgnore] Included file not found: ${resolvedIncludePath}`)
			return null
		}
	...
	validateAccess(filePath: string): boolean {
		// Always allow access if .skycodeignore does not exist
		if (!this.skycodeIgnoreContent) {
			return true
		}
		try {
	...
	validateCommand(command: string): string | undefined {
		// Always allow if no .skycodeignore exists
		if (!this.skycodeIgnoreContent) {
			return undefined
		}
