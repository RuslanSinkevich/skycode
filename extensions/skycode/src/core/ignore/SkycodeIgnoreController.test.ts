...
import { SkycodeIgnoreController } from "./SkycodeIgnoreController"
import "should"

describe("SkycodeIgnoreController", () => {
	let tempDir: string
	let controller: SkycodeIgnoreController
	...
	beforeEach(async () => {
		...
		// Create default .skycodeignore file
		await fs.writeFile(
			path.join(tempDir, ".skycodeignore"),
			[".env", "*.secret", "private/", "# This is a comment", "", "temp.*", "file-with-space-at-end.* ", "**/.git/**"].join(
				"\n",
			),
		)

		controller = new SkycodeIgnoreController(tempDir)
	...
	describe("Default Patterns", () => {
		...
		it("should block access to .skycodeignore file", async () => {
			const result = controller.validateAccess(".skycodeignore")
	...
	describe("Custom Patterns", () => {
		...
		it("should handle comments in .skycodeignore", async () => {
			// Create a new .skycodeignore with comments
			await fs.writeFile(
				path.join(tempDir, ".skycodeignore"),
	...
	describe("Error Handling", () => {
		...
		it("should handle missing .skycodeignore gracefully", async () => {
			// Create a new controller in a directory without .skycodeignore
			const emptyDir = path.join(os.tmpdir(), `llm-test-empty-${Date.now()}`)
			await fs.mkdir(emptyDir)

			try {
				const controller = new SkycodeIgnoreController(emptyDir)
				await controller.initialize()
		...
		it("should handle empty .skycodeignore", async () => {
			await fs.writeFile(path.join(tempDir, ".skycodeignore"), "")
	...
	describe("Include Directive", () => {
		it("should load patterns from an included file", async () => {
			...
			// Create a .skycodeignore file that includes .gitignore and adds an extra pattern "secret.txt"
			await fs.writeFile(path.join(tempDir, ".skycodeignore"), ["!include .gitignore", "secret.txt"].join("\n"))

			// Initialize the controller to load the updated .skycodeignore
			controller = new SkycodeIgnoreController(tempDir)
			await controller.initialize()
			...
			// "secret.txt" should be ignored as specified directly in .skycodeignore
		...
		it("should handle non-existent included file gracefully", async () => {
			// Create a .skycodeignore file that includes a non-existent file
			await fs.writeFile(path.join(tempDir, ".skycodeignore"), ["!include missing-file.txt"].join("\n"))
		...
		it("should handle non-existent included file gracefully alongside a valid pattern", async () => {
			...
			await fs.writeFile(path.join(tempDir, ".skycodeignore"), ["!include non-existent.txt", "*.tmp"].join("\n"))
