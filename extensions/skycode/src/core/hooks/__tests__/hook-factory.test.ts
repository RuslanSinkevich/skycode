...
describe("Hook System", () => {
	...
	beforeEach(async () => {
		...
		// Create .skycoderules/hooks directory
		const hooksDir = path.join(tempDir, ".skycoderules", "hooks")
	...
	describe("StdioHookRunner", () => {
		it("should execute hook script and parse output", async () => {
			...
			const hookPath = path.join(tempDir, ".skycoderules", "hooks", "PreToolUse")
  ...
  contextModification: "Received tool: " + input.preToolUse.toolName
	...
	describe("PostToolUse Hook", () => {
		it("should receive execution results", async () => {
			const hookPath = path.join(tempDir, ".skycoderules", "hooks", "PostToolUse")
			const hookScript = `#!/usr/bin/env node
const input = JSON.parse(require('fs').readFileSync(0, 'utf-8'));
console.log(JSON.stringify({
  cancel: false,
...
console.log(JSON.stringify({ cancel: false }))`
	...
	describe("Error Handling", () => {
		...
		it("should handle hook input with all parameters", async () => {
			...
			const hookScript = `#!/usr/bin/env node
const input = JSON.parse(require('fs').readFileSync(0, 'utf-8'));
const hasAllFields = input.skycodeVersion && input.hookName && input.timestamp && 
...
console.log(JSON.stringify({
  ...
  contextModification: "GLOBAL: Context added"
			...
			const workspaceHookPath = path.join(tempDir, ".skycoderules", "hooks", "PreToolUse")
