...
/**
 * All valid hook types that can be created and executed by Skycode.
...
export async function resolveHooksDirectory(
	...
	if (isGlobal) {
		return globalHooksDirOverride || path.join(os.homedir(), "Documents", "Skycode", "Hooks")
	}
	...
	if (workspaceName) {
		...
		return path.join(targetWorkspace, ".skycoderules", "hooks")
	}

	// Single workspace: use getCwd
	const cwd = await getCwd(getDesktopDir())
	return path.join(cwd, ".skycoderules", "hooks")
}
