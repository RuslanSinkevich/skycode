...
import { version as skycodeVersion } from "../../../package.json"
...
} from "../../shared/proto/skycode/hooks"
 ...
 * provide--the other common parameters like skycodeVersion and userId are handled by the
 * hook system.
 */
...
export abstract class HookRunner<Name extends HookName> {
	 ...
	 * - skycodeVersion: Current Skycode extension version
	 ...
	 * - userId: Skycode user ID, machine ID, or generated UUID
	 *
	...
	protected async completeParams(params: NamedHookInput<Name>): Promise<HookInput> {
		...
		return {
			...
			workspaceRoots,
			userId: getDistinctId(), // Always available: Skycode User ID, machine ID, or generated UUID
			...params,
		}
	}
}
 ...
 *
 * Used in multi-root workspaces where both global hooks (from ~/Documents/Skycode/Hooks/)
 * and workspace-specific hooks (from each workspace's .skycoderules/hooks/) exist for the
 * same hook type.
 *
 * Behavior:
...
function isExpectedHookError(error: unknown): boolean {
	...
	// Note: This is expected because users may have hooks in .skycoderules that they don't want to execute
...
export class HookFactory {
	 ...
	 * Global hooks are located in paths containing "Skycode/Hooks" or "skycode/hooks".
	 */
	 ...
	 * Global hooks are located in ~/Documents/Skycode/Hooks/
	 * Workspace hooks are located in workspace .skycoderules/hooks/ directories
	 *
	 ...
	 * Includes both global hooks (from ~/Documents/Skycode/Hooks/) and workspace hooks
	 * (from .skycoderules/hooks/ in each workspace root).
	 */
	...
	/**
	 * Finds the path to a hook in a .skycoderules hooks directory.
	 *
	 * @param hookName the name of the hook to search for, for example 'PreToolUse'
	 * @param hooksDir the .skycoderules directory path to search
