...
import type { SkycodeStorageMessage } from "@shared/messages/content"
 ...
 */
export function extractTokenUsageFromMessage(message: SkycodeMessage | undefined): TokenUsage {
...
export async function writePreCompactContextFiles(
	taskId: string,
	currentContext: SkycodeStorageMessage[],
...
export interface PreCompactHookParams {
	...
	apiConversationHistory: SkycodeStorageMessage[]
	/** Current deleted range (if any) */
	conversationHistoryDeletedRange?: [number, number]
	/** Skycode messages for extracting token usage */
...
export async function executePreCompactHookWithCleanup(params: PreCompactHookParams): Promise<PreCompactHookResult> {
	...
	try {
		...
		const previousApiReqIndex = findLastIndex(params.skycodeMessages, (m) => m.say === "api_req_started")
		const previousRequest = previousApiReqIndex !== -1 ? params.skycodeMessages[previousApiReqIndex] : undefined
		...
		if (preCompactResult.cancel === true) {
			...
			await params.messageStateHandler.saveSkycodeMessagesAndUpdateHistory()
