...
import type { HookOutput } from "@shared/proto/skycode/hooks"
...
async function updateHookMessage(
	...
	const hookMessageIndex = skycodeMessages.findIndex((m: SkycodeMessage) => m.ts === hookMessageTs)
	if (hookMessageIndex !== -1) {
		await messageStateHandler.updateSkycodeMessage(hookMessageIndex, {
...
async function reorderHookAndToolMessages(messageStateHandler: MessageStateHandler): Promise<void> {
	...
	for (let i = skycodeMessages.length - 1; i >= 0; i--) {
		const msgType = skycodeMessages[i].ask || skycodeMessages[i].say
	...
	for (let i = lastToolMessageIndex + 1; i < skycodeMessages.length; i++) {
		if (skycodeMessages[i].say === "hook_status" || skycodeMessages[i].say === "hook_output_stream") {
	...
	const toolMessage = { ...skycodeMessages[lastToolMessageIndex] }

	// Delete the tool message at its current position
	await messageStateHandler.deleteSkycodeMessage(lastToolMessageIndex)

	// Re-add the tool message at the end (after hook messages)
	await messageStateHandler.addToSkycodeMessages(toolMessage)
}
