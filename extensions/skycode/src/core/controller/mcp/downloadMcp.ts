...
import { McpDownloadResponse } from "@shared/proto/skycode/mcp"
...
/**
 * Write server config directly to skycode_mcp_settings.json via McpHub.
 */
...
RULES:
- Server name in skycode_mcp_settings.json must be exactly: "${mcpDetails.mcpId}"
- Read the existing skycode_mcp_settings.json first, do NOT overwrite other servers.
 ...
 * 3. If config found: write directly to skycode_mcp_settings.json
	...
	try {
		...
		try {
			const response = await axios.post<McpDownloadResponse>(
				`${SkycodeEnv.config().mcpBaseUrl}/download`,
				{ mcpId },
				{
		...
		if (config) {
			...
			// Write config to skycode_mcp_settings.json
