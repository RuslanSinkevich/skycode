...
import { WorktreeDefaults } from "@shared/proto/skycode/worktree"
...
export async function getWorktreeDefaults(_controller: Controller, _request: EmptyRequest): Promise<WorktreeDefaults> {
	...
	const suggestedBranch = `worktree/skycode-${suffix}`

	// Generate suggested path in Documents/Skycode/Worktrees/<project-name>-<suffix>
	...
	const suggestedPath = path.join(documentsPath, "Skycode", "Worktrees", `${projectName}-${suffix}`)
