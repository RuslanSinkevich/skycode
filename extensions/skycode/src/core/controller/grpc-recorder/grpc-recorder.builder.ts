	...
	static getRecorder(controller: Controller): IRecorder {
		if (!GrpcRecorderBuilder.recorder) {
			GrpcRecorderBuilder.recorder = GrpcRecorder.builder()
				.enableIf(process.env.GRPC_RECORDER_ENABLED === "true" && process.env.SKYCODE_ENVIRONMENT === "local")
...
function testFilters(): GrpcRequestFilter[] {
	...
	return [
		...
		(req) => ["skycode.UiService", "skycode.McpService", "skycode.WebService"].includes(req.service),
		(req) =>
			[
