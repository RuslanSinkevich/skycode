...
export async function openFocusChainFile(controller: Controller, request: StringRequest): Promise<Empty> {
	...
	if (currentTask) {
		...
		// TODO - can we decouple this from SkycodeMessages?
		const skycodeMessages = currentTask.messageStateHandler.getSkycodeMessages()
		const lastProgressMessage = skycodeMessages
			.slice()
			.reverse()
