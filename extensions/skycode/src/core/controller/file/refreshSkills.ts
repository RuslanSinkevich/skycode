import { RefreshedSkills, SkillInfo } from "@shared/proto/skycode/file"
...
export async function refreshSkills(controller: Controller): Promise<RefreshedSkills> {
	...
	if (primaryWorkspace) {
		const localDirs = [
			path.join(primaryWorkspace, ".skycoderules", "skills"),
			path.join(primaryWorkspace, ".skycode", "skills"),
