import { CreateSkillRequest, SkillsToggles } from "@shared/proto/skycode/file"
...
export async function createSkillFile(controller: Controller, request: CreateSkillRequest): Promise<SkillsToggles> {
	...
	if (isGlobal) {
		...
		// Create in .skycode/skills by default
		const localSkillsDir = path.join(primaryWorkspace, ".skycode", "skills")
