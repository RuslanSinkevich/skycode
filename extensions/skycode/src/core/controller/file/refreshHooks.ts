import { HookInfo, HooksToggles, WorkspaceHooks } from "@shared/proto/skycode/file"
...
export async function refreshHooks(
	...
	const globalHooksDir = globalHooksDirOverride || path.join(os.homedir(), "Documents", "Skycode", "Hooks")
	...
	for (const workspacePath of workspacePaths.paths) {
		const workspaceHooksDir = path.join(workspacePath, ".skycoderules", "hooks")
