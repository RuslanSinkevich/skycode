import { DeleteAllTaskHistoryCount } from "@shared/proto/skycode/task"
...
export async function deleteAllTaskHistory(controller: Controller): Promise<DeleteAllTaskHistoryCount> {
	try {
		...
		const userChoice = (
			await HostProvider.window.showMessage(
				ShowMessageRequest.create({
					...
					message: t("history.deletePrompt"),
					options: {
						modal: true,
						items: [t("history.deleteExceptFavorites"), t("history.deleteAll")],
					},
				}),
			)
		).selectedOption
		...
		if (userChoice === t("history.deleteExceptFavorites")) {
			...
			if (favoritedTasks.length > 0) {
				...
				const answer = (
					await HostProvider.window.showMessage({
						type: ShowMessageType.WARNING,
						message: t("history.noFavorites"),
						options: {
							modal: true,
							items: [t("history.deleteAllTasks")],
						},
					})
				).selectedOption
		...
		try {
			...
			HostProvider.window.showMessage({
				...
				message: t("history.deleteError", { error: error instanceof Error ? error.message : String(error) }),
			})
		}
