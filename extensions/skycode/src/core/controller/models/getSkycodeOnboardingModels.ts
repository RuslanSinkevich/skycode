...
import { SKYCODE_ONBOARDING_MODELS } from "@/shared/skycode/onboarding"
import { OnboardingModel, OnboardingModelGroup } from "@/shared/proto/skycode/state"

type OnboardingModelOverride = OnboardingModel & { hidden?: boolean }

let cached: OnboardingModelGroup | null = null

export function getSkycodeOnboardingModels(): OnboardingModelGroup {
	if (cached) {
		return cached
	}

	const remoteOverrides = featureFlagsService.getOnboardingOverrides()
	const models = new Map<string, OnboardingModel>(SKYCODE_ONBOARDING_MODELS.map((model) => [model.id, model]))
