...
import { ApiFormat, OcaCompatibleModelInfo, OcaModelInfo } from "@shared/proto/skycode/models"
...
export async function refreshOcaModels(controller: Controller, request: StringRequest): Promise<OcaCompatibleModelInfo> {
	...
	if (!ocaAccessToken) {
		HostProvider.window.showMessage({
			...
			message: t("auth.oca.notAuthenticated"),
		})
	...
	try {
		...
		if (response.data?.data) {
			...
			HostProvider.window.showMessage({
				...
				message: t("auth.oca.refreshed", { url: baseUrl }),
			})
			...
			HostProvider.window.showMessage({
				type: ShowMessageType.ERROR,
				message: t("auth.oca.fetchFailed", { url: baseUrl }),
			})
		}
	} catch (err) {
		let userMsg
		...
		HostProvider.window.showMessage({
			type: ShowMessageType.ERROR,
			message: t("auth.oca.refreshError", { details: userMsg + ` opc-request-id: ${headers["opc-request-id"]}` }),
		})
