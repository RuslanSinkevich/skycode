...
export async function refreshOpenRouterModels(controller: Controller): Promise<Record<string, ModelInfo>> {
	...
	const finalModels = appendSkycodeStealthModels(models)
 ...
 */
const SKYCODE_STEALTH_MODELS: Record<string, ModelInfo> = {}

export function appendSkycodeStealthModels(currentModels: Record<string, ModelInfo>): Record<string, ModelInfo> {
	// Create a shallow clone of the current models to avoid mutating the original object
	const cloned = { ...currentModels }
	for (const [modelId, modelInfo] of Object.entries(SKYCODE_STEALTH_MODELS)) {
		if (!cloned[modelId]) {
