...
export class DifyHandler implements ApiHandler {
	...
	async *createMessage(systemPrompt: string, messages: SkycodeStorageMessage[]): ApiStream {
		...
		const requestBody = {
			...
			user: "skycode-user", // A unique user identifier
			files: [],
		}
	...
	private convertMessagesToQuery(systemPrompt: string, messages: SkycodeStorageMessage[]): string {
	 ...
	 * @param user User identifier (defaults to "skycode-user")
	 * @returns Promise with file upload response
	 */
	async uploadFile(file: Buffer, filename: string, user: string = "skycode-user"): Promise<DifyFileResponse> {
	 ...
	 */
	async stopGeneration(taskId: string, user: string = "skycode-user"): Promise<void> {
	 ...
	 */
	async deleteConversation(conversationId: string, user: string = "skycode-user"): Promise<void> {
