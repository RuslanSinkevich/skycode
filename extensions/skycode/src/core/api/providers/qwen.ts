...
export class QwenHandler implements ApiHandler {
	...
	async *createMessage(systemPrompt: string, messages: SkycodeStorageMessage[], tools?: OpenAITool[]): ApiStream {
		...
		// (local LM Studio / Ollama inference that doesn't extract think tags separately)
		...
		for await (const chunk of stream) {
			...
			// Cloud API (DashScope) with enable_thinking=true: reasoning comes in reasoning_content
			...
			if (delta?.content) {
				if (nativeReasoningReceived) {
					// Cloud API already handled thinking — content is clean answer text
