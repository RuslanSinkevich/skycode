...
interface SkycodeHandlerOptions extends CommonApiHandlerOptions {
	ulid?: string
	taskId?: string
	...
	skycodeAccountId?: string
	geminiThinkingLevel?: string
}

export class SkycodeHandler implements ApiHandler {
	private options: SkycodeHandlerOptions
	private skycodeAccountService = SkycodeAccountService.getInstance()
	...
	async *createMessage(systemPrompt: string, messages: SkycodeStorageMessage[], tools?: OpenAITool[]): ApiStream {
		try {
			...
			for await (const chunk of stream) {
				Logger.debug("SkycodeHandler chunk:" + JSON.stringify(chunk))
				// openrouter returns an error object instead of the openai sdk throwing an error
				if ("error" in chunk) {
					const error = chunk.error as OpenRouterErrorResponse["error"]
					Logger.error(`Skycode API Error: ${error?.code} - ${error?.message}`)
					// Include metadata in the error message if available
					const metadataStr = error.metadata ? `\nMetadata: ${JSON.stringify(error.metadata, null, 2)}` : ""
					throw new Error(`Skycode API Error ${error.code}: ${error.message}${metadataStr}`)
				}
				...
				if ((choice?.finish_reason as string) === "error") {
					...
					if (choiceWithError.error) {
						...
						Logger.error(`Skycode Mid-Stream Error: ${error.code || error.type || "Unknown"} - ${error.message}`)
						throw new Error(`Skycode Mid-Stream Error: ${error.code || error.type || "Unknown"} - ${error.message}`)
					} else {
						throw new Error(
							"Skycode Mid-Stream Error: Stream terminated with error status but no error details provided",
						)
					}
				}
				...
				// Guard: delta can be undefined when API sends chunks without choices (e.g. usage-only chunks)
				if (delta && "reasoning" in delta && delta.reasoning && !shouldSkipReasoningForModel(this.options.openRouterModelId)) {
					yield {
						type: "reasoning",
			...
			if (!didOutputUsage) {
				Logger.warn("Skycode API did not return usage chunk, fetching from generation endpoint")
			...
			Logger.error("Skycode API Error:", error)
			throw error
		}
	}
		...
		if (this.lastGenerationId) {
			try {
				...
				const headers: Record<string, string> = {
					...
					Authorization: `Bearer ${skycodeAccountAuthToken}`,
				}
				Object.assign(headers, await buildSkycodeExtraHeaders())

				const response = await axios.get(`${this.skycodeAccountService.baseUrl}/generation?id=${this.lastGenerationId}`, {
					headers,
				...
				// ignore if fails
				Logger.error("Error fetching skycode generation details:", error)
			}
		}
		return undefined
	}
