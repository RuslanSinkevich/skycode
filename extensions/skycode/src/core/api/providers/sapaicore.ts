...
namespace Bedrock {
	 ...
	 */
	export function formatMessagesForConverseAPI(messages: SkycodeStorageMessage[]): BedrockMessage[] {
...
namespace Gemini {
	...
	export function prepareRequestPayload(
		systemPrompt: string,
		messages: SkycodeStorageMessage[],
...
export class SapAiCoreHandler implements ApiHandler {
	...
	private async *createMessageWithOrchestration(systemPrompt: string, messages: SkycodeStorageMessage[]): ApiStream {
		try {
			...
			// sent directly to the LLM with the Skycode system prompt without validation errors.
	...
	private async *createMessageWithDeployments(systemPrompt: string, messages: SkycodeStorageMessage[]): ApiStream {
		const token = await this.getToken()
		const headers = {
			Authorization: `Bearer ${token}`,
			"AI-Resource-Group": this.options.sapAiResourceGroup || "default",
	...
	private convertMessageParamToSAPMessages(messages: SkycodeStorageMessage[]): ChatMessage[] {
