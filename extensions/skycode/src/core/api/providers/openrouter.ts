...
export class OpenRouterHandler implements ApiHandler {
		...
		if (!this.client) {
			...
			try {
				this.client = new OpenAI({
					...
					defaultHeaders: {
						"HTTP-Referer": "https://skycode-ai.local", // Optional, for including your app on openrouter.ai rankings.
						"X-Title": "Skycode", // Optional. Shows in rankings on openrouter.ai.
					},
	...
	async *createMessage(systemPrompt: string, messages: SkycodeStorageMessage[], tools?: OpenAITool[]): ApiStream {
		...
		for await (const chunk of stream) {
			...
			// Native reasoning field (OpenRouter extracts it for some models)
			...
			if (delta?.content) {
				if (nativeReasoningReceived) {
					// Reasoning already extracted by OpenRouter — content is clean
