...
export class AwsBedrockHandler implements ApiHandler {
	...
	private async getAwsCredentials(): Promise<{
		...
		const providerOptions: ProviderChainOptions = {
			clientConfig: {
				...
				userAgentAppId: `skycode#${ExtensionRegistryInfo.version}`,
			},
		}
		const useProfile =
	 ...
	 */
	private formatDeepseekR1Prompt(systemPrompt: string, messages: SkycodeStorageMessage[]): string {
	 ...
	 */
	private estimateInputTokens(systemPrompt: string, messages: SkycodeStorageMessage[]): number {
	 ...
	 */
	private formatMessagesForConverseAPI(messages: SkycodeStorageMessage[]): Message[] {
