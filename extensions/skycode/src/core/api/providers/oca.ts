...
import { ApiFormat } from "@/shared/proto/index.skycode"
...
export class OcaHandler implements ApiHandler {
	...
	async *createMessageChatApi(systemPrompt: string, messages: SkycodeStorageMessage[], tools?: OpenAITool[]): ApiStream {
		const client = this.ensureClient()
		...
		const chatCompletionsParams: OpenAI.Chat.Completions.ChatCompletionCreateParamsStreaming = {
			...
			...(this.options.taskId && {
				litellm_session_id: `skycode-${this.options.taskId}`,
