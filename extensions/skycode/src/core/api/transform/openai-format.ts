 ...
 *
 * @param toolId - The original tool ID from Skycode/Anthropic format
...
/**
 * Converts an array of SkycodeStorageMessage objects to OpenAI's Completions API format.
 *
 * Handles conversion of Skycode-specific content types (tool uses, tool results, images, reasoning details)
 * into OpenAI's expected message structure, including tool_calls and tool_call_id fields.
 *
 * @param anthropicMessages - Array of SkycodeStorageMessage objects to be converted
 * @returns Array of OpenAI.Chat.ChatCompletionMessageParam objects
 */
export function convertToOpenAiMessages(
	anthropicMessages: Omit<SkycodeStorageMessage, "modelInfo">[],
         ...
         */
			if (anthropicMessage.role === "user") {
				const { nonToolMessages, toolMessages } = anthropicMessage.content.reduce<{
					nonToolMessages: (SkycodeTextContentBlock | SkycodeImageContentBlock)[]
					toolMessages: SkycodeUserToolResultContentBlock[]
				}>(
					(acc, part) => {
				...
				const toolResultImages: SkycodeImageContentBlock[] = []
				...
				const { nonToolMessages, toolMessages } = anthropicMessage.content.reduce<{
					...
					toolMessages: SkycodeAssistantToolUseBlock[]
				}>(
					(acc, part) => {
...
const UNIQUE_ERROR_TOOL_NAME = "_skycode_error_unknown_function_"
