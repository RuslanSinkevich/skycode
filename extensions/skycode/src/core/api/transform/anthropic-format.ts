...
import { SkycodeStorageMessage, convertSkycodeStorageToAnthropicMessage } from "@/shared/messages/content"

/**
 * Converts Skycode storage messages to Anthropic API format with optional cache control.
 * Adds ephemeral cache control to the last two user messages to prevent them from being
 * stored in Anthropic's cache.
 *
 * @param skycodeMessages - Array of Skycode storage messages to convert
...
export function sanitizeAnthropicMessages(
	skycodeMessages: Array<SkycodeStorageMessage | Anthropic.MessageParam>,
	...
	const userMsgIndices = skycodeMessages.reduce((acc, msg, index) => {
		if (msg.role === "user") {
			acc.push(index)
		}
		return acc
	...
	return skycodeMessages.map((msg, index) => {
		const anthropicMsg = convertSkycodeStorageToAnthropicMessage(msg)
