...
- You have access to read-only tools: read_file, search_files, list_files, list_code_definition_names, codebase_search, and ask_followup_question. Use them to explore the codebase and answer the user's questions.
...
- You can use the list_code_definition_names tool to get an overview of source code definitions for all files at the top level of a specified directory.{{WEB_TOOLS_CAPABILITIES}}

IMPORTANT: Do NOT attempt to use execute_command, write_to_file, replace_in_file, browser_action, use_mcp_tool, or any other tool not listed above. They are not available in this mode.`

const getActModeCapabilitiesText = (context: SystemPromptContext) => `CAPABILITIES
...
- You can use the execute_command tool to run commands on the user's computer whenever you feel it can help accomplish the user's task. When you need to execute a CLI command, you must provide a clear explanation of what the command does. Prefer to execute complex CLI commands over creating executable scripts, since they are more flexible and easier to run. Prefer non-interactive commands when possible: use flags to disable pagers (e.g., '--no-pager'), auto-confirm prompts (e.g., '-y' when safe), provide input via flags/arguments rather than stdin, suppress interactive behavior, etc. For commands that may fail, consider redirecting stderr to stdout (e.g., \`command 2>&1\`) so you can see error messages in the output. For long-running commands, the user may keep them running in the background and you will be kept updated on their status along the way. Each command you execute is run in a new terminal instance.{{BROWSER_CAPABILITIES}}{{WEB_TOOLS_CAPABILITIES}}{{MCP_CAPABILITIES}}

<linting_workflow>
After editing files, use the read_diagnostics tool to check for errors you may have introduced. If there are new errors caused by your changes, fix them immediately. Do not leave broken code behind. This workflow is: edit → check diagnostics → fix if needed.
</linting_workflow>

<recommended_workflow>
When asked to make edits or improvements:
1. Analyze the file structure (environment_details) to understand the project layout.
2. Use search_files or list_code_definition_names to find relevant code.
3. Use read_file to examine the files you plan to modify.
4. Make changes using replace_in_file (preferred) or write_to_file.
5. Use read_diagnostics to verify no errors were introduced.
6. If your changes could affect other files, use search_files to find and update them.
</recommended_workflow>`

export async function getCapabilitiesSection(variant: PromptVariant, context: SystemPromptContext): Promise<string> {
	const isAskMode = context.mode === "ask" || context.mode === "chat"

	const template =
		variant.componentOverrides?.[SystemPromptSection.CAPABILITIES]?.template ||
		(isAskMode ? getAskModeCapabilitiesText : getActModeCapabilitiesText)
	...
	const webToolsCapabilities =
		context.skycodeWebToolsEnabled === true
