...
import { type SkycodeToolSpec, resolveInstruction } from "../spec"
...
export class PromptBuilder {
	...
	private static getEnabledTools(variant: PromptVariant, context: SystemPromptContext) {
		let resolvedTools: ReturnType<typeof SkycodeToolSet.getTools> = []
		...
		if (variant?.tools?.length) {
			...
			resolvedTools = SkycodeToolSet.getToolsForVariantWithFallback(variant.family, requestedIds)
			...
			resolvedTools = SkycodeToolSet.getTools(variant.family)
	...
	public static tool(config: SkycodeToolSpec, registry: SkycodeDefaultTool[], context: SystemPromptContext): string {
