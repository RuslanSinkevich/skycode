...
export interface SkycodeToolSpec {
	variant: ModelFamily
	id: SkycodeDefaultTool
	name: string
	description: string
	instruction?: string
	contextRequirements?: (context: SystemPromptContext) => boolean
	parameters?: Array<SkycodeToolSpecParameter>
}

interface SkycodeToolSpecParameter {
	name: string
	required: boolean
	instruction: string | ((context: SystemPromptContext) => string)
	usage?: string
	dependencies?: SkycodeDefaultTool[]
	description?: string
	contextRequirements?: (context: SystemPromptContext) => boolean
...
/**
 * Converts a SkycodeToolSpec into an OpenAI ChatCompletionTool definition
 * Docs: https://openrouter.ai/docs/features/tool-calling#step-1-inference-request-with-tools
 */
export function toolSpecFunctionDefinition(tool: SkycodeToolSpec, context: SystemPromptContext): OpenAITool {
...
/**
 * Converts a SkycodeToolSpec into an Anthropic Tool definition
 */
export function toolSpecInputSchema(tool: SkycodeToolSpec, context: SystemPromptContext): AnthropicTool {
	// Check if the tool should be included based on context requirements
	if (tool.contextRequirements && !tool.contextRequirements(context)) {
		throw new Error(`Tool ${tool.name} does not meet context requirements`)
	}
...
/**
 * Converts a SkycodeToolSpec into a Google Gemini function.
 * Docs: https://ai.google.dev/gemini-api/docs/function-calling
 */
export function toolSpecFunctionDeclarations(tool: SkycodeToolSpec, context: SystemPromptContext): GoogleTool {
	// Check if the tool should be included based on context requirements
	if (tool.contextRequirements && !tool.contextRequirements(context)) {
		throw new Error(`Tool ${tool.name} does not meet context requirements`)
	}
	...
	if (openAITool.type === "function") {
		...
		return {
			...
			input_schema: {
				...
				required: Array.isArray(func.parameters?.required) ? func.parameters.required : [],
			},
		}
	}
