...
const GPT5_1_AGENT_ROLE = (context: SystemPromptContext) => {
...
const GPT5_1_RULES = (context: SystemPromptContext) => {
	const thinkLanguageRule = context.alwaysThinkInPreferredLanguage
		? `\n- ALWAYS use ${context.preferredLanguage || "the user's preferred"} language for your internal reasoning (Chain of Thought) inside <thinking> blocks. This helps you structure your thoughts better for the user.`
		: ""
...
- MCP operations should be used one at a time, similar to other tool usage. Wait for confirmation of success before proceeding with additional operations.${thinkLanguageRule}`
}
...
- In PLAN MODE, depending on the user's request, gather information using codebase_search first for semantic discovery, then read_file for deeper inspection, and use search_files only for exact identifiers/patterns (prefer a narrow path and file_pattern).${context.yoloModeToggled !== true ? " You may also ask the user clarifying questions with ask_followup_question to get a better understanding of the task." : ""}
...
- When in doubt about existing patterns, conventions, or dependencies, **investigate first** using codebase_search, read_file, and search_files before making changes
