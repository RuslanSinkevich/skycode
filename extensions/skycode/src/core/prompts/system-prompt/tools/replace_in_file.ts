...
const id = SkycodeDefaultTool.FILE_EDIT
...
const BASE_DIFF_INSTRUCTIONS = `One or more SEARCH/REPLACE blocks following this exact format:
	...
	4. Special operations:
		...
		* To delete code: Use empty REPLACE section (must still include the ======= separator)
		* CRITICAL: When deleting code, do NOT insert comments like "<!-- Block deleted -->" or "// Code removed". Leave the REPLACE section completely EMPTY to remove the code entirely. We want clean deletions.`
...
const generic: SkycodeToolSpec = {
	...
	// Hide when simplified edit tools are enabled (for weaker models)
