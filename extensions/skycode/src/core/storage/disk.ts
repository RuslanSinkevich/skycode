...
export const GlobalFileNames = {
	...
	mcpSettings: "skycode_mcp_settings.json",
	skycodeRules: ".skycoderules",
	workflows: ".skycoderules/workflows",
	hooksDir: ".skycoderules/hooks",
	skycoderuleSkillsDir: ".skycoderules/skills",
	skycodeSkillsDir: ".skycode/skills",
...
/**
 * Returns the cross-platform path to the Skycode home directory (~/.skycode).
 * This works on macOS, Linux, and Windows:
 * - macOS: /Users/username/.skycode
 * - Linux: /home/username/.skycode
 * - Windows: C:\Users\username\.skycode
 *
 * This is intended to eventually replace ~/Documents/Skycode as the global config location.
 */
export function getSkycodeHomePath(): string {
	return path.join(os.homedir(), ".skycode")
}
...
export async function ensureRulesDirectoryExists(): Promise<string> {
	...
	const skycodeRulesDir = path.join(userDocumentsPath, "Skycode", "Rules")
	try {
		await fs.mkdir(skycodeRulesDir, { recursive: true })
	} catch (_error) {
		return path.join(os.homedir(), "Documents", "Skycode", "Rules") // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine because we will fail gracefully with a path that does not exist
	}
	return skycodeRulesDir
}

export async function ensureWorkflowsDirectoryExists(): Promise<string> {
	const userDocumentsPath = await getDocumentsPath()
	const skycodeWorkflowsDir = path.join(userDocumentsPath, "Skycode", "Workflows")
	try {
		await fs.mkdir(skycodeWorkflowsDir, { recursive: true })
	} catch (_error) {
		return path.join(os.homedir(), "Documents", "Skycode", "Workflows") // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine because we will fail gracefully with a path that does not exist
	}
	return skycodeWorkflowsDir
}

export async function ensureMcpServersDirectoryExists(): Promise<string> {
	const userDocumentsPath = await getDocumentsPath()
	const mcpServersDir = path.join(userDocumentsPath, "Skycode", "MCP")
	try {
		await fs.mkdir(mcpServersDir, { recursive: true })
	} catch (_error) {
		return path.join(os.homedir(), "Documents", "Skycode", "MCP") // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine since this path is only ever used in the system prompt
	}
...
export async function ensureHooksDirectoryExists(): Promise<string> {
	const userDocumentsPath = await getDocumentsPath()
	const skycodeHooksDir = path.join(userDocumentsPath, "Skycode", "Hooks")
	try {
		await fs.mkdir(skycodeHooksDir, { recursive: true })
	} catch (_error) {
		return path.join(os.homedir(), "Documents", "Skycode", "Hooks") // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine because we will fail gracefully with a path that does not exist
	}
	return skycodeHooksDir
}

/**
 * Returns the global skills directory path (~/.skycode/skills).
 * Creates the directory if it doesn't exist.
 */
export async function ensureSkillsDirectoryExists(): Promise<string> {
	const skycodeSkillsDir = path.join(getSkycodeHomePath(), "skills")
	try {
		await fs.mkdir(skycodeSkillsDir, { recursive: true })
	} catch (_error) {
		// Fallback - return the path even if mkdir fails, we'll fail gracefully later
		return skycodeSkillsDir
	}
	return skycodeSkillsDir
}
...
export async function getSavedSkycodeMessages(taskId: string): Promise<SkycodeMessage[]> {
...
export async function saveSkycodeMessages(taskId: string, uiMessages: SkycodeMessage[]) {
	try {
...
export async function collectEnvironmentMetadata(): Promise<Omit<EnvironmentMetadataEntry, "ts">> {
	try {
		...
		return {
			...
			skycode_version: ExtensionRegistryInfo.version,
		}
	} catch (error) {
 ...
 * 2. Each workspace root's .skycoderules/hooks directory (if they exist)
 *
...
/**
 * Gets the paths to the workspace's .skycoderules/hooks directories to search for
	...
	return (
		await Promise.all(
			workspaceRootPaths.map(async (workspaceRootPath) => {
				// Look for a .skycoderules/hooks folder in this workspace root.
