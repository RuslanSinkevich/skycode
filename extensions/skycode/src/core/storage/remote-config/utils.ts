...
export function transformRemoteConfigToStateShape(remoteConfig: RemoteConfig): Partial<RemoteConfigFields> {
	...
	const skycodeSettings = remoteConfig.providerSettings?.Skycode
	if (skycodeSettings) {
		transformed.planModeApiProvider = "skycode"
		transformed.actModeApiProvider = "skycode"
		providers.push("skycode")
	}
...
export function clearRemoteConfig() {
	try {
		...
		// the remote config skycode rules toggle state is stored in global state
