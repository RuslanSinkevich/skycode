import { getSavedSkycodeMessages, getTaskMetadata, readTaskHistoryFromState, writeTaskHistoryToState } from "@core/storage/disk"
...
async function reconstructTaskHistoryItem(taskId: string): Promise<HistoryItem | null> {
	try {
		...
		const skycodeMessages = await getSavedSkycodeMessages(taskId)
		if (skycodeMessages.length === 0) {
		...
		const taskInfo = extractTaskInformation(skycodeMessages, metadata)
...
function extractTaskInformation(skycodeMessages: SkycodeMessage[], metadata: any): TaskInfo {
	// Find the first user message (task description)
	const firstUserMessage = skycodeMessages.find((msg) => msg.type === "say" && msg.say === "text" && msg.text)

	// Extract timestamp from first message or use task ID as fallback
	const timestamp = skycodeMessages.length > 0 ? skycodeMessages[0].ts : Date.now()
	...
	const apiReqMessages = skycodeMessages.filter((msg) => msg.type === "say" && msg.say === "api_req_started" && msg.text)
	...
	const messageSize = JSON.stringify(skycodeMessages).length
