...
├── spec.ts                  # Интерфейс SkycodeToolSpec
├── types.ts                 # Общие типы
├── index.ts                 # Barrel export
├── registry/
│   ├── SkycodeToolSet.ts       # Управление инструментами
...
interface PromptVariant {
  ...
  tools?: SkycodeDefaultTool[]     // Список инструментов
...
```markdown
You are Skycode AI, a skilled software engineer...
...
Каждый инструмент определяется через `SkycodeToolSpec`:

```typescript
const generic: SkycodeToolSpec = {
  variant: ModelFamily.GENERIC,
  id: SkycodeDefaultTool.FILE_READ,
  name: "read_file",
