...
### Что делать
**Файл:** `vscode/extensions/skycode/src/core/indexing/IndexingService.ts`
...
### Что делать
**Файл:** `vscode/extensions/skycode/src/core/indexing/providers/LocalEmbeddingProvider.ts`
...
for (let i = 0; i < texts.length; i++) {
    ...
    if (text.length > MAX_CHUNK_CHARS) {
        console.warn(`[Skycode Indexing] Skipping chunk ${i}: too large (${text.length} chars > ${MAX_CHUNK_CHARS})`)
...
### Что делать
**Файл:** `vscode/extensions/skycode/src/core/indexing/CodeChunker.ts`
...
**Файл:** `vscode/extensions/skycode/src/core/indexing/TreeSitterChunker.ts`
...
### Что делать
**Файл:** `vscode/extensions/skycode/src/core/indexing/SearchEngine.ts`
...
### Что делать
**Файл:** `vscode/extensions/skycode/src/core/indexing/storage/VectorSearch.ts`
...
**Файл:** `vscode/extensions/skycode/src/core/indexing/storage/KeywordSearch.ts`
...
function isDebugEnabled(): boolean {
	try {
		...
		return vscode.workspace.getConfiguration("skycode.indexing").get("debug", false)
	} catch {
		return false
	}
}
...
Конкретно строки:
- `console.log(`[Skycode Search] Query:...` → `if (isDebugEnabled()) console.log(`[Skycode Search] Query:...`
- `console.log(`[Skycode Search] Semantic:...` → `if (isDebugEnabled()) console.log(`[Skycode Search] Semantic:...`
- `console.log(`[Skycode Search] Keyword:...` → `if (isDebugEnabled()) console.log(`[Skycode Search] Keyword:...`
- `console.log(`[Skycode Search] Merged:...` → `if (isDebugEnabled()) console.log(`[Skycode Search] Merged:...`
- `console.log(`[Skycode Search] Reranked:...` → `if (isDebugEnabled()) console.log(`[Skycode Search] Reranked:...`
- `console.log(`[Skycode Search] Final:...` → `if (isDebugEnabled()) console.log(`[Skycode Search] Final:...`

### Проверка
По умолчанию логи не выводятся. Для отладки: `skycode.indexing.debug: true` в настройках.
...
После КАЖДОЙ задачи:
1. `cd vscode/extensions/skycode && node esbuild.mjs` — пересобрать extension
