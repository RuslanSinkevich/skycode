...
Инструменты `web_search` и `web_fetch` **не работают**. Оба обработчика отправляют HTTP-запросы на бэкенд Skycode:

```
POST https://skycode-ai.ru/api/v1/search/websearch
POST https://skycode-ai.ru/api/v1/search/webfetch
```

Эти эндпоинты **не существуют** в `skycode-web`. Результат — **404 Not Found**.

### Почему так получилось

Код унаследован от Roo Code (бывший Cline). У них web-инструменты работали через их облачный бэкенд. При форке `apiBaseUrl` был переключён на `skycode-ai.ru`, но серверные роуты не были реализованы.

### Текущее поведение

1. Модель вызывает `web_search` или `web_fetch`
2. Обработчик проверяет `skycodeWebToolsEnabled` (включён по умолчанию) и авторизацию
...
```
AI модель → WebSearchToolHandler → axios.post(skycode-ai.ru) → 404
AI модель → WebFetchToolHandler  → axios.post(skycode-ai.ru) → 404
```
...
- Импорты `SkycodeEnv`, `AuthService`, `buildSkycodeExtraHeaders`
- Проверку `skycodeWebToolsEnabled`
- Проверку `authToken`
...
```typescript
// БЫЛО:
const skycodeWebToolsEnabled = config.services.stateManager.getGlobalSettingsKey("skycodeWebToolsEnabled")
if (!skycodeWebToolsEnabled) {
  return formatResponse.toolError("Skycode web tools are currently disabled in settings.")
}
...
```typescript
// БЫЛО:
const baseUrl = SkycodeEnv.config().apiBaseUrl
...
### 4a. Убрать проверку `skycodeWebToolsEnabled` из обработчиков

Раньше эта настройка контролировала доступ к облачным API. При локальной работе она не нужна.

**Файлы:**
- `WebSearchToolHandler.ts` — убрать блок `if (!skycodeWebToolsEnabled)`
- `WebFetchToolHandler.ts` — убрать блок `if (!skycodeWebToolsEnabled)`

### 4b. Убрать `contextRequirements` из спецификаций промптов

**Файлы:**
- `src/core/prompts/system-prompt/tools/web_search.ts` — убрать строку `contextRequirements: (context) => context.skycodeWebToolsEnabled === true`
- `src/core/prompts/system-prompt/tools/web_fetch.ts` — убрать строку `contextRequirements: (context) => context.skycodeWebToolsEnabled === true`

Без этого изменения инструменты **не появятся в системном промпте**, если `skycodeWebToolsEnabled === false`. А настройка в UI станет бессмысленной.
...
```json
"chat.wantsToWebSearch": "Skycode AI хочет найти в интернете:",
"chat.webSearched": "Skycode AI искал в интернете:",
...
```json
"chat.wantsToWebSearch": "Skycode AI wants to search the web:",
"chat.webSearched": "Skycode AI searched the web:",
...
**`ru.json`:**
```json
"features.skycodeWebTools": "Включить веб-инструменты Skycode",
"features.skycodeWebToolsDescription": "Включает инструменты веб-поиска и веб-запросов для AI-ассистента.",
"features.skycodeWebToolsAuthRequired": "Для использования веб-инструментов необходимо авторизоваться.",
"features.skycodeWebToolsSignIn": "Войти в аккаунт",
```

**`en.json`:**
```json
"features.skycodeWebTools": "Enable Skycode web tools",
"features.skycodeWebToolsDescription": "Enables web search and web fetch tools for the AI assistant.",
"features.skycodeWebToolsAuthRequired": "Authentication required to use web tools.",
"features.skycodeWebToolsSignIn": "Sign in",
```

### Что нужно изменить

1. **Если оставляем тогл**, но убираем требование авторизации:
   - Удалить ключи `features.skycodeWebToolsAuthRequired` и `features.skycodeWebToolsSignIn` из обоих файлов
   - Обновить `features.skycodeWebToolsDescription`:
...
2. **Если убираем тогл полностью** — удалить все 4 ключа `features.skycodeWebTools*` из обоих файлов.
...
| `webview-ui/src/i18n/locales/ru.json` | **ОБНОВИТЬ** — изменить/удалить ключи `features.skycodeWebTools*`, добавить `browser.downloadingChromium` / `browser.chromiumReady` |
| `webview-ui/src/i18n/locales/en.json` | **ОБНОВИТЬ** — изменить/удалить ключи `features.skycodeWebTools*`, добавить `browser.downloadingChromium` / `browser.chromiumReady` |
...
5. **`skycodeWebToolsEnabled`** — после удаления серверной зависимости можно:
...
8. **Скачивание Chromium — предзагрузка при включении тогла.**
   ...
   **Правильный UX:** Chromium скачивается **заранее**, в момент когда пользователь включает галочку «Веб-инструменты» в настройках. Не при первом запросе модели.
   ...
   1. Пользователь включает тогл `skycodeWebToolsEnabled` в `ExperimentsSection.tsx`
   2. Срабатывает `updateSetting("skycodeWebToolsEnabled", true)`
   ...
   **Вариант: `vscode.window.withProgress()` с уведомлением внизу IDE**
   ...
   ```typescript
   // В updateSettings.ts, после сохранения skycodeWebToolsEnabled = true:
   import { ensureChromiumExists } from "@services/browser/utils"

   if (request.skycodeWebToolsEnabled === true) {
