...
**Проблема:** `rejectAllPendingChanges` падал с `CodeExpectedError` при попытке открыть файл, который уже был удалён (например, `test-skycode.ts`).
...
**Решение:**
- `hunkId` сохраняется в `WriteDiffBlock` и передаётся в `SkycodeSayTool`
