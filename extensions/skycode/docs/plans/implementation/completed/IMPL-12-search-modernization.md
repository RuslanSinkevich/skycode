...
Skycode AI использует **два механизма поиска** по кодовой базе:
...
- Перебилдь extension: `node esbuild.mjs` в `extensions/skycode/`
- `Developer: Reload Window`
...
2. Перебилдь webview (только если трогал UI):
   ```powershell
   cd D:\Users\Admin\Desktop\Skycode\vscode\extensions\skycode\webview-ui
   npm run build
   ```
...
4. Тесты:
   ...
   - Посмотри лог в Output → Skycode: нет ошибок, guardrail срабатывает при широких запросах
