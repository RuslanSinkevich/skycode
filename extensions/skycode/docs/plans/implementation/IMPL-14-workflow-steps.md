...
4. Orchestrator:
   ...
   - На каждом шаге:
     ...
     c. Запускает agent loop (вызывает `recursivelyMakeSkycodeRequests`)
...
steps:
  ...
  - name: "Refine Plan Against Rules"
    ...
    visible: true
    prompt: |
      Проверь созданный план на соответствие правилам проекта (skycode rules).
...
#### Protobuf (`proto/skycode/workflow.proto` — новый файл)
...
Ключевой компонент. Оркестрирует последовательное выполнение шагов workflow. Работает как надстройка над существующим `initiateTaskLoop` / `recursivelyMakeSkycodeRequests`.
...
- Работает с существующим `recursivelyMakeSkycodeRequests` без изменений
...
export class WorkflowOrchestrator {
  ...
  private async executeStep(stepPrompt: string): Promise<void> {
    const userContent: SkycodeContent[] = [
      { type: "text", text: stepPrompt }
    ]
    
    // Используем существующий механизм:
    // - recursivelyMakeSkycodeRequests выполняет agent loop
...
class Task {
   ...
   */
  async initiateStepLoop(userContent: SkycodeContent[]): Promise<void> {
...
if (matchingWorkflow && !matchingWorkflow.isRemote) {
  ...
  if (isMultiStepWorkflow(matchingWorkflow.fullPath)) {
    ...
    return {
      ...
      needsSkycoderulesFileCheck: false,
...
#### В SkycodeRulesToggleModal (вкладка Workflows)
...
| `proto/skycode/workflow.proto` | Protobuf определения для WorkflowService |
...
| `proto/skycode/file.proto` | Опционально: метод `loadWorkflow` |
| `webview-ui/src/components/skycode-rules/SkycodeRulesToggleModal.tsx` | Кнопка Edit для .yaml workflows |
...
4. Protobuf определения в `proto/skycode/workflow.proto`
...
4. Кнопка "+ New Workflow" в SkycodeRulesToggleModal
...
- [ ] Запуск через `.\scripts\code.bat` — Skycode работает
