 ...
 *
 * Структура на диске:
 *   ~/.skycode/indexing/{workspace-hash}/
...
### Файл 2: `vscode/extensions/skycode/esbuild.mjs` (ИЗМЕНИТЬ)
...
Выполнить в `vscode/extensions/skycode/`:
...
1. Удалить папку `~/.skycode/indexing/` чтобы начать с чистого состояния
2. Собрать: `node esbuild.mjs`
3. Запустить форк
4. Нажать "Переиндексировать"
5. Проверить что в `~/.skycode/indexing/{hash}/` создался файл `index.db` (а не JSON файлы)
