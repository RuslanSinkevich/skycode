# Cursor-style execution flow для Skycode AI

## Как было (Skycode, до 2026-02-06)
...
Ключевые отличия от старого Skycode:
...
4. **Security checks остались** — skycodeignore, command permissions проверяются ДО выполнения
...
[Модель вызывает execute_command: rm -rf node_modules]
    → Проверка commandPermissionController (SKYCODE_COMMAND_PERMISSIONS)
...
- Security checks (skycodeignore, command permissions) — остались на месте
