# Skycode AI — Дорожная карта (Roadmap)

> **Цель:** Skycode AI = полный аналог Cursor. VSCode форк трогаем минимально, вся логика в `extensions/skycode/`.

*Создано: 2026-02-06 | Обновлено: 2026-03-01*

---

## Аудит: Skycode AI vs Cursor

### 1. Инструменты (Tools)

| Cursor Tool | Skycode AI Аналог | Статус |
|---|---|---|
...
| Функция Cursor | Skycode AI | Статус |
|---|---|---|
...
| Workspace rules (.cursor/rules/) | user_instructions + .skycode/rules | ✅ Есть (IMPL-08) |
...
- [x] **Голосовой ввод (Voice Input)** — локальная транскрипция речи ✅ (2026-02-21/22)
  ...
  - Загрузка компонентов одним архивом с CDN (skycode-ai.ru) с Bearer JWT авторизацией
...
- [x] **Ребрендинг остатков Cline** — замена иконок терминала, SVG логотипа, шрифтовых иконок на Skycode ✅ (2026-03-01)
  - Иконка терминала: `ThemeIcon("skycode-icon")` вместо Cline robot
  - Шрифтовые файлы `skycode-bot.woff`/`.ttf` пересозданы из SVG логотипа "S"
  - SVG `SkycodeCompactIcon.tsx` заменён на логотип Skycode
  - Заголовок таба чата: `{ value: 'Skycode', original: 'Skycode' }` (без localize2, чтобы русская локализация не перезаписывала)
...
- [x] **Registration Gate + Сайт** — skycode-web (Next.js 16 + Auth.js v5), лендинг, auth (Яндекс ID / Google / Email + password), личный кабинет, админ-панель, система баннеров, защищённые скачивания, Docker-деплой на VPS ✅ (2026-02-22/23)
