...
| `src/shared/SkycodeBanner.ts` | Типы API-ответа: `Banner`, `BannersResponse`, `BannerRules` |
| `src/shared/skycode/banner.ts` | Типы для UI: `BannerCardData`, `BannerAction`, `BannerActionType`, `BANNER_DATA` |
...
Хранение состояния: `localStorage` ключ `skycode_promo_modal`
...
Правила показа:
1. Пользователь не залогинен (`skycodeUser === null`) → Free
