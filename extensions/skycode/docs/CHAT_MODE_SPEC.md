...
Новый пятый режим работы Skycode AI — **«Chat»** (Чат).
...
private static readonly CHAT_MODE_RESTRICTED_TOOLS: SkycodeDefaultTool[] = [
    ...
    SkycodeDefaultTool.DELETE_BLOCK,
    SkycodeDefaultTool.REPLACE_TEXT,
]
```
...
Проверить файлы:
- `proto/skycode/state.proto` — если `Mode` определён как enum, добавить `CHAT = 4;`
- `proto/skycode/models.proto` — если там есть ссылки на Mode
...
| 8 | `proto/skycode/state.proto` | Добавить `CHAT = 4` в enum Mode (если есть) |
